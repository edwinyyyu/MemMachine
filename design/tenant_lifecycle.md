...
Status: proposal, under review in its own PR against `speedkick`, which
carries the segment store overhaul it refers to (#1548). Shared reference for
...
### SegmentStore (shipped in #1548; adjustments marked)
...
- `reclaim_some(key)`: to be added for the reconciler, the bounded step
  for one key: reclaims this key's dead generations, oldest first at the
  clock's resolution; False means exactly "no garbage under this key"; on
  SQLite the DELETE waits on the write lock up to the driver's busy
  timeout and raises past it, which the reconciler treats as "retry
  later". The overhaul (#1545, merged as #1548) built and then removed an
  earlier form of this (`purge_partition`) because it was tied to an inline drain on the
  delete path; the step returns here because single-use keys make each
  reclaim one tenant's job, with progress, retry and failure attributable
  to that tenant, which the global sweeper cannot do.
...
- #1548 (the segment store overhaul, on `speedkick`): kept as is at the
  store level. It already deletes and returns,
  with reclamation by the sweeper within its interval; the nudge and the
  per-tenant `reclaim_some` step are added here, not there.
...
- Keys, to confirm: the text adopts `UUID`-only store keys minted per
  ...
  store ABCs and belongs to the tenant-layer series, not #1548. The
