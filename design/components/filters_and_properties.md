...
- System fields are stored under reserved keys:
  `memmachine_event_timestamp`, `memmachine_event_author` (the author
  id; the name is never stored in a vector record),
...
- A field in a tree must pass `validate_caller_property_key`; system
  ...
  operation (`since`, `before`, `authors`) that the subsystem turns
