...
`SELECT ... WHERE partition_key = :key AND incarnation = :incarnation FOR
SHARE` and raises a stale-partition error when no row matches; reads perform
the same check without the lock. A handle held across delete, or across
