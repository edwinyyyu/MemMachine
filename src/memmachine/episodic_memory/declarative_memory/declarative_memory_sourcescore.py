...
class DeclarativeMemory:
    ...
    async def _score_episode_contexts(
        ...
        source_scores = [
            0.2 if episodes[0].source == "Assistant" else 1.0
            for episodes in episode_contexts
        ]

        episode_context_scores = [
            score * (1 - (1 - source_score) * (1 - score))
            # score * source_score
            for score, source_score in zip(episode_context_scores, source_scores)
        ]
