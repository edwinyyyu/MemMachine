...
class DeclarativeMemory:
    ...
    async def search_scored(
        ...
        *,
        ...
        derivative_origin_segment_uuids = dict.fromkeys(
            UUID(cast(str, matched_derivative_record.properties["_segment_uuid"]))
            for matched_derivative_record in matched_derivative_records
            if matched_derivative_record.properties is not None
        )
    ...
    async def forget_all_episodes(self) -> None:
        ...
        record_uuids = await self._segment_store.delete_all_segments(
            session_key=self._session_key,
        )
        await self._collection.delete(record_uuids=record_uuids)
