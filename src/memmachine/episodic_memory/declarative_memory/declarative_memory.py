...
class DeclarativeMemory:
    ...
    async def expand_episode_context(
        self,
        episode_context: Iterable[Episode],
        *,
        backward_expansion_quota: int = 100,
        forward_expansion_quota: int = 200,
        max_backward_episodes: int = 4,
        max_forward_episodes: int = 8,
    ) -> list[Episode]:
        """
        Expand episode context to more neighboring episodes.

        Args:
            episode_context (Iterable[Episode]):
                The episode context to expand.
        Returns:
            list[Episode]:
                The expanded episode context.
        """
        episode_context = deque(episode_context)
        first_episode = episode_context[0]
        last_episode = episode_context[-1]
        ...
        previous_episodes_content_length = 0
        for previous_episode in previous_episodes:
            episode_context.appendleft(previous_episode)
            previous_episodes_content_length += len(previous_episode.content)
            if previous_episodes_content_length >= backward_expansion_quota:
                break

        next_episodes_content_length = 0
        for next_episode in next_episodes:
            episode_context.append(next_episode)
            next_episodes_content_length += len(next_episode.content)
            if next_episodes_content_length >= forward_expansion_quota:
                break
