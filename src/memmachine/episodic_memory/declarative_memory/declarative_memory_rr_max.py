...
class DeclarativeMemory:
    ...
    async def _score_episode_contexts(
        ...
        episode_contexts = list(episode_contexts)

        context_strings = [
            DeclarativeMemory.string_from_episode_context([episode])
            for episode_context in reversed(episode_contexts)
            for episode in episode_context
        ]

        episode_scores = await self._reranker.score(query, context_strings)
        episode_context_scores = []
        for episode_context in episode_contexts:
            max_score = -float("inf")
            for _ in episode_context:
                score = episode_scores.pop()
                max_score = max(max_score, score)
            episode_context_scores.append(max_score)
