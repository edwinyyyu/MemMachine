...
# from memmachine.common.ou import OrnsteinUhlenbeckRecruiter
...
class DeclarativeMemory:
    ...
    def __init__(self, params: DeclarativeMemoryParams) -> None:
        ...
        # self._ou_recruiter = OrnsteinUhlenbeckRecruiter(
        #     num_values=self._embedder.dimensions,
        #     num_recruits=int(self._embedder.dimensions // 16),
        # )
        ...
        self._compression_content_length_threshold = 600
        self._compression_prompt_template = (
            "<text>\n"
            "{text}\n"
            "</text>\n"
            "\n"
            "Compress the text, preserving meaning exactly. Preserve causal relationships and antecedents for references from the original text. If it's unclear due to missing context, ensure that the sentence structure is flexible enough to be used in the same context as the original text. Do not add fluff. Respond under 150 words."
        )
    ...
    async def add_episodes(
        ...
        # preprocess_episode_tasks = [
        #     self._preprocess_episode(episode) for episode in episodes
        # ]
        # episodes = await asyncio.gather(*preprocess_episode_tasks)
        ...
        embedding_tasks = [
            self._embedder.ingest_embed(
                [derivative.content for derivative in derivatives],
            ),
            self._embedder.ingest_embed(
                [derivative.additional for derivative in derivatives],
            ),
        ]

        derivative_embeddings, derivative_additional_embeddings = await asyncio.gather(
            *embedding_tasks
        )
        ...
        derivative_nodes = [
            Node(
                ...
                properties={
                    ...
                    "additional": derivative.additional,
                }
                | {
                    mangle_filterable_property_key(key): value
                ...
                embeddings={
                    ...
                    DeclarativeMemory._embedding_name(
                        f"{self._embedder.model_id}_additional",
                        self._embedder.dimensions,
                    ): (additional_embedding, self._embedder.similarity_metric),
                },
            )
            for derivative, embedding, additional_embedding in zip(
                derivatives,
                derivative_embeddings,
                derivative_additional_embeddings,
                strict=True,
            )
        ]
    ...
    # async def _preprocess_episode(
    #     self,
    #     episode: Episode,
    # ) -> Episode:
    #     """
    #     Preprocess episode before adding to memory.

    #     Args:
    #         episode (Episode):
    #             The episode to preprocess.

    #     Returns:
    #         Episode:
    #             The preprocessed episode.

    #     """
    #     match episode.content_type:
    #         case ContentType.MESSAGE | ContentType.TEXT:
    #             if len(episode.content) <= self._compression_content_length_threshold or self._language_model is None:
    #                 return episode

    #             compressed_content, _ = await self._language_model.generate_response(
    #                 system_prompt=self._compression_prompt_template.format(
    ...
    async def _pattern_separation(
        self,
        embeddings: Iterable[list[float]],
    ) -> list[list[float]]:
        """
        Apply pattern separation to embeddings.

        Args:
            embeddings (list[list[float]]):
                The embeddings to apply pattern separation to.
            ...
            case ContentType.MESSAGE:
                return [
                    Derivative(
                        ...
                        additional=f"{episode.source}: {episode.additional}",
                        filterable_properties=episode.filterable_properties,
                    )
                ]
    ...
    async def search(
        ...
        *,
        ...
        # query_embedding = datetime_rotary_decay(
        #     query_embedding, datetime.datetime.now(),
        # )
    ...
    @staticmethod
    def string_from_episode_context_additional(
        ...
        for episode in episode_context:
            match episode.content_type:
                case ContentType.MESSAGE:
                    ...
                    context_string += f"[{context_date} at {context_time}] {episode.source}: {json.dumps(episode.additional)}\n"
                case ContentType.TEXT:
                    context_string += json.dumps(episode.content) + "\n"
    ...
    def _episode_from_episode_node(episode_node: Node) -> Episode:
        return Episode(
            ...
            additional=episode_node.properties.get("additional"),
