...
class DeclarativeMemory:
    ...
    async def _accumulate_episodes(
        ...
        episodes = sorted(
            episodes, key=lambda episode: (episode.timestamp, episode.uid)
        )

        accumulated_episode_context: list[Episode] = []
        while len(accumulated_episode_context) < limit and len(episodes) > 0:
            candidates = [
            ...
            best_episode = episodes.pop(best_episode_index)
            accumulated_episode_context.append(best_episode)
