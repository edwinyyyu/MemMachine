...
class DeclarativeMemory:
    ...
    async def search(
        ...
        *,
        ...
        accumulated_episodes = await self._accumulate_episodes(
            query,
            nuclear_episodes,
            episode_contexts,
    ...
    async def _accumulate_episodes(
        ...
        available_episodes = dict.fromkeys(nuclear_episodes)

        episode_neighbors = defaultdict(set)
        previous_episode = None
        for episode_context in episode_contexts:
            for episode in episode_context:
                if previous_episode is not None:
                    episode_neighbors[episode].add(previous_episode)
                    episode_neighbors[previous_episode].add(episode)
                previous_episode = episode

        accumulated_episode_context: set[Episode] = set()
        while len(accumulated_episode_context) < limit and len(available_episodes) > 0:
            candidates = [
            ...
            best_episode_index = scores.index(max(scores))

            best_episode = list(available_episodes.keys())[best_episode_index]
