...
class DeclarativeMemory:
    ...
    async def search(
        ...
        *,
        ...
        search_derivative_nodes_results = await asyncio.gather(
            *search_derivative_nodes_tasks,
        )
        ...
        similar_user_derivative_nodes = search_derivative_nodes_results[0][0]
        similar_assistant_derivative_nodes = search_derivative_nodes_results[1][0]

        matched_derivative_nodes = similar_user_derivative_nodes + similar_assistant_derivative_nodes
        ...
        contextualize_episode_tasks = [
            self._llm_contextualize_episode(
                query,
                nuclear_episode,
        ...
        unique_episode_contexts = dict.fromkeys(
            tuple(episode_context)
            for episode_context in await asyncio.gather(*contextualize_episode_tasks)
            if episode_context
        )

        episode_context_scores = await self._score_episode_contexts(
            query,
            unique_episode_contexts.keys(),
        )
        ...
        #     for nuclear_episode, episode_context in zip(
        #         nuclear_episodes,
        #         episode_contexts,
        #     )
        # ]
        ...
        unified_episode_context = DeclarativeMemory._unify_episode_contexts(
            reranked_episode_contexts,
    ...
    class ContributingIndexes(BaseModel):
        indexes: list[int]

    async def _llm_contextualize_episode(
        self,
        query: str,
        nuclear_episode: Episode,
        max_backward_episodes: int = 7,
        max_forward_episodes: int = 7,
        mangled_property_filter: FilterExpr | None = None,
    ) -> list[Episode]:
        previous_episode_nodes = (
            await self._vector_graph_store.search_directional_nodes(
        ...
        formatted_chunks = "\n".join(
            f"{i}. [{DeclarativeMemory._format_date(episode.timestamp.date())} at {DeclarativeMemory._format_time(episode.timestamp.time())}] {episode.source}: {json.dumps(episode.content)}"
            for i, episode in enumerate(all_episodes)
        )

        if "Tiger" in formatted_chunks:
            print(f"Tiger found")

        contextualize_system_prompt = (
            "You are part of a distributed system with access to different chunks. Your task is to return the indexes of all chunks which may contribute to answering the query.\n"
            "\n"
            "<instructions>\n"
            "1. Consider whether the chunk contributes directly to answering the query.\n"
            "2. Consider the implications of each chunk beyond what is explicitly stated.\n"
            "3. Consider how each chunk may provide context for other chunks to produce a more precise final answer.\n"
            "4. There exist other chunks that are not visible to you. Be generous in determining whether a chunk may contribute to answering the query.\n"
            "5. Include any pieces of information that provide even tangentially related context to the query.\n"
            "6. Include any pieces of information that contradict or challenge the assumptions in the query, as they are important for determining the global relevance.\n"
            "</instructions>\n"
        )
        ...
        response = await self._language_model.generate_parsed_response(
            output_format=DeclarativeMemory.ContributingIndexes,
            system_prompt=contextualize_system_prompt,
            user_prompt=contextualize_user_prompt,
        )
        ...
        context = {
            all_episodes[i] for i in result if 0 <= i < len(all_episodes)
        }

        return sorted(context, key=lambda episode: (episode.timestamp, episode.uid))
    ...
    def _unify_anchored_episode_contexts(
        ...
        for nuclear_episode, context in anchored_episode_contexts:
            ...
            if (len(episode_set) + len(context)) <= max_num_episodes:
                ...
                if nuclear_episode in context:
                    nuclear_index = context.index(nuclear_episode)
                else:
                    nuclear_index = len(context) - 1
