...
class DeclarativeMemory:
    ...
    async def _contextualize_episode(
        ...
        max_backward_episodes: int = 10,
        max_forward_episodes: int = 10,
        ...
        start, end = DeclarativeMemory._get_percentile_span(
    ...
    @staticmethod
    def _get_percentile_span(index, distances):
        percentile = 70 - 4.4 * np.log(1 + len(distances))
        distance_threshold = float(np.percentile(distances, percentile))
