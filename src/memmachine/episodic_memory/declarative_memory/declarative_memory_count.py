...
class DeclarativeMemory:
    ...
    def _unify_episode_contexts(
        ...
        episode_scores: dict[Episode, list[int, int]] = {}

        for rank, context in enumerate(episode_contexts):
            for episode in context:
                episode_scores.setdefault(episode, [rank, 0])
                episode_scores[episode][1] += 1

        ranked_episodes = [
            episode
            for episode, (rank, count) in sorted(
