...
class DeclarativeMemory:
    ...
    async def _identify_useless_episodes(
        ...
        indexed_context = DeclarativeMemory.indexed_string_from_episode_context(episodes, start=0)
    ...
    async def _identify_useless_episodes_binary(
        ...
        0 = useful, 1 = useless.
        """
        indexed_context = DeclarativeMemory.indexed_string_from_episode_context(episodes, start=1)
    ...
    @staticmethod
    def indexed_string_from_episode_context(episode_context: Iterable[Episode], start: int = 0) -> str:
        """Format episode context as an indexed string."""
        context_string = ""

        for index, episode in enumerate(episode_context, start=start):
            context_date = DeclarativeMemory._format_date(
                episode.timestamp.date(),
            )
            context_time = DeclarativeMemory._format_time(
