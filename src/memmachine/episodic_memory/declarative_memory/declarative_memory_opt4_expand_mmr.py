...
class DeclarativeMemory:
    ...
    async def search(
        ...
        *,
        ...
        # Associate each episode with first matched derivative that led to it.
        # Note: dict preserves insertion order.
        source_episode_nodes: dict[Node, Node] = {}
        for derivative_node, episode_nodes in zip(
            ...
            for episode_node in episode_nodes:
                source_episode_nodes.setdefault(episode_node, derivative_node)
        ...
        source_derivative_embeddings = [
            ...
            for derivative_node in source_episode_nodes.values()
        ]

        pairwise_derivative_similarities = np.array([
            compute_similarity(
                source_derivative_embedding,
                source_derivative_embeddings,
                self._embedder.similarity_metric,
            )
            for source_derivative_embedding in source_derivative_embeddings
        ])
        min_similarity = np.min(pairwise_derivative_similarities)
        max_similarity = np.max(pairwise_derivative_similarities)

        normalized_derivative_similarities = (np.array(pairwise_derivative_similarities) - min_similarity) / (max_similarity - min_similarity + 1e-9)
        pairwise_time_differences_seconds = np.array(
            [
                abs(source_derivative_node.properties["timestamp"] - other_source_derivative_node.properties["timestamp"]).total_seconds()
                for other_source_derivative_node in source_episode_nodes.values()
            ]
            for source_derivative_node in source_episode_nodes.values()
        )

        temporal_decay = np.exp(-0.2 * pairwise_time_differences_seconds / (3600))

        pairwise_candidate_similarities = normalized_derivative_similarities * temporal_decay

        ranked_indexes = rank_by_mmr(
            candidate_relevances=episode_context_scores,
            pairwise_candidate_similarities=pairwise_candidate_similarities.astype(float).tolist(),
            lambda_param=0.5,
        )
