...
from memmachine.common.utils import rank_by_mmr, unflatten_like
...
class DeclarativeMemory:
    ...
    def __init__(self, params: DeclarativeMemoryParams) -> None:
        ...
        self._max_num_derivatives_per_episode = 10
    ...
    async def add_episodes(
        ...
        episodes_derivative_embeddings = unflatten_like(
        ...
        average_embeddings = [
            np.average(
                episode_derivative_embeddings,
                axis=0,
                weights=[len(derivative.content) for derivative in episode_derivatives],
            )
            .astype(float)
            .tolist()
            for episode_derivatives, episode_derivative_embeddings in zip(
                episodes_derivatives,
                episodes_derivative_embeddings,
                strict=True,
            )
        ]

        episodes_ranked_derivative_indexes = [
            rank_by_mmr(
                average_embedding,
                episode_derivative_embeddings,
                self._embedder.similarity_metric,
            )
            for average_embedding, episode_derivative_embeddings in zip(
                average_embeddings,
                episodes_derivative_embeddings,
                strict=True,
            )
        ...
        pruned_episodes_derivatives = []
        pruned_episodes_derivative_embeddings = []
        for (
            episode_ranked_derivative_indexes,
            episode_derivatives,
            episode_derivative_embeddings,
        ) in zip(
            episodes_ranked_derivative_indexes,
            ...
            pruned_episode_derivatives = []
            pruned_episode_derivative_embeddings = []
            for index in episode_ranked_derivative_indexes[
                : self._max_num_derivatives_per_episode
            ]:
                pruned_episode_derivatives.append(episode_derivatives[index])
                pruned_episode_derivative_embeddings.append(
                    episode_derivative_embeddings[index]
                )

            pruned_episodes_derivatives.append(pruned_episode_derivatives)
            pruned_episodes_derivative_embeddings.append(
                pruned_episode_derivative_embeddings
            )

        pruned_derivatives = [
            derivative
            for pruned_episode_derivatives in pruned_episodes_derivatives
            for derivative in pruned_episode_derivatives
        ]
        pruned_derivative_embeddings = [
            embedding
            for pruned_episode_derivative_embeddings in pruned_episodes_derivative_embeddings
            for embedding in pruned_episode_derivative_embeddings
        ]
        ...
        derivative_episode_edges = [
            ...
            for episode, pruned_episode_derivatives in zip(
                episodes,
                pruned_episodes_derivatives,
                strict=True,
            )
    ...
    async def _derive_derivatives(
        ...
        match episode.content_type:
            case ContentType.MESSAGE:
                ...
                derived_contents = {
                    f"[{message_timestamp}] {episode.source}: {episode.content}"
                }
                for line in episode.content.strip().splitlines():
                    derived_contents.update(sent_tokenize(line.strip()))
                ...
                return [
                    Derivative(
                        ...
                        content=f"[{message_timestamp}] {episode.source}: {derived_content}",
