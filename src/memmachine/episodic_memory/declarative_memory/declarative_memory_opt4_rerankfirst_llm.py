...
class DeclarativeMemory:
    ...
    async def search(
        ...
        *,
        ...
        # Rerank episodes.
        episode_scores = await self._score_episode_contexts(
            query,
        ...
        reranked_episodes = [
            nuclear_episode
            for _, nuclear_episode in sorted(
                zip(
                    episode_scores,
                    nuclear_episodes,
                    strict=True,
        ...
        contextualize_episode_tasks = [
            ...
            for nuclear_episode in reranked_episodes
        ]
    ...
    async def _llm_contextualize_episode(
        ...
        query: str,
        nuclear_episode: Episode,
        max_backward_episodes: int = 20,
        max_forward_episodes: int = 20,
        mangled_property_filter: FilterExpr | None = None,
    ) -> list[Episode]:
        previous_episode_nodes = (
            await self._vector_graph_store.search_directional_nodes(
        ...
        contextualize_system_prompt = (
            "Your task is to answer the query using the following chunks, and return the indexes of all chunks that contribute to your final answer.\n"
            "\n"
            "<instructions>\n"
            "1. Consider how each chunk provides context to every other chunk to produce a more precise final answer.\n"
            "2. Consider the implications of each chunk beyond what is explicitly stated.\n"
            "3. Do not worry about false positives. It is better to provide too much rather than too little.\n"
            "</instructions>\n"
        )
        ...
        contextualize_user_prompt = (
            ...
            "\n"
            f"Query: {query}\n"
            "Output the integer indexes (prepended to each chunk) of every chunk that contributes to your final answer."
        )
        ...
        result = response.output_parsed.indexes

        context = {
            all_episodes[i] for i in result if 0 <= i < len(all_episodes)
        }
        context.add(nuclear_episode)
