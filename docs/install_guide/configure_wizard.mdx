...
generated `uri` points at a local Milvus Lite file. Without those optional
extras, the wizard falls back to the local SQLiteVectorStore. You can edit the
generated `cfg.yml` afterward to point Milvus at a standalone server or Zilliz
Cloud by changing `uri`, `token`, and `db_name`.
