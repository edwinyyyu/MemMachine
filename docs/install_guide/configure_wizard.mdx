...
To generate a configuration that uses an optional VectorStore backend, install
the matching extra before running the wizard. For example:

```bash
pip install "memmachine-server[milvus]"
```
...
The wizard can generate an event-memory configuration backed by a VectorStore.
If the Qdrant extra is installed, Qdrant is selected by default. If Qdrant is
not installed but the Milvus extra is installed, Milvus is selected and the
generated `uri` points at a Milvus server on `http://localhost:19530`. Without
those optional extras, the wizard falls back to the local SQLiteVectorStore.
You can edit the generated `cfg.yml` afterward to point Milvus at another
server or Zilliz Cloud by changing `uri`, `token`, and `db_name`.

The relational database is configured to use SQLite by default. For users who
wish to use other relational databases, manual configuration of the `cfg.yml`
file will be necessary after running the wizard.
