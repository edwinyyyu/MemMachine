  ...
  <Accordion title="Resources">
    ...
    resources:
      ...
      **Parameter Descriptions:**
      ...
      | `config.collection_registry` | Qdrant and Milvus: the name of a relational database under `resources.databases` that holds the store's collection registry, which arbitrates collection creation and deletion across server processes. The registry keys its rows by the store's own id under `resources.databases`, so renaming that id orphans every collection registered under it. | *Required* |
