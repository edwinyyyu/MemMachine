...
<AccordionGroup>
  ...
  <Accordion title="Semantic Memory">
    ...
    **Parameters:**
    | Parameter           | Description                                                                 | Default    |
    |---------------------|-----------------------------------------------------------------------------|------------|
    | `enabled`           | Whether semantic memory is enabled. Auto-disabled when required fields are missing. Omit the whole `semantic_memory` section, or set only `enabled: false`, to turn semantic memory off. | `true` |
    | `llm_model`         | The ID of a language model defined in `resources.language_models` for semantic processing. Required when semantic memory is enabled. | `null` |
    | `embedding_model`   | The ID of an embedder defined in `resources.embedders` for creating embeddings. Required when semantic memory is enabled. | `null` |
    | `database`          | The ID of a database defined in `resources.databases` for semantic storage. Required when semantic memory is enabled. | `null` |
    | `config_database`   | The ID of a SQL database used for semantic configuration metadata. Required when semantic memory is enabled. | `null` |
  ...
  <Accordion title="Resources">
        ...
        databases:
          ...
          qdrant_vector_store:
            ...
            config:
              ...
              prefer_grpc: false
              https: false
              partition_registry: profile_storage
    ...
    resources:
        databases:
          ...
          qdrant_vector_store: # VectorStore for event or vector-backed semantic memory
            provider: qdrant # Qdrant server or Qdrant Cloud
            config:
              host: localhost # Qdrant host
              port: 6333 # REST port
              grpc_port: 6334 # gRPC port
              prefer_grpc: false # Use gRPC instead of REST
              https: false # Use TLS
              partition_registry: profile_storage # Relational database holding the partition registry
              request_timeout_seconds: 30 # Bounds every request to Qdrant
          ...
          milvus_vector_store: # VectorStore for event or vector-backed semantic memory
            provider: milvus # Milvus server or Zilliz Cloud
            config:
              uri: http://localhost:19530 # Milvus server or Zilliz Cloud endpoint
              ...
              partition_registry: profile_storage # Relational database holding the partition registry
              request_timeout_seconds: 30 # Bounds every request to Milvus
      ```
    </Tab>
    </Tabs>
      **Parameter Descriptions:**
      ...
      | `config.metrics_factory_id` | ID of the metrics exporter factory the Qdrant or Neo4j store reports its operation latencies to. | `prometheus` |
      | `config.partition_registry` | Qdrant and Milvus: the name of a relational database under `resources.databases` that holds the store's partition registry (see [Vector stores](/open_source/databases#vector-stores)). | *Required* |
      | `config.tombstone_retention_seconds` | Qdrant and Milvus: whole seconds a deleted partition's records are kept before its purge starts, so every write in flight at the deletion has landed and is reclaimed. | `86400` |
      | `config.request_timeout_seconds` | Qdrant and Milvus: whole seconds a request to the vector store may take before the client gives up; must be positive. | `30` |
      | `config.max_varchar_length` | Milvus: bytes a declared string property can hold, the length of its VARCHAR field; Milvus refuses a length above its `proxy.maxVarCharLength`. | `65535` |
      | `config.purge_batch_size` | Milvus: the most entities one purge round lists and deletes; Milvus refuses a query whose limit exceeds its `quotaAndLimits.limits.maxQueryResultWindow`. | `10000` |
      | `config.hnsw_config` | Qdrant: HNSW tuning of the store's collection in YAML form of `qdrant_client.models.HnswConfigDiff` (e.g. `payload_m`, `ef_construct`); `m` must be 0 or omitted. | unset |
      | `config.optimizers_config` | Qdrant: optimizer tuning of the store's collection, the YAML form of `qdrant_client.models.OptimizersConfigDiff`. | unset |
      | `config.quantization_config` | Qdrant: quantization of the store's collection, a single-key map selecting the method (`scalar`, `product`, `binary`, `turbo`) in the YAML form of `qdrant_client.models.QuantizationConfig`. | unset |
