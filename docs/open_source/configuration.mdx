        ...
        databases:
          ...
          milvus_vector_store: # VectorStore for event or vector-backed semantic memory
            ...
            config:
              ...
              registry_database: profile_storage # Relational database holding the collection registry
      ```
    </Tab>
    </Tabs>
      **Parameter Descriptions:**
      ...
      | `config.registry_database` | Qdrant and Milvus: the name of a relational database under `resources.databases` that holds the store's collection registry, which arbitrates collection creation and deletion across server processes. | *Required* |
