  ...
  <Accordion title="Resources">
        ...
        databases:
          ...
          qdrant_vector_store:
            ...
            config:
              ...
              prefer_grpc: false
              https: false
              collection_registry: profile_storage
    ...
    resources:
        databases:
          ...
          qdrant_vector_store: # VectorStore for event or vector-backed semantic memory
            ...
            config:
              ...
              https: false # Use TLS
              collection_registry: profile_storage # Relational database holding the collection registry
      ...
      **Parameter Descriptions:**
      ...
      | `config.collection_registry` | Qdrant and Milvus: the name of a relational database under `resources.databases` that holds the store's collection registry (see [Vector stores](/open_source/databases#vector-stores)). | *Required* |
      | `config.tombstone_retention_seconds` | Qdrant and Milvus: whole seconds a deleted collection's records are kept before its purge starts, so every write in flight at the deletion has landed and is reclaimed. | `86400` |
