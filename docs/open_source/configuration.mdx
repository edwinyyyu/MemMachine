  ...
  <Accordion title="Resources">
    ...
    resources:
      ...
      **Parameter Descriptions:**
      ...
      | `config.collection_registry` | Qdrant and Milvus: the name of a relational database under `resources.databases` that holds the store's collection registry, which arbitrates collection creation and deletion across server processes. Each backend has its own registry, named by the backend's key under `resources.databases`, which must be lowercase letters, digits and underscores, at most 32 bytes. | *Required* |
      | `config.tombstone_retention_seconds` | Qdrant and Milvus: whole seconds a deleted collection's registry entry outlives the first purge round that finds nothing under it, so a write that landed after that round is still reclaimed. | `86400` |
