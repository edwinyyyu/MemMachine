  ...
  <Accordion title="Resources">
    ...
    resources:
        databases:
          ...
          qdrant_vector_store: # VectorStore for event or vector-backed semantic memory
            ...
            config:
              ...
              registry_database: sqlite_test # Relational database (a name above) holding the partition registry; required
              tombstone_retention_seconds: 86400 # A deleted partition's tombstone outlives its first clean purge round this long
      ...
      **Parameter Descriptions:**
      ...
      | `config.registry_database` | Qdrant and Milvus: the name of a relational database under `resources.databases` that holds the store's partition registry and purge queue. | *Required* |
      | `config.tombstone_retention_seconds` | Qdrant and Milvus: whole seconds a deleted partition's tombstone is kept after a purge round found nothing left of it, before the tombstone itself is removed. | `86400` |
      ...
      | `config.metrics_factory_id` | Qdrant: ID of the metrics exporter factory the store reports its operation latencies to. | `prometheus` |
