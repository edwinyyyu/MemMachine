...
<AccordionGroup>
  ...
  <Accordion title="Episodic Memory">
    ...
    **Parameter Descriptions:**
    ...
    <Warning>
    **`properties_schema` creates database resources, and tenants share them.**
    Every project (tenant) declares its own `properties_schema`, and the schema
    is fixed for the life of the project: to change it, delete the project and
    create it again (its memories start empty). Filters may name any `m.<key>`;
    a declared key is indexed where the backend supports it, an undeclared key
    is evaluated without an index. On Qdrant and Milvus the vector store maps
    every distinct `(vector dimensions, properties_schema)` to one **physical
    collection** that all projects declaring that schema share, created the
    first time a project declares it and never dropped. One tenant's choices
    therefore change what other tenants get:

    - **Physical collections grow with distinct schemas.** A project declaring
      a schema nobody else uses creates a physical collection for the whole
      deployment. Qdrant Cloud allows 1000 collections per cluster by default
      and every collection carries its own resource overhead; Milvus allows
      65,536 collections per instance.
    - **Qdrant indexes every declared key** as a payload index on the shared
      physical collection, next to the system keys MemMachine indexes itself.
      Each index costs memory for every tenant's points, and Qdrant degrades
      with large index counts. MemMachine creates its Qdrant collections with
      strict mode disabled, so Qdrant Cloud's default caps (100 payload
      indexes, rejection of unindexed filters) do not apply to them;
      collections created by an earlier MemMachine keep their own setting.
    - **Qdrant evaluates an undeclared key per point, within the project.**
      Every query carries the project's indexed tenant key, so an undeclared
      `m.<key>` condition is checked against the payload of the project's own
      candidate points, read from disk by default. A selective undeclared
      filter either scans all of the project's points or fragments the graph
      traversal and returns fewer results. The cost lands on the querying
      project; other tenants feel it only as load on the shared node.
    - **Milvus indexes nothing.** Declared and undeclared keys are both stored
      as dynamic fields (65,536 bytes per row, all keys together), and the
      filter, tenant term included, is evaluated per row over the project's
      hash partition, which holds every tenant hashing there (1 of 16 by
      default). Milvus allows 64 fields and 1,024 partitions per collection.
    - **SQLite stores share nothing.** Each project gets its own tables (and,
      for the search-engine store, its own index file). `sqlite_vec` applies
      any filter after taking the `limit` nearest neighbors, so a selective
      filter returns fewer results than `limit`; the search-engine store
      evaluates the filter per visited candidate with one SQL statement each.
      Neither indexes a declared key in a way that changes this.
    </Warning>
  </Accordion>
  ...
  <Accordion title="Resources">
    ...
    resources:
      ...
      **Parameter Descriptions:**
      ...
      | `config.request_timeout` | Seconds a request to Qdrant or Milvus may take before the client gives up; bounds every remote write. | *Required for `qdrant` and `milvus`* |
