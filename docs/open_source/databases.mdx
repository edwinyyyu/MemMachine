...
SQLiteVecVectorStore. Qdrant and Milvus keep their partition registry (which
partitions exist, and which deleted ones await reclamation) in a relational
database named by `registry_database` (a relational database configured under
`resources.databases`, typically the one the segment store uses), so that
several server processes can create and delete partitions on one backend.
Deleting a partition unregisters it at once; its records are reclaimed by a
background purge, which keeps the partition's registry entry as a tombstone
until a purge round finds nothing, `tombstone_retention_seconds` (default a
day) have passed, and a round finds nothing again, so a write that landed
after an earlier round is still reclaimed. Keep the retention orders of
magnitude above `request_timeout_seconds`.
Milvus supports three deployment shapes through the same configuration
fields:

- **Milvus Lite:** set `uri` to a local `.db` path such as
  `./memmachine_milvus.db`.
  ...
  databases:
    milvus_vector_store:
      ...
      config:
        ...
        registry_database: profile_storage
