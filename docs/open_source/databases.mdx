...
SQLiteVecVectorStore. A store is one collection on its backend, named by
its vector store name, one per memory and embedder
(`long_term_memory__<embedder>` for the event-backed long-term memory,
`semantic_memory__<embedder>` for semantic memory), and each session's data
is a partition of it. Qdrant and Milvus keep the registry
...
bound to one node. Each store has its own registry there, named by its
vector store name (lowercase letters, digits and underscores, at most 32
bytes), so several stores can keep their registries in one database. Milvus supports
