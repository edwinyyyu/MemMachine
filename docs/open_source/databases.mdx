...
to one node. The registry keys its rows by the store's id under
`resources.databases`, so renaming that id orphans every collection registered
under it. Milvus supports three deployment shapes through the same
