...
PERSONAL_FILLER = [
    ...
    ("Any reminders for today?",
     "Nothing urgent. Want me to suggest a priority list?"),
    ("I had oatmeal for breakfast again.",
     "Consistent! Oatmeal is a solid choice. Any toppings today?"),
    ("Just coffee so far.",
     "Coffee first, food later — classic."),
    ("What time is sunset today?",
     "Around 7:42pm. Nice long evening."),
    ("I'm going for a walk.",
     "Have a good one! Weather looks fine."),
    ("Back from the walk, felt good.",
     "Glad to hear! Did you go your usual route?"),
    ("Yeah, the park loop.",
     "Nice. That one's about 2 miles, right?"),
    ("Any good podcasts to recommend?",
     "How about 'Huberman Lab' or 'The Daily'? Depends on mood."),
    ("I'll try The Daily. Thanks.",
     "Good choice, it's a short daily briefing."),
    ("What's the weather this weekend?",
     "Looks mild — 65 and sunny Saturday, 60 with clouds Sunday."),
    ("Perfect hiking weather.",
     "Indeed. Any trail in mind?"),
    ("Maybe Eagle Ridge.",
     "Classic. That's the 4-mile loop, right?"),
    ...
    ("Good idea. I have leftovers actually.",
     "Perfect — no waste."),
    ...
    ("What's a good dinner idea?",
     "How about salmon with roasted veggies?"),
    ("Already had fish this week.",
     "Then maybe chicken stir-fry — quick and easy."),
    ("I watched that new Netflix show.",
     "Oh nice, which one?"),
    ("The mystery one everyone's talking about.",
     "Ah yes, I've heard good things."),
    ("Laundry day today.",
     "Always satisfying to have it done."),
    ("Yep. Folding is the worst part though.",
     "Agreed, that's the tedious half."),
...
WORK_FILLER = [
    ("Standup in 10 minutes.",
     "Got it. Want me to prep a quick status bullet?"),
    ("Quick sync — anything pending on my side?",
     "Nothing critical. Mostly awaiting client feedback."),
    ("Reviewing PRs this morning.",
     "Good idea — merges tend to pile up mid-week."),
    ("Client email inbox is a mess.",
     "Want to triage together?"),
    ("Later. Just venting.",
     "Fair enough."),
    ("Team lunch at noon.",
     "Nice break. Where are you all going?"),
    ("The sandwich place downstairs.",
     "Classic choice."),
    ("Vanessa sent over the revised mockups.",
     "Great — quick turnaround from her."),
    ("Tom is blocked on the API spec.",
     "Want me to draft a quick outline?"),
    ...
    ("Right, that's what Maria set up.",
     "Yes, she configured it last week."),
    ("Running low on coffee. BRB.",
     "Priorities."),
    ("Back. Now what was I doing?",
     "You were reviewing the latest mockup revisions."),
    ("Right, thanks.",
     "No problem."),
    ("Any blockers I should escalate?",
     "Not that I'm aware of."),
    ("Ok quiet morning then.",
     "Make the most of it."),
    ("Team retro tomorrow.",
     "Want to prep any talking points?"),
    ("Later today.",
     "Sure."),
    ("I need to update the Notion board.",
     "Want me to help structure it?"),
    ("No, I got it.",
     "Alright."),
    ("Just pushed the hotfix.",
     "Good — anything to test manually?"),
    ("I already did. Green.",
     "Nice."),
    ("Vendor meeting moved to Friday.",
     "Noted. Friday at the usual time?"),
    ("Yeah, 2pm.",
     "Got it."),
    ...
    ("Working from home tomorrow.",
     "Sounds good. Any morning meetings to call out?"),
]


MEDICAL_FILLER = [
    ("Took my morning pills.",
     "Good. Metformin and lisinopril as usual?"),
    ("Yep.",
     "On track."),
    ("Walked 30 minutes again today.",
     "Nice — consistency really pays off for A1C."),
    ("Feeling pretty good today.",
     "Glad to hear."),
    ("Blood sugar reading this morning was 118.",
     "Solid morning number."),
    ("Had oatmeal for breakfast.",
     "Good choice — slow carbs, minimal spike."),
    ("Drank plenty of water today.",
     "Hydration matters a lot, especially on metformin."),
    ("No knee pain today, surprisingly.",
     "Some days are better. Good news."),
    ("Slept through the night last night.",
     "Wonderful! That's rare lately."),
    ("Quick stretch routine done.",
     "Helps the knee too."),
    ("Bought more vitamin D today.",
     "Good — running out would mess up the trend."),
    ("Bought groceries — low sugar week.",
     "Smart. Keeps the A1C trending down."),
    ("Had a small dessert though.",
     "A little is fine. Everything in moderation."),
    ("Doctor's office confirmed my appointment.",
     "Good. Dr. Patel, right?"),
    ("Yes. I'll write down the questions I want to ask.",
     "Great idea."),
    ("Wife went to her PT session today.",
     "How's her shoulder doing?"),
    ("Slowly improving.",
     "PT takes time but it works."),
    ("Skipped ibuprofen today. Knee was fine.",
     "Good — less NSAID use is safer with your lisinopril."),
    ("No sharp pains today.",
     "Encouraging."),
    ("Walking routine is becoming automatic.",
     "Habit formation at work."),
    ("Tried a yoga video this morning.",
     "Nice — easier on the joints than pounding pavement."),
    ("Liked it a lot.",
     "Could become a regular thing."),
    ("Making a salad for lunch today.",
     "Keep it simple and filling."),
    ("Added chickpeas for protein.",
     "Great move — fiber + protein."),
    ("Avoided dessert today.",
     "Nice."),
    ("Read up on glycemic index.",
     "Understanding the science helps a lot."),
    ("Finally figured out which carbs spike me most.",
     "That's useful data to carry forward."),
    ("White bread is definitely a no-go.",
     "Yeah, that's a common finding."),
    ("Switching to whole grain.",
     "Much better option."),
    ("Had my tea and did evening stretches.",
     "Nice wind-down routine."),
]


def _pick_filler(pool: list[tuple[str, str]], rng: random.Random,
    ...
    "synth_personal": [
        {
            ...
            "anchors": [
                "p_dinner_plan", "p_bob_peanut", "p_sarah_vegan",
                "p_patels_no_beef", "p_linda_no_mushrooms",
                "p_rachel_celiac", "p_bob_lactose",
                "p_emma_tree_nut", "p_sarah_soy",
        ...
        },
        {
            ...
            "category": "completeness",
            "anchors": [
                "p_bob_birthday", "p_rachel_xmas", "p_mom_anniversary",
            ],
        },
    ],
    "synth_work": [
        {
            ...
            "category": "completeness",
            "anchors": [
                "w_brand_primary", "w_brand_secondary",
                "w_typography", "w_brand_voice",
                "w_cms_migration", "w_animation_scope",
                "w_motion_guidelines",
            ],
        },
        {
    ...
    "synth_medical": [
        {
            ...
            "category": "completeness",
            "anchors": [
                "m_metformin", "m_lisinopril",
                "m_atorvastatin", "m_vitamin_d",
            ],
        },
        {
        ...
        {
            "question": "How has the user's A1C and vitamin D level changed "
                        "over time?",
            "category": "aggregation",
            "anchors": [
                "m_a1c_improving", "m_vitamin_d_levels", "m_walking",
            ],
        },
        {
            ...
            "category": "cross_reference",
            "anchors": [
                "m_ibuprofen_warning", "m_new_med_tylenol",
            ],
        },
        {
            ...
            "category": "completeness",
            "anchors": [
                "m_blood_panel", "m_appt_jan25", "m_pt_referral",
            ],
        },
    ],
}
    ...
    for length in lengths:
        ...
        for conv_name, cfg in TOPICS.items():
            ...
            turns, anchor_ids = build_conversation(
                anchors, filler, length, rng,
            )
            actual = len(turns)
            print(f"  {conv_name}: anchor_count={len(anchors)}, "
...
def embed_all_data(lengths: list[int]) -> None:
    ...
    for length in lengths:
        ...
        for start in range(0, len(missing_texts), EMBED_BATCH):
            ...
            print(f"    Embedding batch {start // EMBED_BATCH + 1} "
...
class ScalingArch:
    ...
    def __init__(self, store: SegmentStore, client: OpenAI,
                 emb_cache: ScalingEmbeddingCache,
                 llm_cache: ScalingLLMCache) -> None:
        self.store = store
        self.client = client
...
def evaluate_question(arch: ScalingArch, q: dict) -> dict:
    ...
    cosine_result = arch.store.search(
        q_emb, top_k=50, conversation_id=conv_id
    )
...
def summarize(rows: list[dict], arch_name: str, length: int) -> dict:
    n = len(rows)
    summary: dict = {
        "arch": arch_name, "conversation_length": length, "n": n,
    }
    if n == 0:
        return summary
    for K in BUDGETS:
    ...
    summary["avg_embed_calls"] = round(
        sum(r["embed_calls"] for r in rows) / n, 1)
...
def run_eval(lengths: list[int], arch_names: list[str]) -> None:
    ...
    for length in lengths:
        ...
        print(f"\n{'=' * 70}")
        print(f"LENGTH={length}  segments={len(store.segments)}  "
              f"questions={len(qs)}")
        print(f"{'=' * 70}")
        ...
        for arch_name in arch_names:
            ...
            for i, q in enumerate(qs):
                ...
                print(f"  [{i + 1}/{len(qs)}] {q['category']}: {q_short}...",
                      flush=True)
                try:
    ...
    print("=" * 92)
    header = f"{'Arch':<22s}  " + "  ".join(
        f"{L:>6d}T" for L in lengths
    ) + "      " + "  ".join(
        f"{L:>6d}T" for L in lengths
    )
    print(f"{'Architecture':<22s}  " + "  ".join(
        f"{L:>7d}" for L in lengths
    ))
    print("delta r@20:")
    for arch_name in arch_names:
        row = [arch_name]
...
def main() -> None:
    ...
    p.add_argument("--embed", action="store_true",
                   help="Embed generated data to npz files.")
    p.add_argument("--run", action="store_true",
                   help="Run retrieval eval across archs.")
    p.add_argument("--all", action="store_true",
                   help="Generate + embed + run.")
