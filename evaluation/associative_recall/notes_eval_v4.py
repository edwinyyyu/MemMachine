"""Evaluate v4 model-note-augmented EventMemory on LoCoMo-30 + task-sufficiency.

v4 uses the neutral listener-observations prompt (RESOLVED/FACT/COUNT/UPDATE/
LINK/NAME + PHATIC skip). Collections live at prefix `arc_em_lc30_notes_v4_*`.

Four retrieval architectures (same shape as v3's notes_eval.py):
  1. em_cosine_notes_v4          raw-cosine, dual-event (msgs+notes) index
  2. em_v2f_notes_v4             v2f speakerformat cues, dual-event index
  3. em_v2f_notes_msgs_only_v4   messages-only control (event_type="message")
  4. em_v2f_notes_only_v4        notes-only ablation (event_type="model_note")

Task-sufficiency (A/B/C):
  A. single-shot v2f on STANDARD ingest (baseline, arc_em_lc30_v1_*)
  B. single-shot v2f on v4 notes ingest
  C. proactive decomposition on v4 notes ingest

Usage:
  uv run python evaluation/associative_recall/notes_eval_v4.py --phase retrieval
  uv run python evaluation/associative_recall/notes_eval_v4.py --phase sufficiency
  uv run python evaluation/associative_recall/notes_eval_v4.py   # both
"""
...
# v4-dedicated caches (never collide with v3).
NOTES_V2F_CUEGEN_CACHE = CACHE_DIR / "notes_v4_v2f_cuegen_cache.json"
NOTES_JUDGE_CACHE = CACHE_DIR / "notes_v4_judge_cache.json"
NOTES_SINGLESHOT_CUEGEN_CACHE = CACHE_DIR / "notes_v4_singleshot_cuegen_cache.json"

NOTES_COLLECTIONS_META = RESULTS_DIR / "eventmemory_notes_v4_collections.json"
STD_COLLECTIONS_META = RESULTS_DIR / "eventmemory_collections.json"


# ---------------------------------------------------------------------------
# Retrieval architectures (structure identical to v3 notes_eval)
# ---------------------------------------------------------------------------
...
async def evaluate_retrieval_question(
    ...
    *,
    ...
    t0 = time.monotonic()
    meta: dict = {}
    if arch == "em_cosine_notes_v4":
        hits = await em_cosine_notes_v4(memory, q_text, K=max_K)
    elif arch == "em_v2f_notes_v4":
        hits, meta = await em_v2f_notes_v4(
            memory, q_text, K=max_K, llm_cache=cache,
            openai_client=openai_client, participants=participants,
        )
    elif arch == "em_v2f_notes_msgs_only_v4":
        hits, meta = await em_v2f_notes_v4(
            memory, q_text, K=max_K, llm_cache=cache,
            openai_client=openai_client, participants=participants,
            property_filter=_event_type_filter("message"),
        )
    elif arch == "em_v2f_notes_only_v4":
        hits, meta = await em_v2f_notes_v4(
            memory, q_text, K=max_K, llm_cache=cache,
            openai_client=openai_client, participants=participants,
...
async def main() -> None:
    ...
    # Warm v2f cache by reading from v3/v2 speakerformat caches (question-only
    # primers are question-dependent, not collection-dependent, so primer cues
    # are shareable ONLY when the primer context matches byte-for-byte; here
    # they won't because notes-index primers look different, so most lookups
    # will miss — but reading from these is still safe).
    v2f_cache = _MergedLLMCache(
        reader_paths=[
    ...
    final = {
        "config": {
            ...
            "K": args.K,
            "prompt_version": notes_meta.get("prompt_version", "v4"),
        }
    }
    ...
    try:
        if args.phase in ("retrieval", "both"):
            ...
            archs = [
                "em_cosine_notes_v4",
                "em_v2f_notes_v4",
                "em_v2f_notes_msgs_only_v4",
                "em_v2f_notes_only_v4",
            ]
            ...
            v2f_cache.save()
            # Snapshot retrieval numbers alone for easy diffing.
            retrieval_out = RESULTS_DIR / "model_notes_v4_retrieval.json"
            with open(retrieval_out, "w") as f:
                json.dump(retrieval_results, f, indent=2)
            print(f"Saved: {retrieval_out}")
        ...
        if args.phase in ("sufficiency", "both"):
            ...
            sufficiency_out = RESULTS_DIR / "model_notes_v4_sufficiency.json"
            with open(sufficiency_out, "w") as f:
                json.dump(final["sufficiency"], f, indent=2)
            print(f"Saved: {sufficiency_out}")
            for c in caches.values():
                c.save()
    finally:
    ...
    out_json = RESULTS_DIR / "model_notes_v4.json"
    with open(out_json, "w") as f:
        json.dump(final, f, indent=2)
    print(f"Saved: {out_json}")

    md = build_report(final, notes_meta)
    out_md = RESULTS_DIR / "model_notes_v4.md"
    out_md.write_text(md)
    print(f"Saved: {out_md}")


# Known v3 reference numbers (for side-by-side) — retrieval was not run for v3,
# so we compare against the standard em_v2f_speakerformat baseline reported in
# model_notes.md (R@20=0.8167, R@50=0.8917).
V3_BASELINE_EM_V2F = {"r@20": 0.8167, "r@50": 0.8917}
V3_SUFFICIENCY = {
    "A_std_singleshot": {"mean_sufficiency": 4.85, "mean_coverage": 4.95, "mean_depth": 4.55, "mean_noise": 7.0, "mean_n_notes": 0.0},
    "B_notes_singleshot": {"mean_sufficiency": 5.1, "mean_coverage": 5.6, "mean_depth": 4.45, "mean_noise": 7.85, "mean_n_notes": 48.75},
    "C_notes_proactive": {"mean_sufficiency": 5.45, "mean_coverage": 5.85, "mean_depth": 4.75, "mean_noise": 7.5, "mean_n_notes": 42.95},
}


def build_report(final: dict, notes_meta: dict) -> str:
    lines: list[str] = []
    lines.append("# Model-note augmented EventMemory (v4)")
    lines.append("")
    lines.append("## Architecture (v4)")
    lines.append("")
    lines.append(
        "- Same EM ingestion + V_combined retrieval pipeline as v3. "
        "Only difference: the note-generation prompt and its output shape."
    )
    lines.append(
        "- v4 prompt = \"neutral A''\" listener-observations prompt "
        "(RESOLVED/FACT/COUNT/UPDATE/LINK/NAME labels + PHATIC skip rule), "
        "with domain-neutral inline examples. Source: "
        "`notes_prompt_neutral.py` / `em_setup_notes_v4.py`."
    )
    lines.append(
    ...
    # Ingest stats.
    ingest_meta = notes_meta.get("conversations", [])
    total_turns = sum(c.get("n_turns_ingested", 0) for c in ingest_meta)
    total_notes = sum(c.get("n_notes_ingested", 0) for c in ingest_meta)
    total_phatic = sum(c.get("n_phatic", 0) for c in ingest_meta)
    denom = total_turns if total_turns > 0 else 1
    label_totals: dict[str, int] = {}
    for c in ingest_meta:
        for lbl, cnt in (c.get("label_totals") or {}).items():
            label_totals[lbl] = label_totals.get(lbl, 0) + cnt
    total_labels = sum(label_totals.values())
    lines.append("## Ingest stats (v4)")
    lines.append("")
    lines.append(f"- Turns ingested: {total_turns}")
    lines.append(f"- Notes ingested (non-PHATIC): {total_notes}")
    lines.append(f"- PHATIC turns (note skipped): {total_phatic}  ({total_phatic/denom:.1%} of turns)")
    lines.append(f"- Mean labels per non-PHATIC note: {total_labels/max(total_notes,1):.2f}")
    if label_totals:
        lines.append("")
        lines.append("| Label | Count | % of labels |")
        lines.append("| --- | --- | --- |")
        for lbl in ("RESOLVED", "FACT", "COUNT", "UPDATE", "LINK", "NAME"):
            c = label_totals.get(lbl, 0)
            pct = c / max(total_labels, 1)
            lines.append(f"| {lbl} | {c} | {pct:.1%} |")
    lines.append("")
    ...
    if "retrieval" in final:
        ...
        lines.append("")
        lines.append(
            "v3 did not run the retrieval phase. Baseline reference is the "
            "standard `em_v2f_speakerformat` (no notes) reported in `model_notes.md`: "
            f"R@20={V3_BASELINE_EM_V2F['r@20']}, R@50={V3_BASELINE_EM_V2F['r@50']}."
        )
        lines.append("")
        lines.append("| Architecture | R@20 | delta vs em_v2f | R@50 | delta vs em_v2f | time (s) |")
        lines.append("| --- | --- | --- | --- | --- | --- |")
        for arch, data in final["retrieval"]["archs"].items():
            s = data["summary"]
            d20 = s["mean_r@20"] - V3_BASELINE_EM_V2F["r@20"]
            d50 = s["mean_r@50"] - V3_BASELINE_EM_V2F["r@50"]
            lines.append(
                f"| `{arch}` | {s['mean_r@20']:.4f} | {d20:+.4f} | "
                f"{s['mean_r@50']:.4f} | {d50:+.4f} | {s['time_s']:.1f} |"
            )
        lines.append("")
    ...
    if "sufficiency" in final:
        ...
        lines.append("- B = single-shot v2f over v4 NOTES ingest (messages + notes).")
        lines.append("- C = proactive decomposition over v4 NOTES ingest.")
        lines.append("")
        ...
        lines.append(f"**v4 Delta vs baseline A**: B-A = {dBA:+.3f}, C-A = {dCA:+.3f}.")
        lines.append("")

        lines.append("### v3 comparison (from `model_notes.md`)")
        lines.append("")
        lines.append("| System | v3 Suff | v4 Suff | v4 - v3 |")
        lines.append("| --- | --- | --- | --- |")
        for key in ("A_std_singleshot", "B_notes_singleshot", "C_notes_proactive"):
            v3_s = V3_SUFFICIENCY[key]["mean_sufficiency"]
            v4_s = s[key]["mean_sufficiency"]
            lines.append(f"| {key} | {v3_s} | {v4_s} | {v4_s - v3_s:+.3f} |")
        lines.append("")

        lines.append("### Per-task sufficiency (v4)")
        lines.append("")
    ...
    if "retrieval" in final:
        em_v2f = final["retrieval"]["archs"].get("em_v2f_notes_v4", {}).get("summary", {})
        if em_v2f:
            d50 = em_v2f["mean_r@50"] - V3_BASELINE_EM_V2F["r@50"]
            verdict_lines.append(
                f"- Retrieval: `em_v2f_notes_v4` R@50 = {em_v2f['mean_r@50']:.4f} "
                f"vs em_v2f baseline {V3_BASELINE_EM_V2F['r@50']} "
                f"(delta {d50:+.4f})."
            )
    if "sufficiency" in final:
        s = final["sufficiency"]
        ...
        dCA = s["C_notes_proactive"]["mean_sufficiency"] - s["A_std_singleshot"]["mean_sufficiency"]
        v3_best = max(
            V3_SUFFICIENCY["B_notes_singleshot"]["mean_sufficiency"] - V3_SUFFICIENCY["A_std_singleshot"]["mean_sufficiency"],
            V3_SUFFICIENCY["C_notes_proactive"]["mean_sufficiency"] - V3_SUFFICIENCY["A_std_singleshot"]["mean_sufficiency"],
        )
        v4_best = max(dBA, dCA)
        verdict_lines.append(
            f"- Task-sufficiency: v4 best delta vs baseline A = {v4_best:+.3f} "
            f"(v3 best was {v3_best:+.3f}). B-A={dBA:+.3f}, C-A={dCA:+.3f}."
        )

        if "retrieval" in final:
            em_v2f_r50 = final["retrieval"]["archs"].get("em_v2f_notes_v4", {}).get("summary", {}).get("mean_r@50", 0.0)
            retrieval_lift = em_v2f_r50 > V3_BASELINE_EM_V2F["r@50"]
        else:
            retrieval_lift = False
        suff_lift = v4_best > v3_best + 0.1  # at least 0.1pt better than v3's best
        if retrieval_lift and suff_lift:
            verdict = "SHIP v4"
        elif suff_lift and not retrieval_lift:
            verdict = "SHIP v4 (sufficiency-only; retrieval neutral)"
        elif retrieval_lift and not suff_lift:
            verdict = "UNCLEAR (retrieval gain, sufficiency flat)"
        elif v4_best >= v3_best - 0.1:
            verdict = "KEEP v3 (v4 is a tie, prefer older+proven)"
        else:
            verdict = "KEEP v3 (v4 regressed on sufficiency)"
        verdict_lines.append(f"- **Verdict: {verdict}**")
    lines.extend(verdict_lines)
    lines.append("")
    lines.append("## Outputs")
    lines.append("")
    lines.append("- `results/model_notes_v4.json`")
    lines.append("- `results/model_notes_v4_retrieval.json`")
    lines.append("- `results/model_notes_v4_sufficiency.json`")
    lines.append("- `results/model_notes_v4.md`")
    lines.append("- Notes collections manifest: `results/eventmemory_notes_v4_collections.json`")
    lines.append("- Source: `em_setup_notes_v4.py`, `notes_eval_v4.py`")
    return "\n".join(lines)
