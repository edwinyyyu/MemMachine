...
def analyze_baseline_gaps(results_file: str) -> None:
    ...
    print(
        f"  Partial (0 < r@20 < 1): {len(partial)} ({len(partial) / len(results):.0%})"
    )
    ...
    print(
        f"  {'category':35s} {'n':>3s} {'mean':>6s} {'perf':>5s} {'zero':>5s} {'#src':>5s}"
    )
    print("  " + "-" * 65)
    for cat in sorted(by_cat):
        ...
        print(
            f"  {cat:35s} {len(rows):>3d} {mean_b20:>6.3f} {n_perfect:>5d} {n_zero:>5d} {avg_src:>5.1f}"
        )
    ...
    if zero:
        ...
        for r in zero:
            ...
            print(
                f"    [{r['category']:30s}] assoc r@20={a20:.3f} src={r['source_chat_ids']}"
            )
    ...
    if partial:
        ...
        for r in sorted(partial, key=lambda x: x["baseline_recalls"]["r@20"]):
            ...
            a20 = r["assoc_recalls"]["r@20"]
            print(
                f"    [{r['category']:30s}] b={b20:.3f} a={a20:.3f} d={a20 - b20:+.3f} src={r['source_chat_ids']}"
            )
            print(f"      {r['question'][:80]}")
