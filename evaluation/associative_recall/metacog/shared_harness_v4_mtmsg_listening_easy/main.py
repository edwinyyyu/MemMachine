"""Shared harness v4_mtmsg_listening_easy — listening-mode SYSTEM_PROMPT variant.

Architecture: copied from shared_harness_v4_mtmsg_subdec_cogonly_easy. The
substantive change is a SYSTEM_PROMPT swap to the streaming-aware "listening
mode" framing (adapted from shared_harness_v4_mtmsg_streaming):

  - The agent is told it is in a working loop, listens for implicit
    constraint-facts the user has shared previously, and should NOT enumerate
    all sub-decisions up front. Sub-decisions arise as the work unfolds; emit
    STEP_OUTPUT for each as you decide it.
  - References to "tagged INCOMING USER STREAM messages" and the stream-
    closed signal are REMOVED — single-message tasks have no stream.
  - The "don't DONE prematurely" framing is preserved as general discipline.

USER_INITIAL becomes a listening-mode first-message handler: the user's task
description is the first message; the model should listen for constraint-
facts and emit STEP_OUTPUTs as decisions arise rather than enumerate them
all at turn 1.

USER_FOLLOWUP is plain (no per-turn implicit-constraint nudge — that's the
cogonly variant). Pure SYSTEM_PROMPT swap test.

HYPOTHESIS: does deferred-decomposition listening mode match or beat
subdec_split's turn-1 enumeration directive on EASY 10 (single-message
scenarios)? If it matches/beats, listening mode is a unified turn-1 strategy
that ALSO handles streams — clean architectural simplification.

Usage (smoke):
    SH_SCENARIO_INDICES=1,5,6 SH_RESULTS_SUBDIR=results_smoke_s0 \\
        SH_DB_SUFFIX=_smoke_s0 uv run python \\
        evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_listening_easy/main.py
"""
...
# ---------- System prompt — LISTENING-MODE adaptation ----------
#
...
#   - References to tagged INCOMING USER STREAM messages and the stream-
#     closed signal are REMOVED — this is a single-message task variant.
...
This is turn 1. You have no probe results yet. In your THINKING, briefly \
note what kind of work this appears to be and what categories of \
constraint-facts you might want to listen for as you read past chat \
history (e.g., guest allergies for a banquet; brand guidelines for a \
presentation; travel preferences for a trip). Don't enumerate all \
sub-decisions yet — sub-decisions will reveal themselves as you surface \
relevant facts and see what concrete choices the deliverable actually \
requires. Issue PROBES targeting the implicit constraint-facts the task \
description doesn't mention but that materially shape the answer. Emit \
STEP_OUTPUTs as sub-decisions arise during the run — not all at once at \
the end.
"""
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR
        / f"eventmemory_shared_harness_v4_mtmsg_listening{_DB_SUFFIX}.sqlite3"
    )
    ...
    print(f"\n=== Cross-scenario means (v4_mtmsg_listening_easy, n={len(results)}) ===")
