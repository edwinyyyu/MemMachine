"""Shared harness v4_keys — keys-in-context with fetch-by-id.

ONE change vs v3 (shared_harness_v3/main.py):
  - Items in WM appear as KEYS + short previews (~80 chars), not full content.
  - New action `fetch` pulls the full verbatim content for a specific id back
    into WM. Fetched items become full-content items in WM and follow normal
    LRU rules (citation refreshes them).

The full content lives in external memory (the EM contains all chat events).
Items the agent has not fetched are visible only as keys+previews. WM thus
holds many more items in key form (visible breadth) but only fetches FULL
CONTENT for items the model actively engages with (selective depth).

Priming addition:
  Items appear as keys + short previews. To engage with an item's full
  content, emit {"type": "fetch", "id": "..."} and the next turn's WM will
  include the full content. Use fetch when a preview suggests an item is
  load-bearing for your current sub-decision but you need the full text to
  confirm or to extract specifics.

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_keys/main.py
"""
...
# Length of preview shown for unfetched chat items (in characters).
PREVIEW_CHARS = 80
...
def make_preview(text: str, n_chars: int = PREVIEW_CHARS) -> str:
    t = (text or "").replace("\n", " ").strip()
    if len(t) <= n_chars:
        return t
    return t[: n_chars - 1].rstrip() + "..."
...
class Item:
    ...
    # NEW: chat items default to "keys-only" until fetched. Non-chat items
    # (reason, ret, assess, step) are always full-content.
    fetched: bool = True
    ...
    def render_key(self) -> str:
        """Keys-format render: id + short preview only."""
        prev = make_preview(self.content)
        nchars = len(self.content)
        return (
            f"[{self.item_id}] ({self.kind}, last_ref=t{self.last_referenced_turn}, "
            f'key-only, full={nchars}c): "{prev}"'
        )

    def render_for_tier(self, *, tier: int) -> str:
        """Render an item according to tier and fetched-state.

        Chat items that have not been fetched render as keys (any tier).
        Fetched chat items and non-chat items render verbatim in tier1/tier2
        and summary in tier3.
        """
        if self.kind == "chat" and not self.fetched:
    ...
    def size_tokens(self) -> int:
        if self.kind == "chat" and not self.fetched:
            return n_tokens(self.render_key())
        if self.summary:
...
# v4_keys-specific addition explaining keys-format and fetch action.
KEYS_AND_FETCH_BLOCK = """\
--- KEYS + FETCH (v4_keys) ---

Chat items in your working memory appear as KEYS + SHORT PREVIEWS, not full content. A line like:
  [chat-12] (chat, last_ref=t2, key-only, full=420c): "Sam mentioned anaphylaxis to peanuts, severity 5/5 — even trace..."
shows the id, the preview (first ~80 chars), and the full character-length so you know what's hidden.

To engage with an item's full content, emit a `fetch` action:
  {"type": "fetch", "id": "chat-12", "reason": "<why I want full content>"}
The next turn's WM will include the full verbatim content for that item. Fetched items become full-content items and follow normal LRU rules (citation refreshes them).

When to fetch:
- A preview suggests the item is load-bearing for your current sub-decision but you need the full text to confirm or to extract specifics (numbers, names, dates, exact constraints).
- Don't fetch every item you see — that defeats the purpose. Fetch selectively, based on what the preview suggests vs what your current sub-decision needs.
- DO cite fetched items inline once you use them — citation refreshes their LRU position so they don't get evicted while you're still using them.

Note: `retrieve` brings NEW items into TIER 1 in keys-format; `fetch` expands an EXISTING key into full content. They are different actions."""
...

{
  ...
  "actions": [
    ...
    {"type": "fetch",    "id": "<chat-id>", "reason": "<why I want the full content>"},
...
- TIER 1 (transient retrieval buffer): the most recent retrieval's hits, each shown with its chat-N id. Replaced each retrieval round.
- TIER 2 (active LRU, cap ~15): items you've recently CITED. The system tracks what you cite to keep relevant items active.
- TIER 3 (compressed older items): one-line summaries of items you stopped citing. IDs are preserved so you can re-promote them by citing.

Chat items appear as KEYS + SHORT PREVIEWS (see "KEYS + FETCH" below) until you fetch them.
...
- When you cite, the system refreshes the item's LRU position. Items you stop citing fall out as new items get cited.
- INLINE-QUOTE load-bearing content next to the citation marker. The [id] is for traceability; the inline quoted content is for clarity AND for robustness against compression. Example: "Maya has a strict shellfish allergy [chat-43]" — the words before [chat-43] make the reasoning survive if [chat-43] later compresses.
...
Your working memory shows newest items at the top. Chat items appear as KEYS + SHORT PREVIEWS (see "KEYS + FETCH" below) until you fetch them. When WM exceeds the token budget, oldest items get FIFO-evicted. Cite items like [chat-3] when you rely on them so your traceability survives.
...
def render_wm_operator(state: OperatorState) -> str:
    ...
    if state.tier1:
        for it in state.tier1:
            parts.append(it.render_for_tier(tier=1))
    else:
        parts.append("(empty)")
    parts.append("\n--- TIER 2 (active LRU) ---")
    if state.tier2:
        for it in state.tier2:
            parts.append(it.render_for_tier(tier=2))
    else:
        parts.append("(empty)")
    parts.append("\n--- TIER 3 (compressed older, ids preserved) ---")
    if state.tier3:
        for it in state.tier3:
            parts.append(it.render_for_tier(tier=3))
    else:
        parts.append("(empty)")
    return "\n".join(parts)
...
def render_wm_baseline(state: BaselineState) -> str:
    ...
    if not state.wm:
        ...
        for it in state.wm:
            # Baseline has no tiering; treat as tier 2-equivalent.
            parts.append(it.render_for_tier(tier=2))
    return "\n".join(parts)
...
def render_behavioral_history(history: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "--- BEHAVIORAL HISTORY ---",
        f"{'turn':>4}  {'retrieves':>9}  {'fetches':>7}  {'assesses':>8}  {'step_outs':>9}  cue_excerpts",
    ]
    for h in history:
        cue_str = " / ".join(f'"{c}"' for c in h["cue_excerpts"]) or "(none)"
        lines.append(
            f"{h['turn']:>4}  {h['retrieves']:>9}  {h.get('fetches', 0):>7}  "
            f"{h['assesses']:>8}  {h['step_outs']:>9}  {cue_str}"
        )
    return "\n".join(lines)
...
class OperatorState:
    ...
    def replace_tier1(self, items: list[Item]) -> None:
        # Items that are still tracked elsewhere (e.g. were in tier2 before this
        # retrieve) shouldn't be removed from all_items; only drop tier1
        # exclusives.
        kept_ids = {i.item_id for i in self.tier2 + self.tier3}
        for old in self.tier1:
            if old.item_id not in kept_ids:
    ...
    def fetch(self, item_id: str, turn: int) -> str:
        """Mark a chat item as fetched (full-content) and refresh LRU."""
        if item_id not in self.all_items:
            return "unknown"
        it = self.all_items[item_id]
        if it.kind != "chat":
            return "not_chat"
        was_fetched = it.fetched
        it.fetched = True
        it.last_referenced_turn = turn
        # Promote to tier2 if currently in tier1 or tier3.
        if it in self.tier1:
            self.tier1.remove(it)
            self.tier2.insert(0, it)
            return "tier1->tier2(fetched)"
        if it in self.tier2:
            self.tier2.remove(it)
            self.tier2.insert(0, it)
            return (
                "tier2->tier2(fetched)" if not was_fetched else "tier2->tier2(refresh)"
            )
        if it in self.tier3:
            it.summary = None
            self.tier3.remove(it)
            self.tier2.insert(0, it)
            return "tier3->tier2(fetched)"
        return "untracked"
    ...
    def verbatim_tokens(self) -> int:
        # Mirror render: tier1/tier2 may be keys for unfetched chats.
        s_parts = []
        for it in self.tier1:
            s_parts.append(it.render_for_tier(tier=1))
        for it in self.tier2:
            s_parts.append(it.render_for_tier(tier=2))
        for it in self.tier3:
            s_parts.append(it.render_for_tier(tier=3))
        return n_tokens("\n".join(s_parts))
...
class BaselineState:
    ...
    def fetch(self, item_id: str, turn: int) -> str:
        """Mark a chat item as fetched and bring it to the top of WM."""
        if item_id not in self.all_items:
            return "unknown"
        it = self.all_items[item_id]
        if it.kind != "chat":
        ...
        if it in self.wm:
            ...
            self.wm.insert(0, it)
            return "fetched"
        # Item is tracked but not in the wm list; re-add it.
        self.wm.insert(0, it)
        return "fetched(re-added)"

    def tokens(self) -> int:
        s = "\n".join(i.render_for_tier(tier=2) for i in self.wm)
        return n_tokens(s)
...
async def run_agent_loop(
    *,
    ...
    # Track every fetch action for analysis.
    ...
    chat0 = Item(
        ...
        fetched=True,  # task is always full-content
    )
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        for a in actions:
            if not isinstance(a, dict):
                continue
            for fld in ("re", "why", "content", "label", "cue", "reason", "id"):
                v = a.get(fld)
                if isinstance(v, str):
            ...
            if atype == "retrieve":
                ...
                for idx, h in enumerate(hits):
                    ...
                    if isinstance(state, OperatorState) and chat_id in state.all_items:
                        ...
                        # Make sure full content is up-to-date even for known items.
                    ...
                    if (
                        ...
                        hit_items.append(existing)
                        continue
                    # NEW chat item: defaults to keys-only (fetched=False).
                    item = Item(
                        item_id=chat_id,
                        kind="chat",
                        content=content,
                ...
                if isinstance(state, OperatorState):
                    ...
                    for it in hit_items:
                        # Push only if not already in WM.
                        if it not in state.wm:
                            state.push(it)
                        else:
                            state.all_items[it.item_id] = it
                    state.push(ret_item)
                ...
                fetch_id = (a.get("id") or "").strip()
                reason = a.get("reason") or ""
                if not fetch_id:
                    log.actions_summary.append({"type": "fetch", "skipped": "empty_id"})
                    continue
                n_fetches += 1
                if isinstance(state, OperatorState):
                    res = state.fetch(fetch_id, turn)
                else:
                    res = state.fetch(fetch_id, turn)
                # Determine if this id is a "gold" item (planted).
                turn_fetched_ids.append(fetch_id)
                fetch_log.append(
                    {
                        "turn": turn,
                        "id": fetch_id,
...
async def run_one_scenario(
    *,
    ...
    async def _run_one(variant: str) -> tuple[str, dict[str, Any]]:
        ...
        n_fetches = sum(t.get("n_fetches", 0) for t in agent_result["trace"])
        print(
...
async def main() -> None:
    # v4_keys runs only 2 scenarios for quick exploration.
    # Indices into the global scenarios list:
    #   10 = multi-hop-banquet-01
    #   17 = world-knowledge-bridge-01
    SELECTED_INDICES = [10, 17]
    K_list = SCORE_K_LIST
    variants = ["baseline_fifo", "operator_lru"]

    scenarios_all = load_scenarios()
    scenarios = [scenarios_all[i] for i in SELECTED_INDICES]
    print(
        f"[main] running {len(scenarios)} scenarios: "
    ...
    sqlite_path = RESULTS_DIR / "eventmemory_shared_harness_v4_keys.sqlite3"
    ...
    for r in results:
        ...
        for variant in variants:
            ...
            n_fetches_total = sum(t.get("n_fetches", 0) for t in agent_result["trace"])
            row[variant] = {
                ...
                "n_fetches_total": n_fetches_total,
            }
    ...
    for variant in variants:
        ...
        for r in results:
            ...
            n_fetches_grand += sum(t.get("n_fetches", 0) for t in agent_result["trace"])
        summary["per_variant_means"][variant] = {
            ...
            "n_fetches_total": n_fetches_grand,
            ...
            "fetches_per_turn": round(n_fetches_grand / n_turns_total, 4)
            if n_turns_total
            else None,
        }
    ...
    for variant in variants:
        ...
        print(
            ...
            f"fetches/turn={m['fetches_per_turn']} | "
            f"max_wm_mean={m['max_wm_tokens_mean']} | parse_fail={m['parse_failures_total']}"
        )
    print("\n=== Reference (v3 on the same 2 scenarios) ===")
    print("  multi-hop-banquet-01 baseline_fifo:  cov=1.0   | full_R@5=0.5556")
    print("  multi-hop-banquet-01 operator_lru:   cov=0.667 | full_R@5=0.1667")
    print("  world-knowledge-bridge-01 baseline:  cov=0.222 | full_R@5=0.0444")
    print("  world-knowledge-bridge-01 operator:  cov=0.556 | full_R@5=0.0444")
    print(f"\nWrote {summary_path}")
