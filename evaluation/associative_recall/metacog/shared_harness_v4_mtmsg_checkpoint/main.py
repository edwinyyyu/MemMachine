"""Shared harness v4_mtmsg_checkpoint — compaction + BATCH CHECKPOINT REVIEW.

Forked from shared_harness_v4_mtmsg_revisit.

Hypothesis: commitment abandonment is mitigated more strongly by a BATCH
review than by single-oldest-commitment revisits. Every K stream messages
delivered, the harness pauses the stream and asks the agent to triage ALL
currently-open commitments in one structured pass — classify each as
READY_TO_DELIVER / STILL_WAITING / ABANDON, and act on each in the same turn.

This is a different scheduling primitive from the revisit variant:
  - Revisit: re-surface the SINGLE oldest commitment, frequently.
  - Checkpoint: pause every K stream messages, review ALL open commitments
    as a batch (like a planning meeting), and act on each.
...
Mechanism summary:
  ...
  - Checkpoint scheduler fires every SH_CHECKPOINT_PERIOD stream messages
    delivered (default 10). When triggered:
      (1) the harness builds a list of ALL currently-open commitments with
          ages and summaries
      (2) injects a CHECKPOINT REVIEW addendum at the end of the next user
          followup, instructing the agent to emit one
          ``CHECKPOINT_DECISION: <label>: READY_TO_DELIVER | STILL_WAITING | ABANDON``
          line per commitment and to emit a STEP_OUTPUT for any
          READY_TO_DELIVER labels in the same turn.
  - Parsing for checkpoint response:
      * READY_TO_DELIVER → expect a STEP_OUTPUT with a label matching the
        commitment; on match the harness closes the commitment.
      * STILL_WAITING → keep open, no harness-side action.
      * ABANDON → harness closes the commitment via the open-sub-decisions
        tracker.

Set SH_DISABLE_CHECKPOINT=1 to ablate the scheduler.
...
Usage:
    ...
    # Recommended smoke config (compaction on, motivation off, checkpoint on)
    SH_SCENARIO_FILE=evaluation/associative_recall/data/perpetual_scenarios.json \\
        SH_RESULTS_SUBDIR=results_smoke_s0 SH_DB_SUFFIX=_smoke_s0 \\
        SH_DISABLE_MOTIVATION=1 \\
        uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_checkpoint/main.py \\
            --max-turns 120
...
NAMESPACE = "arc_em_checkpoint"
COLLECTION_PREFIX = "arc_cp"
...
# This harness's primary mechanism is the batch checkpoint review scheduler.
...
# Fire a checkpoint review every N stream messages delivered. A "stream
# message delivered" is when the harness actually injects the next
# scenario message into a followup (turn 1 counts as the first delivery).
# Default 10: roughly aligns with a major "planning meeting" cadence on
# a 30-message stream.
SH_CHECKPOINT_PERIOD = int(os.environ.get("SH_CHECKPOINT_PERIOD", "10"))

# When 1/true, the checkpoint scheduler is disabled (ablation).
SH_DISABLE_CHECKPOINT = os.environ.get("SH_DISABLE_CHECKPOINT", "0").lower() in (
    "1",
    "true",
    "yes",
)
...
DIRECTIVE_LINE_RE = re.compile(
    r"^\s*(THINKING|PROBE|STEP_OUTPUT|DONE|CHECKPOINT_DECISION)\b",
    re.MULTILINE | re.IGNORECASE,
)
...
# Checkpoint decision marker — agent's classification verdict for a
# specific open commitment during a CHECKPOINT REVIEW turn. Format:
#   CHECKPOINT_DECISION: <label>: READY_TO_DELIVER | STILL_WAITING | ABANDON
# Be permissive about whitespace and case in the verdict.
CHECKPOINT_DECISION_LINE_RE = re.compile(
    r"^\s*CHECKPOINT_DECISION\s*:\s*(.+?)\s*:\s*"
    r"(READY_TO_DELIVER|STILL_WAITING|ABANDON)\s*$",
    re.MULTILINE | re.IGNORECASE,
)
...
def parse_checkpoint_decisions(raw: str) -> list[dict[str, str]]:
    """Return a list of `{label, verdict}` extracted from CHECKPOINT_DECISION
    lines. Verdict is upper-cased and one of READY_TO_DELIVER | STILL_WAITING
    | ABANDON. Empty-label rows are dropped.
    """
    out: list[dict[str, str]] = []
    for m in CHECKPOINT_DECISION_LINE_RE.finditer(raw or ""):
        label = (m.group(1) or "").strip()
        verdict = (m.group(2) or "").strip().upper()
        if not label:
            continue
        out.append({"label": label, "verdict": verdict})
    return out
...
# ---------- System prompt — extended from streaming v2 with checkpoint-review
# classification markers documentation. ----------
...
CHECKPOINT REVIEW: the harness may periodically inject a `[CHECKPOINT \
REVIEW (turn N): Below are all currently-open commitments...]` block at \
the end of a followup. This is a batch triage moment — like a planning \
meeting where you decide what's actionable, what's still blocked, and \
what should be dropped. For EACH listed commitment, you MUST in this \
turn's response include exactly one line of the form:

  CHECKPOINT_DECISION: <label>: READY_TO_DELIVER

  CHECKPOINT_DECISION: <label>: STILL_WAITING

  CHECKPOINT_DECISION: <label>: ABANDON

Then:
  - For each READY_TO_DELIVER commitment, ALSO emit a `STEP_OUTPUT: \
<label>: ...` for that same label in the SAME turn.
  - For STILL_WAITING, briefly state in THINKING what info is needed.
  - For ABANDON, the harness will close the commitment in its tracker; \
no STEP_OUTPUT needed.

Don't skip any listed commitment. Don't invent labels that weren't \
listed. The labels are exactly the strings the harness prints.
...
message OR a checkpoint review. Emit STEP_OUTPUT only when the user has \
actually asked for something concrete — not for context they've just shared.

CHECKPOINT_DECISION: <label>: READY_TO_DELIVER | STILL_WAITING | ABANDON
  Purpose: in response to a CHECKPOINT REVIEW, classify the listed \
commitment. If READY_TO_DELIVER, emit a matching STEP_OUTPUT this turn. \
If ABANDON, the harness will close it.
...
async def open_empty_memory(
    ...
    *,
    ...
    info = {
        ...
        "n_messages": 0,
        "ingest_mode": "online_with_checkpoint",
    }
    return memory, info
...
class TurnLog:
    ...
    checkpoint_injected_after_this_turn: bool = False
    checkpoint_n_commitments_listed: int = 0
    checkpoint_response_this_turn: bool = False
    checkpoint_decisions_parsed: list[dict[str, str]] = field(default_factory=list)
    checkpoint_n_ready: int = 0
    checkpoint_n_waiting: int = 0
    checkpoint_n_abandon: int = 0
    checkpoint_n_unrecognized_label: int = 0
    checkpoint_n_ready_with_step_output: int = 0
...
def td_to_dict(t: TurnLog) -> dict[str, Any]:
    return {
        ...
        "checkpoint_injected_after_this_turn": t.checkpoint_injected_after_this_turn,
        "checkpoint_n_commitments_listed": t.checkpoint_n_commitments_listed,
        "checkpoint_response_this_turn": t.checkpoint_response_this_turn,
        "checkpoint_decisions_parsed": t.checkpoint_decisions_parsed,
        "checkpoint_n_ready": t.checkpoint_n_ready,
        "checkpoint_n_waiting": t.checkpoint_n_waiting,
        "checkpoint_n_abandon": t.checkpoint_n_abandon,
        "checkpoint_n_unrecognized_label": t.checkpoint_n_unrecognized_label,
        "checkpoint_n_ready_with_step_output": t.checkpoint_n_ready_with_step_output,
    }
...
def _collect_open_commitments(
    open_sub_decisions: dict[str, dict[str, Any]],
    *,
    at_turn: int,
) -> list[tuple[str, int, int, dict[str, Any]]]:
    """Return list of (label, opened_at_turn, age_in_turns, info) for ALL
    currently-open commitments. Sorted by opened-at ascending (oldest
    first) so that the rendered list reads chronologically.
    """
    ...
    for label, info in open_sub_decisions.items():
        ...
        rows.append((label, opened_at, age, info))
    rows.sort(key=lambda r: (r[1], r[0]))
    return rows


def _format_checkpoint_addendum(
    rows: list[tuple[str, int, int, dict[str, Any]]],
    *,
    at_turn: int,
) -> str:
    """Build the CHECKPOINT REVIEW string injected into the next followup.

    Principle-level — references the abstract commitment labels only,
    no scenario specifics. The instruction asks the agent for a per-label
    classification and (for READY_TO_DELIVER) a same-turn STEP_OUTPUT.
    """
    bullets: list[str] = []
    for i, (label, opened_at, age, info) in enumerate(rows, start=1):
        summary = (info.get("summary") or "").strip()
        if summary:
            bullets.append(
                f"{i}. {label} (opened turn {opened_at}, age {age} turns): "
                f"{summary}"
            )
        else:
            bullets.append(
    ...
    bullets_block = "\n".join(bullets) if bullets else "(no open commitments)"
    return (
        f"[CHECKPOINT REVIEW (turn {at_turn}): Below are all currently-open "
        f"commitments. For each, classify and act:\n"
        f"{bullets_block}\n\n"
        f"For each commitment listed above, in this turn's response you "
        f"MUST include exactly one line of the form:\n"
        f"  CHECKPOINT_DECISION: <label>: READY_TO_DELIVER | STILL_WAITING | ABANDON\n\n"
        f"If READY_TO_DELIVER, also emit a `STEP_OUTPUT: <label>: ...` "
        f"addressing it this same turn. If ABANDON, the harness will close "
        f"the commitment in its tracker. If STILL_WAITING, briefly state in "
        f"THINKING what info is needed before it can be delivered.]"
    )
...
async def run_agent_loop(
    *,
    ...
    # Fire a checkpoint every SH_CHECKPOINT_PERIOD stream messages delivered.
    # ``stream_msgs_delivered_count`` is the total number of distinct stream
    # messages delivered to the agent so far (including turn 1's initial msg).
    # We trigger when this count crosses a multiple of SH_CHECKPOINT_PERIOD
    # AND has not already triggered for that multiple.
    checkpoint_events: list[dict[str, Any]] = []
    # When a checkpoint is injected at end of turn T, the NEXT agent turn
    # (T+1) is the one whose response should be classified. We carry the
    # event dict forward so we can update response fields in-place after the
    # next turn's parse.
    pending_checkpoint_event: dict[str, Any] | None = None
    pending_checkpoint_labels: list[str] = []
    last_checkpoint_multiple_fired: int = 0  # which N*K boundary already fired
    stream_msgs_delivered_count: int = 1  # turn 1's initial message
    n_commitments_closed_via_checkpoint = 0
    n_commitments_abandoned_via_checkpoint = 0
    ...
    for turn in range(1, max_turns + 1):
        ...
        # --- Parse CHECKPOINT_DECISION lines ---
        checkpoint_decisions = parse_checkpoint_decisions(raw)

        # --- Classify response to a pending checkpoint (if injected last turn) ---
        if pending_checkpoint_event is not None:
            log.checkpoint_response_this_turn = True
            # Match parsed decisions against the listed labels. Be tolerant
            # of paraphrase: exact match, then case-insensitive, then
            # substring (either direction).
            listed_lkeys = [(lbl, lbl.lower().strip()) for lbl in pending_checkpoint_labels]

            def _match_listed(label: str) -> str | None:
                lk = label.lower().strip()
                if not lk:
                    return None
                # exact ci
                for orig, lkey in listed_lkeys:
                    if lk == lkey:
                        return orig
                # substring either direction
                for orig, lkey in listed_lkeys:
                    if lk and (lk in lkey or lkey in lk):
                        return orig
                return None
            ...
            abandon_labels: list[str] = []
            waiting_labels: list[str] = []
            classified_originals: set[str] = set()

            for dec in checkpoint_decisions:
                label = dec["label"]
                verdict = dec["verdict"]
                matched = _match_listed(label)
                if matched is None:
                    n_unrecognized += 1
                    continue
                classified_originals.add(matched)
                if verdict == "READY_TO_DELIVER":
                    n_ready += 1
                    ready_labels.append(matched)
                elif verdict == "STILL_WAITING":
                    n_waiting += 1
                    waiting_labels.append(matched)
                elif verdict == "ABANDON":
                    n_abandon += 1
                    abandon_labels.append(matched)
                    if _close_commitment(
                        ...
                        n_commitments_abandoned_via_checkpoint += 1

            # For READY_TO_DELIVER labels, look at this turn's STEP_OUTPUTs.
            # If any matches the ready label (exact / substring either way),
            # close the commitment in the tracker.
            so_labels_lower = [
                (so.get("raw_label", "") or "").lower().strip() for so in step_outs
            ]
            so_contents_lower = [
                (so.get("content", "") or "").lower() for so in step_outs
            ]
            n_ready_with_step_output = 0
            for rl in ready_labels:
                rlk = rl.lower().strip()
                matched_so = False
                for so_lab, so_content in zip(so_labels_lower, so_contents_lower):
                    if so_lab == rlk or (
                        rlk and (rlk in so_lab or so_lab in rlk)
                    ):
                        matched_so = True
                        break
                    if rlk and rlk in so_content:
                        matched_so = True
                        break
                if matched_so:
                    n_ready_with_step_output += 1
                    if _close_commitment(
                        open_sub_decisions,
                        rl,
                        at_turn=turn,
                        reason="closed_via_checkpoint_ready_step_output",
                    ):
                        n_commitments_closed_via_checkpoint += 1

            log.checkpoint_decisions_parsed = [
                {"label": d["label"], "verdict": d["verdict"]}
                for d in checkpoint_decisions
            ]
            log.checkpoint_n_ready = n_ready
            log.checkpoint_n_waiting = n_waiting
            log.checkpoint_n_abandon = n_abandon
            log.checkpoint_n_unrecognized_label = n_unrecognized
            log.checkpoint_n_ready_with_step_output = n_ready_with_step_output

            pending_checkpoint_event["agent_responded"] = bool(checkpoint_decisions)
            pending_checkpoint_event["responded_at_turn"] = turn
            pending_checkpoint_event["n_classified"] = (
                n_ready + n_waiting + n_abandon
            )
            pending_checkpoint_event["n_ready"] = n_ready
            pending_checkpoint_event["n_waiting"] = n_waiting
            pending_checkpoint_event["n_abandon"] = n_abandon
            pending_checkpoint_event["n_unrecognized_label"] = n_unrecognized
            pending_checkpoint_event["ready_labels"] = ready_labels
            pending_checkpoint_event["waiting_labels"] = waiting_labels
            pending_checkpoint_event["abandon_labels"] = abandon_labels
            pending_checkpoint_event["n_actually_delivered"] = (
                n_ready_with_step_output
            )
            pending_checkpoint_event["n_abandoned"] = n_abandon
            pending_checkpoint_event["classifications"] = [
                {"label": d["label"], "verdict": d["verdict"]}
                for d in checkpoint_decisions
            ]
            pending_checkpoint_event = None
            pending_checkpoint_labels = []
        ...
        # ---- Update stream-delivered counter ----
        if delivered_new_stream_this_turn:
            stream_msgs_delivered_count += 1

        # ---- Decide if a CHECKPOINT REVIEW should be appended ----
        # Eligibility:
        #   - scheduler enabled
        #   - cumulative stream-msgs-delivered crosses a fresh
        #     SH_CHECKPOINT_PERIOD multiple since the last fired
        #   - at least one currently-open commitment to review
        #   - not piggybacked on PREMATURE_DONE (don't pile on)
        #   - allowed to coexist with stream-delivery or stream-closed; this
        #     is a batch triage moment, not a fallback
        is_premature_done = log.premature_done
        target_multiple = (
            stream_msgs_delivered_count // SH_CHECKPOINT_PERIOD
            if SH_CHECKPOINT_PERIOD > 0
            else 0
        )
        checkpoint_eligible = (
            not SH_DISABLE_CHECKPOINT
            and SH_CHECKPOINT_PERIOD > 0
            and not is_premature_done
            and target_multiple > last_checkpoint_multiple_fired
        )
        checkpoint_appendix = ""
        if checkpoint_eligible:
            rows = _collect_open_commitments(open_sub_decisions, at_turn=turn)
            if rows:
                checkpoint_appendix = _format_checkpoint_addendum(
                    rows, at_turn=turn
                )
                log.checkpoint_injected_after_this_turn = True
                log.checkpoint_n_commitments_listed = len(rows)
                listed_labels = [r[0] for r in rows]
                event_dict: dict[str, Any] = {
                    "at_turn": turn,
                    "n_commitments_reviewed": len(rows),
                    "listed_labels": listed_labels,
                    "stream_msgs_delivered_at_trigger": stream_msgs_delivered_count,
                    ...
                    "n_waiting": 0,
                    "n_abandon": 0,
                    "n_unrecognized_label": 0,
                    "ready_labels": [],
                    "waiting_labels": [],
                    "abandon_labels": [],
                    "n_actually_delivered": 0,
                    "n_abandoned": 0,
                    "classifications": [],
                }
                checkpoint_events.append(event_dict)
                pending_checkpoint_event = event_dict
                pending_checkpoint_labels = list(listed_labels)
                last_checkpoint_multiple_fired = target_multiple
            else:
                # No open commitments — still advance the marker so we don't
                # fire repeatedly at every turn until something opens.
                last_checkpoint_multiple_fired = target_multiple

        # Compose the followup message: optional motivation prefix + body
        # + optional checkpoint addendum.
        ...
        if checkpoint_appendix:
            followup_parts.append(checkpoint_appendix)
        mt_messages.append(
            {
                "role": "user",
                "content": "\n\n".join(followup_parts),
    ...
    n_checkpoint_total = len(checkpoint_events)
    n_checkpoint_responded = sum(
        1 for ev in checkpoint_events if ev.get("agent_responded")
    )

    return {
        "scenario_id": sid,
        "variant": "mtmsg_checkpoint",
        "config": {
            ...
            "checkpoint_disabled": SH_DISABLE_CHECKPOINT,
            "checkpoint_period": SH_CHECKPOINT_PERIOD,
        ...
        "n_checkpoint_total": n_checkpoint_total,
        "n_checkpoint_responded": n_checkpoint_responded,
        "n_commitments_closed_via_checkpoint": n_commitments_closed_via_checkpoint,
        "n_commitments_abandoned_via_checkpoint": (
            n_commitments_abandoned_via_checkpoint
        ),
        "stream_msgs_delivered_count_final": stream_msgs_delivered_count,
    }
...
async def run_one_scenario(
    *,
    ...
    print(f"  [{sid}] starting variant=mtmsg_checkpoint", flush=True)
    ...
    print(
        f"  [{sid}] mtmsg_checkpoint: turns={agent_result['n_turns']} | "
        ...
        f"checkpoints={agent_result['n_checkpoint_total']} (responded={agent_result['n_checkpoint_responded']}) | "
        f"cp_closes={agent_result['n_commitments_closed_via_checkpoint']} cp_abandons={agent_result['n_commitments_abandoned_via_checkpoint']} | "
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR
        / f"eventmemory_shared_harness_v4_mtmsg_checkpoint{suffix}.sqlite3"
    )
        ...
        "scenarios": [
            {
                ...
                "n_checkpoint_total": (r.get("agent_result") or {}).get(
                    "n_checkpoint_total"
                ),
                "n_checkpoint_responded": (r.get("agent_result") or {}).get(
                    "n_checkpoint_responded"
                ),
                "n_commitments_closed_via_checkpoint": (
                    (r.get("agent_result") or {}).get(
                        "n_commitments_closed_via_checkpoint"
                    )
                ),
                "n_commitments_abandoned_via_checkpoint": (
