"""Shared harness v4_mtmsg_4k — stress test the bounded-WM architecture
at a TIGHTER 4k token cap.

Same architecture as shared_harness_v4_mtmsg_full, but WM_CAP=4000 instead
of 10000. The 10k-cap run won the 10-scenario benchmark (cov 0.820 / R@5
0.538) but truncation only fired on 3/10 scenarios — the cap was largely
dormant. This run forces truncation on ~7-10/10 to validate the
emergent-rehearsal mechanism: the model restating load-bearing facts in
recent THINKING blocks should let those facts survive truncation because
their restated form lives in un-truncated recent text.

Reference numbers (10 hard scenarios, indices 10-19):
  v4_mtmsg_full (10k):  cov 0.820 / R@5 0.538  (truncation 3/10)
  v3 operator_lru:      cov 0.718 / R@5 0.302
  v1 operator_lru:      cov 0.810 / R@5 0.506
  SA-full champion:     cov 0.876 / R@5 0.646

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_4k/main.py
"""
...
# get dropped (system stays). 4k stress test (vs 10k baseline).
MT_HARD_CAP = 4_000
...
async def main() -> None:
    ...
    sqlite_path = RESULTS_DIR / "eventmemory_shared_harness_v4_mtmsg_4k.sqlite3"
    ...
    print(f"\n=== Cross-scenario means (v4_mtmsg_4k, n={len(results)}) ===")
    ...
    print("\n=== References ===")
    print("  v4_mtmsg 10k:        cov=0.820 | full_R@5=0.538  (truncation 3/10)")
    print("  v3 op (10 hard):     cov=0.718 | full_R@5=0.302")
    print("  v1 op (10 hard):     cov=0.810 | full_R@5=0.506")
