"""Shared harness v4_mtmsg_compaction — streaming v2 + compaction-on-eviction.

Forked from shared_harness_v4_mtmsg_streaming_v2. The ONLY substantive change
vs streaming v2 is that working-memory bounding switches from hard-truncate
(drop oldest user/assistant pairs verbatim) to COMPACTION:

  When at turn-start the thread crosses `SH_COMPACTION_THRESHOLD * MT_HARD_CAP`,
  the harness:
    1. Identifies the OLDEST HALF of user/assistant pairs (preserving the
       SYSTEM_PROMPT and the most-recent SH_COMPACTION_KEEP_RECENT pairs).
    2. Calls a separate gpt-5-mini "compaction" LLM that extracts:
         (a) a 3-5 sentence narrative summary
         (b) a list of concrete facts (each tagged with original turn idx)
         (c) sub-decisions opened/closed/still-active
         (d) retrieval cues the agent should be ready to look up later
       Output is structured JSON via response_format=json_schema.
    3. Writes each extracted fact to EventMemory as a new event tagged with
       `compacted_from_turn_range` metadata.
    4. Replaces the evicted span in `mt_messages` with a single user-style
       summary message: "[COMPACTED MEMORY (covering turns X-Y): ...]".

Set `SH_DISABLE_COMPACTION=1` to fall back to hard-truncate baseline behavior
for ablation.

System prompt and follow-up templates are byte-equal to streaming v2 — the
prompt explicitly mentions hard-truncation but we deliberately do not edit it
per build instructions. Whether the prompt mismatch matters is one of the
runtime questions to validate.

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_compaction/main.py
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_compaction/main.py --scenario family-dinner-stream-01
...
NAMESPACE = "arc_em_compaction"
COLLECTION_PREFIX = "arc_cmp"
...
# Env: SH_COMPACTION_THRESHOLD (float in (0, 1))
SH_COMPACTION_THRESHOLD = float(os.environ.get("SH_COMPACTION_THRESHOLD", "0.85"))

# How many user/assistant pairs to keep intact at the END of the thread (most
# recent). Older content above this preserved tail is eligible for compaction.
# Env: SH_COMPACTION_KEEP_RECENT (int)
SH_COMPACTION_KEEP_RECENT = int(os.environ.get("SH_COMPACTION_KEEP_RECENT", "3"))

# When set to 1/true, compaction is disabled and we fall back to streaming v2's
# hard-truncate-on-cap baseline. Useful for ablation.
# Env: SH_DISABLE_COMPACTION (1/0)
...
# Max budget on the compactor LLM's response.
...
# ---------- Line parsers (verbatim from streaming v2) ----------
...
# ---------- System prompt — IDENTICAL to streaming v2 ----------
#
# Per build instructions: keep SYSTEM_PROMPT, USER_INITIAL, USER_FOLLOWUP* byte-
# equal to streaming v2. The only substantive change is replacing hard-truncate
# with compaction.
...
# ---------- Hard-truncate fallback (verbatim from streaming v2) ----------
        ...
        "properties": {
            ...
            "facts": {
                ...
                "description": (
                    ...
                    "index when known. Prefer facts that constrain future "
                    "decisions over restatements of generic context."
                ),
                "items": {
                    "type": "object",
                    "properties": {
...
Guidelines:
- Prefer facts that constrain future decisions (preferences, constraints, \
specifics, names, dates, numbers) over restatements of generic context.
...
def _format_span_for_compaction(span_messages: list[dict[str, str]]) -> str:
    """Render the to-be-evicted messages as a transcript-style block."""
    chunks: list[str] = []
...
async def call_compactor(
    ...
    *,
    ...
    """Run gpt-5-mini over the to-be-evicted span and return structured JSON.

    Returns {"narrative": str, "facts": [...], "sub_decisions": [...],
              "retrieval_cues": [...], "raw": str, "error": str|None}.
    """
...
def _identify_compaction_span(
    ...
    """Return (start_idx, end_idx) of the slice of mt_messages eligible for
    compaction. Indices are into mt_messages and refer to the slice
    `mt_messages[start_idx:end_idx]` (Python convention).

    Layout assumption:
      mt_messages[0] = system
      mt_messages[1] = first user (USER_INITIAL_STREAM)
      mt_messages[2] = first assistant
      ... alternating user/assistant ...

    Eligible-for-compaction span:
      - Skip system prompt at index 0.
      - Preserve the most-recent `keep_recent_pairs` user/assistant pairs at
        the END of the list (PLUS any trailing user without an assistant — in
        practice the loop appends a new user followup right before the next
        agent call, but compaction is checked AT TURN START before that
        followup, so the tail ends in an assistant message).
      - Take the OLDEST HALF of the remaining pairs.

    Returns (1, 1) if nothing is eligible (signal: no compaction this turn).
    """
    if len(mt_messages) <= 1:
        return (1, 1)

    # Walk the indices >=1 and pair them up. Expected pattern is U/A/U/A...
    # but be robust to a trailing unmatched message.
    body_indices = list(range(1, len(mt_messages)))
    # Group into pairs of (user, assistant). If trailing mismatched, keep it
    # as a singleton.
    pairs: list[list[int]] = []
    i = 0
    ...
    # Eligible region = pairs that are NOT in the most-recent keep_recent_pairs.
    eligible_pairs = pairs[:-keep_recent_pairs] if keep_recent_pairs > 0 else pairs[:]
    if not eligible_pairs:
        return (1, 1)

    # Oldest HALF (rounding up) of the eligible pairs.
    ...
    start_idx = min(flat)
    end_idx = max(flat) + 1  # python-slice exclusive
    return (start_idx, end_idx)


def _stream_turns_in_span(span_messages: list[dict[str, str]]) -> list[int]:
    """Best-effort extract the stream-turn integers mentioned in the span via
    the `INCOMING USER STREAM MESSAGE (turn N)` tag and the `Turn N.`
    prefix from harness followups. Returned sorted asc, dedup'd.
    """
    tag_re = re.compile(
...
async def open_empty_memory(
    ...
    *,
    ...
    ingestion turn-by-turn via OnlineStreamIngester, plus compacted facts
    via the compaction loop.
    """
    sid = scenario["id"]
    ...
    info = {
        ...
        "n_messages": 0,
        "ingest_mode": "online_with_compaction",
    }
    return memory, info
...
async def write_compacted_fact(
    *,
    ...
    """Write one compacted fact into EventMemory as a new event.

    Tagged so we can attribute later. We synthesize a unique fact_id and place
    it on the timeline at compaction_at_turn so chronological retrieval keeps
    sane ordering, while `fact_source_turn` (when known) records true origin.
    """
...
# ---------- LLM-judge DP coverage (verbatim from streaming v2) ----------
...
async def maybe_compact(
    *,
    ...
    """Decide whether to compact at this turn-start; if so, do it.

    Returns None if no compaction fired. Otherwise returns a dict with the
    full compaction-event record (suitable for trace logging).

    Mutates `mt_messages` in place: replaces the evicted span with a single
    user-style summary message.
    """
    ...
    # Write each fact to EventMemory.
    ...
    summary_msg = (
        ...
        f"Active sub-decisions: {sub_dec_block}. "
        f"Open retrieval cues: {cues_block}.]"
    )

    # Replace the span with a single user message (so the alternation user/
    # assistant continues to make sense to the chat API).
...
async def run_agent_loop(
    *,
    ...
    for turn in range(1, max_turns + 1):
        # ---- Compaction check at TURN START (before agent emits) ----
        ...
        # As a safety belt against the compactor failing to bring us under the
        # cap (e.g. one giant pair held intact in keep_recent), still apply
        # hard-truncate as last resort. This is a no-op when compaction is
        # working. When SH_DISABLE_COMPACTION is set, this IS the bounding
        # mechanism (matches streaming v2 baseline).
...
async def run_one_scenario(
    *,
    ...
    print(f"  [{sid}] starting variant=mtmsg_compaction", flush=True)
    ...
    print(
        f"  [{sid}] mtmsg_compaction: turns={agent_result['n_turns']} | "
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "running parallel ablation jobs. The collection prefix is also "
            "salted via NAMESPACE which stays constant; use distinct "
            "scenarios files for parallel runs to fully avoid collisions."
        ),
    )
    ...
    suffix = args.sqlite_suffix or ""
    sqlite_path = (
        RESULTS_DIR
        / f"eventmemory_shared_harness_v4_mtmsg_compaction{suffix}.sqlite3"
    )
