"""Shared harness v4_mtmsg_subdec_pergate_easy — per-emission gating variant.

Same architecture as shared_harness_v4_mtmsg_subdec_split_easy:
bounded WM via continuous mt_messages thread, hard-truncated from oldest at
10k tokens; emits THINKING:, PROBE:, STEP_OUTPUT:, DONE.

DELTA vs subdec_split_easy: introduces a NEW output type
`IMPLICIT_CONSTRAINTS_PROBE: <natural-language probe>` and a STRUCTURAL RULE:
every STEP_OUTPUT emission MUST be immediately preceded in the same turn by
an IMPLICIT_CONSTRAINTS_PROBE that names the implicit user-context facts
that apply to THAT specific sub-decision's deliverable.

Hypothesis: forcing the implicit-constraint question at every decision moment
(not just turn 1) yields a continuous architecture for free — and may help
on scenarios where the relevant facts only become visible AFTER intermediate
retrievals that re-shape the sub-decision.

Baseline subdec_split_easy reference (10 easy scenarios, indices 0-9):
  run1: cov 0.917 / full_R@5 0.799  (n=1 excl presentation-01)
  run2: cov 0.95  / full_R@5 0.7528 (n=1)
  n=2 mean: cov 0.934 / R@5 0.776

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_subdec_pergate_easy/main.py
    # second run (writes to results_run2/):
    SH_RESULTS_SUBDIR=results_run2 uv run python .../main.py
"""
...
# NOTE: PROBE_LINE_RE matches plain `PROBE:` only — IMPLICIT_CONSTRAINTS_PROBE
# is a separate directive and must NOT be parsed as a regular probe.
PROBE_LINE_RE = re.compile(
    r"^\s*PROBE\s*:\s*(.+?)\s*$", re.MULTILINE | re.IGNORECASE
)
# A separate parse for IMPLICIT_CONSTRAINTS_PROBE — used both as a regular probe
# (we run retrieval on it just like any other) AND for compliance telemetry.
IMPLICIT_PROBE_LINE_RE = re.compile(
    r"^\s*IMPLICIT_CONSTRAINTS_PROBE\s*:\s*(.+?)\s*$",
...
# Used to find the next directive boundary when extracting a STEP_OUTPUT body.
# Includes IMPLICIT_CONSTRAINTS_PROBE so a STEP_OUTPUT body doesn't accidentally
# absorb a following implicit-constraints-probe.
DIRECTIVE_LINE_RE = re.compile(
    r"^\s*(THINKING|PROBE|IMPLICIT_CONSTRAINTS_PROBE|STEP_OUTPUT|DONE)\b",
    re.MULTILINE | re.IGNORECASE,
)
...
def parse_implicit_probes(raw: str) -> list[str]:
    """Parse IMPLICIT_CONSTRAINTS_PROBE: <text> lines."""
    return [m.group(1).strip() for m in IMPLICIT_PROBE_LINE_RE.finditer(raw or "")]


def per_emission_compliance(raw: str) -> dict[str, int]:
    """Inspect the raw turn text and check whether each STEP_OUTPUT line is
    immediately preceded (anywhere earlier in the same turn, before the next
    STEP_OUTPUT) by at least one IMPLICIT_CONSTRAINTS_PROBE line.

    We use a structural-but-lenient interpretation: a STEP_OUTPUT is "gated"
    iff at least one IMPLICIT_CONSTRAINTS_PROBE appears between the previous
    STEP_OUTPUT (or start of turn) and the current STEP_OUTPUT header.
    """
    if not raw:
        return {"step_outs": 0, "gated": 0, "ungated": 0}
    head_iter = list(STEP_OUTPUT_HEAD_RE.finditer(raw))
    if not head_iter:
        return {"step_outs": 0, "gated": 0, "ungated": 0}
    impl_iter = list(IMPLICIT_PROBE_LINE_RE.finditer(raw))
    gated = 0
    prev_end = 0
    for h in head_iter:
        start, end = h.start(), h.end()
        # Did any implicit probe land in [prev_end, start)?
        for ip in impl_iter:
            if prev_end <= ip.start() < start:
                gated += 1
                break
        prev_end = end
    return {
        "step_outs": len(head_iter),
        "gated": gated,
        "ungated": len(head_iter) - gated,
    }
...
as the next user message. At most 4 PROBE lines per turn (counted across \
PROBE and IMPLICIT_CONSTRAINTS_PROBE combined); extras are dropped. \
No across-run cap — keep probing while memory has more to give.

IMPLICIT_CONSTRAINTS_PROBE: <retrieval query>
  Purpose: a probe specifically aimed at the implicit user-context facts \
that apply to the SPECIFIC sub-decision deliverable you are about to \
commit. Treat it as the answer to: "what does the user implicitly \
expect from THIS sub-decision that the task statement doesn't say?" \
Examples of what this catches: a banquet menu deliverable implicitly \
needs the guest allergy list; a presentation slide deliverable \
implicitly needs the brand color palette and any audience-specific \
sensitivities.
  Consequence: the harness runs it as a regular retrieval probe (top 5 \
hits by semantic similarity), counted against the same 4-per-turn cap \
as PROBE. Hits come back in the next user message.

STRUCTURAL RULE — PER-EMISSION GATING:
  Every STEP_OUTPUT emission MUST be immediately preceded in the SAME \
turn by at least one IMPLICIT_CONSTRAINTS_PROBE that names the implicit \
constraints applying to THAT specific sub-decision. The probe must \
target THAT sub-decision's deliverable, not a generic "what does the \
user want" query. If you can't think of any implicit constraints worth \
asking about, you are almost certainly missing one — sub-decisions \
without implicit constraints are rare.
  WHY: implicit-constraint facts only become relevant at the moment a \
specific deliverable is being committed. A turn-1 enumeration covers \
the obvious sub-decisions, but as work unfolds new sub-decisions emerge \
and prior facts shift in relevance. Asking the implicit-constraints \
question right before each STEP_OUTPUT — when you know what the \
deliverable is going to look like — is the moment with the highest \
signal-to-noise probe.
  COMPOSING A TURN that emits STEP_OUTPUT:
    1) THINKING: draft the sub-decision deliverable in your head.
    2) IMPLICIT_CONSTRAINTS_PROBE: what implicit user-context facts \
apply to THIS deliverable?
    3) (if you also need other facts) PROBE: ...
    4) Wait — the harness will run the probes and return hits next turn.
  After receiving hits, then on a LATER turn emit the actual STEP_OUTPUT \
informed by the surfaced facts, immediately preceded by another \
IMPLICIT_CONSTRAINTS_PROBE confirming you've covered the implicit \
constraints for that deliverable.
...
You may emit any combination of THINKING / PROBE / IMPLICIT_CONSTRAINTS_PROBE \
/ STEP_OUTPUT lines per turn, optionally followed by DONE. After your turn \
the harness parses PROBE and IMPLICIT_CONSTRAINTS_PROBE lines, runs \
retrieval, and either appends the next user message (with hits) or \
terminates if you emitted DONE.
"""
...
keep them together when they're a single unit.

Reminder of the structural rule: every STEP_OUTPUT line you eventually \
emit MUST be immediately preceded by an IMPLICIT_CONSTRAINTS_PROBE \
naming the implicit user-context facts for THAT specific deliverable. \
On this turn (turn 1), no STEP_OUTPUTs are expected yet — just probe. \
Your probes can be a mix: regular PROBEs for the obvious facts behind \
each sub-decision, plus IMPLICIT_CONSTRAINTS_PROBEs for the implicit \
user-context facts that the task description doesn't mention but that \
materially shape each sub-decision (e.g., a banquet implicitly needs \
guest allergies; a presentation implicitly needs brand guidelines). On \
later turns, when you actually emit a STEP_OUTPUT, you MUST again \
precede it with an IMPLICIT_CONSTRAINTS_PROBE for that specific \
sub-decision, even if you already probed similarly on turn 1 — the \
deliverable's exact shape only crystallizes once you're ready to commit.
"""
...
facts: {consecutive_dry_turns}. Per-emission gating last turn: \
{gated_step_outs}/{n_step_outs_last_turn} STEP_OUTPUTs were preceded \
by an IMPLICIT_CONSTRAINTS_PROBE.

Reminder: every STEP_OUTPUT this turn MUST be immediately preceded by \
an IMPLICIT_CONSTRAINTS_PROBE naming the implicit user-context facts \
for THAT specific deliverable. The probe goes BEFORE the STEP_OUTPUT \
in this same turn's text.
"""
...
class TurnLog:
    ...
    plain_probes: list[str] = field(default_factory=list)
    implicit_probes: list[str] = field(default_factory=list)
    n_hits: int = 0
    ...
    n_step_outs_gated: int = 0
    n_step_outs_ungated: int = 0
...
async def run_agent_loop(
    *,
    ...
    n_probes_total = 0
    n_implicit_probes_total = 0
    n_step_outs_gated_total = 0
    n_step_outs_ungated_total = 0
    seen_chat_ids: set[str] = set()
    done_emitted = False
    consecutive_dry_turns = 0
    # Carry-forward gating telemetry to embed in next turn's followup.
    last_turn_gated = 0
    last_turn_step_outs = 0
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        # Per-emission gating compliance check.
        compliance = per_emission_compliance(raw)
        log.n_step_outs_gated = compliance["gated"]
        log.n_step_outs_ungated = compliance["ungated"]
        n_step_outs_gated_total += compliance["gated"]
        n_step_outs_ungated_total += compliance["ungated"]
        if compliance["ungated"] > 0:
            print(
                f"  [{sid}] turn {turn}: WARNING — "
                f"{compliance['ungated']}/{compliance['step_outs']} STEP_OUTPUTs "
                f"emitted WITHOUT preceding IMPLICIT_CONSTRAINTS_PROBE",
                flush=True,
            )
        for so in step_outs:
        ...
        # Both PROBE and IMPLICIT_CONSTRAINTS_PROBE are run as retrieval queries.
        # They share the same 4-per-turn cap (in document order across both).
        plain_probes = parse_probes(raw)
        implicit_probes = parse_implicit_probes(raw)
        # Build a unified probe list preserving emission order — interleave
        # by walking match positions across both regexes.
        probe_matches: list[tuple[int, str, str]] = []
        for m in PROBE_LINE_RE.finditer(raw or ""):
            probe_matches.append((m.start(), "plain", m.group(1).strip()))
        for m in IMPLICIT_PROBE_LINE_RE.finditer(raw or ""):
            probe_matches.append((m.start(), "implicit", m.group(1).strip()))
        probe_matches.sort(key=lambda x: x[0])
        unified = [(kind, txt) for _, kind, txt in probe_matches][:4]
        probes = [txt for _, txt in unified]
        log.probes = probes
        log.plain_probes = plain_probes
        log.implicit_probes = implicit_probes
        n_implicit_probes_total += len(implicit_probes)
            ...
            mt_messages.append(
                {
                    ...
                    "content": USER_FOLLOWUP.format(
                        ...
                        gated_step_outs=compliance["gated"],
                        n_step_outs_last_turn=compliance["step_outs"],
                    ),
                }
            )
        else:
    ...
    return {
        "scenario_id": sid,
        "variant": "mtmsg_capped_pergate",
        "trace": [td_to_dict(t) for t in trace],
        "step_outputs": final_step_outputs,
        "step_outputs_log": step_outputs_log,
        "n_turns": len(trace),
        "n_probes_total": n_probes_total,
        "n_implicit_probes_total": n_implicit_probes_total,
        "n_step_outs_gated_total": n_step_outs_gated_total,
        "n_step_outs_ungated_total": n_step_outs_ungated_total,
...
def td_to_dict(t: TurnLog) -> dict[str, Any]:
    return {
        ...
        "raw_full": t.raw_full,
        "probes": t.probes,
        "plain_probes": t.plain_probes,
        "implicit_probes": t.implicit_probes,
        "n_hits": t.n_hits,
        "new_hit_ids": t.new_hit_ids,
        "n_step_outs": t.n_step_outs,
        "n_step_outs_gated": t.n_step_outs_gated,
        "n_step_outs_ungated": t.n_step_outs_ungated,
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR / f"eventmemory_shared_harness_v4_mtmsg_pergate{_DB_SUFFIX}.sqlite3"
    )
    ...
    for r in results:
        ...
        row = {
            ...
            "mtmsg_capped": {
                ...
                "n_implicit_probes_total": agent_result.get(
                    "n_implicit_probes_total", 0
                ),
                "n_step_outs_gated_total": agent_result.get(
                    "n_step_outs_gated_total", 0
                ),
                "n_step_outs_ungated_total": agent_result.get(
                    "n_step_outs_ungated_total", 0
                ),
    ...
    for r in results:
        ...
        n_implicit_probes_grand += agent_result.get("n_implicit_probes_total", 0)
        n_gated_grand += agent_result.get("n_step_outs_gated_total", 0)
        n_ungated_grand += agent_result.get("n_step_outs_ungated_total", 0)
    total_step_outs = n_gated_grand + n_ungated_grand
    summary["per_variant_means"]["mtmsg_capped"] = {
        ...
        "n_implicit_probes_total": n_implicit_probes_grand,
        "n_step_outs_gated_total": n_gated_grand,
        "n_step_outs_ungated_total": n_ungated_grand,
        "compliance_rate": round(n_gated_grand / total_step_outs, 4)
        if total_step_outs
        else None,
    ...
    # Tag the SUMMARY filename with the results-subdir suffix when run2-style.
    summary_name = (
        "SUMMARY.json"
    ...
    print(
        f"\n=== Cross-scenario means (subdec_pergate_easy, n={len(results)}, "
        f"out={_RESULTS_SUBDIR}) ==="
    )
    m = summary["per_variant_means"]["mtmsg_capped"]
    print(
        f"  pergate: cov={m['coverage_mean']} | full_R@5={m['full_R@5_mean']} | "
        f"full_R@10={m['full_R@10_mean']} | cond_R@5={m['cond_R@5_mean']} | "
        f"compliance={m.get('compliance_rate')} | "
        f"impl_probes={m.get('n_implicit_probes_total')} | "
