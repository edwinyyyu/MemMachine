...
def render_with_warning_line(
    messages: list[dict[str, str]], cap: int, warning_pct: float = 0.15
) -> list[dict[str, str]]:
    """Mark the oldest pairs that would be dropped at next truncation event.

    Computes a "warning threshold" at cap*(1-warning_pct). If current thread
    exceeds the threshold, identifies oldest pair(s) whose removal would
    bring tokens under the threshold, and wraps their content with AT-RISK
    markers so the model sees what's about to be cut.
    """
    if not messages or len(messages) < 3:
        return messages
    target = int(cap * (1 - warning_pct))
    total = messages_tokens(messages)
    if total <= target:
        return messages

    # Walk forward from index 1 (after system); find cutoff where total - removed_tokens <= target
    sys_msg = messages[0]
    body = list(messages[1:])
    removed_tokens = 0
    cutoff = 0  # exclusive end of at-risk range in body
    while cutoff < len(body) and total - removed_tokens > target:
        msg_tokens = messages_tokens([body[cutoff]])
        removed_tokens += msg_tokens
        cutoff += 1
    if cutoff <= 0:
        return messages

    # Wrap body[:cutoff] with AT-RISK markers; keep body[cutoff:] as-is
    out = [sys_msg]
    if cutoff > 0:
        marker_pre = {
            "role": "user",
            "content": "AT-RISK BLOCK START — the next "
            + str(cutoff)
            + " message(s) will be dropped at the next truncation if the thread stays over "
            + str(int(cap * (1 - warning_pct)))
            + " tokens. If they contain load-bearing facts, restate them in your THINKING this turn so they survive.",
        }
        out.append(marker_pre)
        out.extend(body[:cutoff])
        marker_post = {
            "role": "user",
            "content": "AT-RISK BLOCK END.",
        }
        out.append(marker_post)
    out.extend(body[cutoff:])
    return out
...
async def llm_chat_messages(
    ...
    *,
    ...
    rendered = render_with_warning_line(messages, MT_HARD_CAP, warning_pct=0.15)
...
WARNING-LINE MECHANISM: when the thread approaches the cap, the system marks \
the oldest content with `AT-RISK BLOCK START` ... `AT-RISK BLOCK END`. \
Anything inside this block is at risk of being dropped at the next \
truncation. When you see this marker, scan the at-risk content for any \
load-bearing facts you still need, and restate them in your next THINKING \
to lock them into your recent (un-truncated) tail.
