...
plan — buffet for 60, two veg mains, gluten-free station, $42/head.`
  Purpose: a deliverable for one sub-decision. Each STEP_OUTPUT entry \
is APPEND-ONLY: use a fresh integer id every time you emit, never \
re-use a prior id. The next id is always one higher than the highest \
id you've used so far.
  Consequence: at task end, EVERY STEP_OUTPUT you emitted is shipped to \
the user as part of the final deliverables — none are overwritten. \
This means: emit a focused, fact-specific entry as soon as you have \
concrete facts; if more facts surface later, emit a new entry with a \
new id (you can reference an earlier entry by its id in the text). The \
goal is to leave a paper trail of specific facts, not one polished \
summary.
