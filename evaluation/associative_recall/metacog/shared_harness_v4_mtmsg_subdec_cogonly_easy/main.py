"""Shared harness v4_mtmsg_subdec_cogonly_easy — cognitive-only reflection variant.

Architecture: copied from shared_harness_v4_mtmsg_subdec_split_easy. The only
substantive change is a per-turn principle-level reminder appended to
USER_FOLLOWUP that asks the model to reflect (in THINKING) on what implicit
user-context facts might apply to its current focus, and to let that
reflection shape which probes it issues — using the EXISTING probe budget,
NOT adding probes on top.

NO new output types. NO IMPLICIT_CONSTRAINTS_PROBE. NO structural emission
gating. The reflection is purely cognitive — the variant tests whether the
subdec_split lift comes from a write-time mechanism (different probe
choices) or from a retrieval-side mechanism (more probes).

Pergate (per-step IMPLICIT_CONSTRAINTS_PROBE) and boundaries (DECISION_OPEN/
CLOSE-injected directive) regressed R@5 by 10-27pp vs subdec_split despite
holding/lifting coverage. Hypothesis: the extra probe surface displaces gold
in top-5. cogonly tests the same intent without expanding the probe surface.

Usage (smoke):
    SH_SCENARIO_INDICES=1,5,8 SH_RESULTS_SUBDIR=results_smoke_s0 \\
        SH_DB_SUFFIX=_smoke_s0 uv run python \\
        evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_subdec_cogonly_easy/main.py
"""
...
Before issuing this turn's probes, briefly reflect in THINKING: what \
implicit user-context facts (things the user has shared previously that \
the task description doesn't name) might materially shape your current \
focus? Let that reflection guide which probes you choose to issue — same \
probe budget, no extra probes on top.
"""
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR
        / f"eventmemory_shared_harness_v4_mtmsg_cogonly{_DB_SUFFIX}.sqlite3"
    )
    ...
    print(f"\n=== Cross-scenario means (v4_mtmsg_subdec_cogonly_easy, n={len(results)}) ===")
