"""Shared harness v4_mtmsg_extcue_easy — external cue generator variant.

Architecture: copied verbatim from shared_harness_v4_mtmsg_subdec_split_easy.
SYSTEM_PROMPT, USER_INITIAL, USER_FOLLOWUP are IDENTICAL to subdec_split.
The only mechanism added is an EXTERNAL CUE GENERATOR: a separate
gpt-5-mini call (low reasoning) invoked once per agent turn AFTER the agent
emits its turn output and BEFORE retrieval runs. Its job is to propose
1-2 retrieval queries for implicit user-context facts based on the agent's
current focus, in a separate prompt scope (not the main conversation thread).
...
This isolates the "decoupled cognition" effect from a probe-budget
expansion confound. Each external cue is rendered to the agent (in the
NEXT user turn, alongside agent-probe hits) prefixed with `[EXT]` so the
agent can attribute hits.

Hypothesis: per-emission gating (pergate) and per-turn cogonly reflection
both regress R@5 ~25pp / -13pp by either flooding probes or stochastic
engagement. An external generator runs OUT OF BAND, doesn't disturb the
main agent's reasoning thread, and contributes a small bounded number of
implicit-constraint cues per turn.

Env hooks (matching cogonly/pergate variants):
  SH_RESULTS_SUBDIR     - per-run results dir under THIS_DIR (default: results)
  SH_SCENARIO_INDICES   - comma-separated indices into mid_execution scenarios
  SH_DB_SUFFIX          - suffix appended to SQLite filename (per-seed isolation)
...
Usage (smoke):
    SH_SCENARIO_INDICES=1,5,8 SH_RESULTS_SUBDIR=results_smoke_s0 \\
        ...
        evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_extcue_easy/main.py
"""
...
# per-turn probe cap (agent + external) is held at 4, matching the existing
# subdec_split budget. External cues fill remaining slots after agent probes,
# up to EXT_CUE_BUDGET. Override via SH_EXT_CUE_BUDGET env.
...
async def external_cue_generator(
    *,
    ...
    scope each turn focused on the current task + recent agent output.
...
async def run_agent_loop(
    *,
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        # Replacement policy: agent probes fill first (capped at total
        # per-turn budget = 4), then external cues fill remaining slots up to
        # EXT_CUE_BUDGET. If the agent saturates the total budget, no
        # external cues are added this turn (decoupled-cognition test
        # without expanding probe surface).
        ...
        ext_cues: list[str] = []
        if ext_budget_this_turn > 0 and not done_emitted:
            try:
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR
        / f"eventmemory_shared_harness_v4_mtmsg_extcue{_DB_SUFFIX}.sqlite3"
    )
    ...
    print(f"\n=== Cross-scenario means (v4_mtmsg_extcue_easy, n={len(results)}) ===")
