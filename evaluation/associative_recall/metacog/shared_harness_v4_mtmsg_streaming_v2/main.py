"""Shared harness v4_mtmsg_streaming_v2 — second-iteration streaming harness.

Adapted from shared_harness_v4_mtmsg_streaming. Three substantive changes vs v1:

  1. ONLINE INGEST. v1 pre-ingested ALL stream messages at scenario startup,
     which leaks future context into memory. v2 starts memory empty and ingests
     each stream message into EM only at the agent turn that delivers it.
     This way an agent's PROBE can only retrieve PAST messages, never future.

  2. LLM-JUDGE DP COVERAGE. v1 used token-overlap heuristics in
     `annotate_step_output` which over-credited (any keyword match). v2 calls
     a gpt-5-mini judge per (STEP_OUTPUT, decision_point) and asks whether the
     agent's deliverable text references the decision-point's required facts.
     Output is a structured JSON via response_format=json_schema.

  3. PREMATURE-DONE HANDLING. v1 broke the loop on first DONE emit. v2 only
     breaks when `done_emitted AND stream_closed_announced`. If the agent
     emits DONE before stream is closed, the harness injects a USER_FOLLOWUP
     instructing the agent to keep listening, and continues.

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_streaming_v2/main.py
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_streaming_v2/main.py --scenario family-dinner-stream-01
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_streaming_v2/main.py --scenarios-file streaming_scenarios_v2.json --scenario home-renovation-stream-01
...
NAMESPACE = "arc_em_streaming_v2"
COLLECTION_PREFIX = "arc_sv2"
...
# ---------- System prompt — IDENTICAL to v1 streaming-aware version ----------
#
...
# ---------- Thread truncation (verbatim from v1) ----------
...
class OnlineStreamIngester:
    ...
    an Event with the same property layout v1 used for pre-ingest, so probe
...
async def run_one_scenario(
    *,
    ...
    print(f"  [{sid}] starting variant=mtmsg_streaming_v2", flush=True)
    ...
    print(
        f"  [{sid}] mtmsg_streaming_v2: turns={agent_result['n_turns']} | "
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR / "eventmemory_shared_harness_v4_mtmsg_streaming_v2.sqlite3"
    )
