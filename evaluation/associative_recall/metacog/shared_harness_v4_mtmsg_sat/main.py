"""Shared harness v4_mtmsg_sat — v4_mtmsg_full + saturation-needs-evidence priming.

Identical to shared_harness_v4_mtmsg_full architecturally; the only change is
a SATURATION section added to the SYSTEM prompt (ported verbatim from v3).
Targets the two remaining 1-turn early-DONE failures observed in
v4_mtmsg_full validation:
  - adversarial-pricing-01: R@5 -0.375 vs v3 (1-turn early-DONE)
  - negation-and-inference-01: cov -0.38 vs v3 (1-turn early-DONE)

Reference numbers (10 hard scenarios, indices 10-19):
  v3 operator_lru:        cov 0.718 / R@5 0.302
  v4_mtmsg_full (D):      cov 0.820 / R@5 0.538
  SA-full champion:       cov 0.876 / R@5 0.646

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_sat/main.py
"""
...
(a) You have written STEP_OUTPUT lines for all sub-decisions in the task, AND recent retrievals consistently surface no new useful content.
...
async def main() -> None:
    ...
    sqlite_path = RESULTS_DIR / "eventmemory_shared_harness_v4_mtmsg_sat.sqlite3"
    ...
    print(f"\n=== Cross-scenario means (v4_mtmsg_sat, n={len(results)}) ===")
    ...
    print("  v4_mtmsg_full (10 h):  cov=0.820 | full_R@5=0.538")
