...
TURN_SCHEMA = {
    ...
    "properties": {
        "thinking": {
            "type": "string",
            "description": (
                "Your reasoning, working notes, and restated facts. Persists "
                "into your future context after truncation."
            ),
        },
        "probes": {
            "type": "array",
            "items": {"type": "string"},
            "description": (
                "Retrieval queries to surface stored facts. At most 4 per "
                "turn (extras are dropped). Empty array if no probes this "
                "turn."
            ),
        },
        "step_outputs": {
            ...
            "items": {
                ...
                "properties": {
                    "id": {
                        "type": "integer",
                        "description": (
                            "Small integer naming which distinct "
                            "sub-decision this entry is for. Different "
                            "sub-decisions get different ids; the same "
                            "id again is a revision (latest wins)."
                        ),
                    },
                    "text": {
                        "type": "string",
                        "description": (
                            "The deliverable text for this sub-decision. "
                            "Ships verbatim to the user as the final "
                            "deliverable for this id."
                        ),
                    },
                },
                "required": ["id", "text"],
            ...
            "description": (
                "One entry per distinct sub-decision. Empty array if no "
                "step_outputs to emit this turn."
            ),
        },
        "done": {
            "type": "boolean",
            "description": (
                "Set to true to terminate the task irreversibly. After "
                "done=true, the harness stops running your turns. The "
                "step_outputs recorded so far become the final deliverables."
            ),
        },
    },
    "required": ["thinking", "probes", "step_outputs", "done"],
    "additionalProperties": False,
}
...
working memory. You run inside a continuous conversation thread. The thread \
is your context. When it crosses a token cap, the OLDEST user/assistant \
turns are hard-dropped (the system prompt stays). There is NO compression, \
NO LRU, NO citations — just hard truncation from the front.

A fact surfaced from memory survives in your context only as long as it \
stays inside the un-truncated tail of the thread. If newer probe results \
push it past the cap, it is gone — the only way to recover it is to probe \
memory again. So when you find an important fact, restate it in your \
`thinking` field; your own words will outlive the original retrieval text.

Memory holds specific facts (constraints, preferences, allergies, dates, \
numbers, identities) the user shared previously, retrieved by semantic \
similarity to your probe text. A probe whose words appear in a stored fact \
surfaces it; a probe whose words don't match won't, even if logically \
related. The task description ALONE does not contain those facts — without \
retrieval you will produce answers that miss real binding constraints.

Your output is a JSON object matching this shape (purpose and consequence \
of each field):
...
only effect is that your future self may read it after truncation.
  ...
  probes (array of strings, max 4)
    Purpose: retrieval queries that surface stored facts.
    Consequence: the harness runs each probe (top {RETRIEVE_K} hits by \
semantic similarity) immediately after parsing your turn, then appends \
the hits as the next user message. Extras beyond 4 are dropped. No \
across-run cap — keep probing while memory has more to give.

  step_outputs (array of {id: integer, text: string})
    Purpose: finished deliverables, ONE entry per distinct sub-decision \
in the task. The id is a small integer (1, 2, 3, ...) that names which \
sub-decision the entry is for. Different sub-decisions get DIFFERENT \
ids; emitting the same id again is a REVISION of that sub-decision \
(latest version wins).
...
Whatever is in `text` is what the user receives.

  done (boolean)
    Purpose: signal that the task is complete — you have nothing more \
worth retrieving and your step_outputs reflect the work as well as you \
can do it.
    Consequence: setting `done=true` ends the run. The harness stops \
running your turns; the recorded step_outputs become the final \
deliverables. Emit `done=true` when probing has gone silent and your \
step_outputs are filled in; do NOT keep the run going indefinitely \
out of caution.

PROBE-GENERATION DISCIPLINES:
- **Implications and chains**: when a probe surfaces a fact, ask "if this \
is true, what other fact must / probably exists alongside it?" — and \
probe for that next. Most sub-decisions need 2-5 facts combined.
- **Optimistic cues**: when an abstract probe ("X's preference") fails, \
also probe specific plausible values ("X prefers Thursday", "X prefers \
30 min"). If memory contains the fact in any framing, one will surface.
- **Close reading**: facts can be stated by negation, buried in narrative, \
or mentioned in passing — don't dismiss a hit because it doesn't restate \
the probe verbatim.
"""

SYSTEM_PROMPT = SYSTEM_PROMPT.replace("{RETRIEVE_K}", str(RETRIEVE_K))
...
This is turn 1. You have no probe results yet.
"""
...
async def run_agent_loop(
    *,
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        try:
            parsed = await llm_chat_messages(openai_client, mt_messages)
        ...
        # Re-serialize for the thread so the assistant turn is well-formed
        # JSON in context (matching what the model just produced).
        ...
        # Structured outputs guarantee schema compliance — no parsing needed.
        step_outs = [
            {"raw_label": str(so["id"]), "content": so["text"]}
            for so in parsed.get("step_outputs", [])
        ]
        ...
        probes = list(parsed.get("probes", []))[:4]
        log.probes = probes
