"""Shared harness v4_mtmsg_revisit — compaction + DETERMINISTIC REVISIT scheduler.

Forked from shared_harness_v4_mtmsg_compaction_motivation (iter10).

Iter11 hypothesis: motivation as soft prompt-injection is too weak —
the LLM ignores it. A deterministic harness scheduler is stronger: the
harness picks the OLDEST open commitment and injects a specific revisit
prompt that's concrete and action-oriented ("produce STEP_OUTPUT for
commitment X now or mark it deferred"), not advisory ("you should
consider X").

This is the proactive-revisit architecture. It's a harness mechanism,
not another LLM call. Cheaper and more reliable.
...
Usage:
    ...
    SH_SCENARIO_FILE=evaluation/associative_recall/data/perpetual_scenarios.json \\
        ...
        uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_revisit/main.py \\
            --max-turns 120
...
NAMESPACE = "arc_em_revisit"
COLLECTION_PREFIX = "arc_rv"
    ...
    "schema": {
        ...
        "properties": {
            ...
            "sub_decisions": {
                ...
                "maxItems": 5,
                "description": (
                    "Concrete agent COMMITMENTS to produce a specific "
                    "deliverable (a STEP_OUTPUT such as a plan, draft, "
                    "recommendation, or decision) addressing this "
                    "sub-decision. Do NOT include user-context facts, "
                    "feedback, or state observations — those are facts, not "
                    "commitments. Hard cap of 5 items; most spans need 1-2."
                ),
                "items": {
                    "type": "object",
                    "properties": {
...
index when known)
  (c) sub-decisions that the agent has COMMITTED to producing a \
deliverable for
...
Sub-decisions are AGENT COMMITMENTS, not user-context facts. The test for \
whether something belongs in sub_decisions is: "Will the agent need to emit \
a specific STEP_OUTPUT (a plan, draft, recommendation, or decision) to \
discharge this?"
- state="opened": the agent is committed to producing a specific \
deliverable that addresses this sub-decision, and the deliverable has NOT \
yet been emitted.
- state="active": same criterion, plus the agent has begun working on the \
deliverable (e.g., partial draft, exploration) but has not yet emitted the \
final STEP_OUTPUT.
- state="closed": the agent has emitted the corresponding STEP_OUTPUT \
(the deliverable is now on record).

DO NOT mark as "opened" or "active":
- User-context facts (preferences, allergies, availability windows, \
constraints the user reported). These are facts about the world; record \
them in `facts`, not sub_decisions.
- User feedback / appreciation / reactions ("user liked X", "user \
pushed back on Y").
- State observations (what phase the project is in, what tools are \
available, who is on the team).
- Topics that were merely discussed without a corresponding agent \
commitment to deliver something.

These are FACTS, not COMMITMENTS. If you are unsure whether something is a \
commitment, ask: "What concrete STEP_OUTPUT would close this?" If no \
specific deliverable answers that question, it is NOT a sub_decision — \
record it under `facts` instead.

Hard cap: at most 5 sub_decisions per span. Most spans contain only 1-2 \
genuine commitments; emitting more than 5 is a strong signal that \
user-context facts have leaked into sub_decisions. When in doubt, omit.
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR
        / f"eventmemory_shared_harness_v4_mtmsg_revisit{suffix}.sqlite3"
    )
