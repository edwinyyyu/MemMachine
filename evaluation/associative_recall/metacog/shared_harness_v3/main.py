"""Shared harness v3 — bounded WM agent with saturation-needs-evidence priming.

ONE change vs v2 (shared_harness_v2/main.py):
  - Replaces the saturation paragraph inside DRAFTS_BLOCK with an explicit
    SATURATION section: claiming "no more useful info" requires having
    actually retrieved. Two valid grounds (drafts-complete-and-recent-
    retrievals-silent OR concrete-candidate-probes-failed). Optimistic-cue
    example included. Applied to BOTH variants.

This addresses v2's failure mode: 20/20 runs ended at turn 1 with 0-1
step_outputs because the looser priming let the model claim saturation
without ever retrieving.

Reference: SA-full reports cov 0.876 / full_R@5 0.646 on 10 hard scenarios.

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v3/main.py
"""
...
async def main() -> None:
    ...
    sqlite_path = RESULTS_DIR / "eventmemory_shared_harness_v3.sqlite3"
