"""Shared harness v2 — bounded WM agent with self-inspection priming.

Two changes vs v1 (shared_harness/main.py):
  1. Visible BEHAVIORAL HISTORY block (per-turn action counts + cue excerpts)
     and RECENT REASONING block (last 3 turns, ~80 tokens each) replace the
     prior single-line EFFORT SIGNALS.
  2. Priming reframed for self-diagnosis: dropped the rigid Turns 1-4 / 4-10
     schedule and "DO NOT emit done on turn 1" rule, replaced by a principle-
     level SELF-INSPECTION section. Applied to BOTH variants.

Also: step_outputs are revisable. Later step_output for the same step_id
overwrites earlier; scoring uses the latest version.

Reference: SA-full reports cov 0.876 / full_R@5 0.646 on 10 hard scenarios.

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v2/main.py
"""
...
# ---------- Priming prompts (v2: self-inspection + post-retrieval reflection) ----------

# Shared blocks — verbatim from spec.
...
Saturation: stop retrieving when the marginal value of the next retrieval is less than the marginal value of writing a draft revision. Keep going while productive."""
...
# ---------- Behavioral history rendering (Change 1) ----------


def render_behavioral_history(history: list[dict[str, Any]]) -> str:
    """history is a list of per-turn dicts:
    {"turn": int, "retrieves": int, "assesses": int, "step_outs": int,
     "cue_excerpts": [str, ...], "reasoning_compressed": str}
    """
    if not history:
...
async def run_agent_loop(
    *,
    ...
    trace: list[TurnLog] = []
    # step_outputs is a dict keyed by step_id — later overwrites earlier (revisable)
    step_outputs_by_id: dict[int, dict[str, Any]] = {}
    # We also keep an emitted-actions log for debugging (one entry per emit).
    ...
    # Behavioral history visible to the model
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        # Visible step_outputs are the latest per step_id.
        ...
        for a in actions:
            ...
            if atype == "retrieve":
                ...
                if isinstance(step_id_raw, int):
                    ...
                    # Fallback: assign next int after current max
                ...
                # Always push the new item to WM (so reasoning sees it)
                s_item = Item(
                    item_id=sid_item,
                    kind="step",
                ...
                if isinstance(state, OperatorState):
                    ...
                    state.push(s_item)
                # Overwrite-on-same-step_id for SCORING.
    ...
    # Final step_outputs list = latest per step_id
...
async def main() -> None:
    ...
    sqlite_path = RESULTS_DIR / "eventmemory_shared_harness_v2.sqlite3"
