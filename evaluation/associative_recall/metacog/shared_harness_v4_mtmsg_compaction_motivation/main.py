"""Shared harness v4_mtmsg_compaction_motivation — compaction + motivation.

Forked from shared_harness_v4_mtmsg_compaction. Layers a per-turn motivation
generator on top of the compaction harness. The two changes vs the compaction
harness:

  1. Scenario-source flexibility. ``SH_SCENARIO_FILE`` env var (or
     --scenarios-file flag) controls which JSON scenario file is loaded.
     Defaults to ``streaming_scenarios.json``. For perpetual benchmark set
     to ``perpetual_scenarios.json``.

  2. Motivation generator. After each compaction event ("sleep
     consolidation") AND at a periodic cadence (every
     ``SH_MOTIVATION_PERIOD`` turns), we call
     ``motivation.update_motivation`` and inject the resulting
     ``drive_directive`` as a prefix on the NEXT USER_FOLLOWUP message:
        ``[CURRENT MOTIVATION: <state>, intensity <intensity>. <directive>]``
     (existing followup body is left unchanged).

Set ``SH_DISABLE_COMPACTION=1`` to fall back to hard-truncate baseline.
Set ``SH_DISABLE_MOTIVATION=1`` to ablate motivation injection.

System prompt and follow-up templates remain byte-equal to streaming v2 / the
compaction harness — motivation directive is a NEW prefix on the followup
content string, NOT a SYSTEM_PROMPT change.

Usage examples (see BUILD-NOTES.md for the full ablation matrix):
...
# Distinct namespaces from the bare compaction harness so parallel runs don't
# clobber each other's EM state.
NAMESPACE = "arc_em_compmot"
COLLECTION_PREFIX = "arc_cm"
...
# Cap on facts emitted by the compactor per event. iter10 fix:
# without this, compactors over-extract (188-301 facts in n=1 perpetual run
# vs ~8 gold), polluting EM and degrading retrieval signal.
...
SH_DISABLE_MOTIVATION = os.environ.get("SH_DISABLE_MOTIVATION", "0").lower() in (
    "1",
    "true",
    "yes",
)
...
# ---------- Line parsers (verbatim from streaming v2 / compaction) ----------
...
async def open_empty_memory(
    ...
    *,
    ...
    info = {
        ...
        "n_messages": 0,
        "ingest_mode": "online_with_compaction_motivation",
    }
    return memory, info
    ...
    for sd in sub_decisions_from_compaction or []:
        ...
        if state == "closed":
            if existing is not None:
                ...
                # Closed-only mention without prior open; record it as closed
                # so we don't re-add later if it reappears.
            ...
            if existing is None:
                ...
                # Already tracked — refresh summary if richer; do not reset age.
            ...
            if existing is None:
                ...
                # Keep as-is; refresh summary if the compactor provided richer.
                if summary and not existing.get("summary"):
                    existing["summary"] = summary
...
def _format_open_commitments_block(
    ...
    *,
    ...
    Lists currently-active sub-decisions (state != 'closed') with their age
    in turns. Returns an empty string if nothing is active.
    """
    parts: list[str] = []
...
async def maybe_compact(
    *,
    ...
    # Defense-in-depth: enforce SH_COMPACTION_MAX_FACTS at write-side as well
    # as in the schema (LLMs occasionally exceed maxItems).
    ...
    # ---- Update harness-level open_sub_decisions tracker (iter10 Fix 1) ----
    ...
    # ---- iter10 Fix 3: explicit OPEN COMMITMENTS line sourced from the
    # harness-level tracker (NOT just this compaction's sub_decisions). This
    # refreshes the agent on commitments from earlier compactions whose
    # original turns have long since scrolled out.
...
# ---------- Motivation injection helpers ----------


def _format_motivation_prefix(state: MotivationState) -> str:
    """Render the motivation directive as the line that will prefix the next
    USER_FOLLOWUP body."""
...
def _build_recent_activity_summary(
    *,
    ...
    """Choose recent_activity_summary input for the motivation generator.

    Prefer the latest compaction's narrative if it happened within the last
    SH_MOTIVATION_PERIOD turns; otherwise concatenate the last-3 THINKING
    blocks from the agent's transcript.
    """
    if (
...
async def run_agent_loop(
    *,
    ...
    # iter10 Fix 1: harness-level open-sub-decisions tracker. Keyed by label,
    # carries {state, summary, opened_at_compaction_turn, [closed_at_turn]}.
    # Updated on each compaction event from the compactor's sub_decisions[*].
    open_sub_decisions: dict[str, dict[str, Any]] = {}

    # ---- Motivation tracking ----
    current_motivation: MotivationState = initial_motivation_state(turn=0)
    # If motivation is OFF, we keep MotivationState dormant — never updated and
    # never injected into the followup.
    motivation_events: list[dict[str, Any]] = []
    last_motivation_update_turn = 0  # turn index of most-recent update
    last_compaction_turn: int | None = None
    pending_motivation_prefix: str | None = None  # directive to prepend to next followup
    recent_thinkings: list[str] = []

    # User-input / completion bookkeeping for motivation inputs.
    # turns_since_last_user_input: counts agent turns since the last turn
    #   the harness delivered an INCOMING USER STREAM MESSAGE.
    # turns_since_last_completion: counts agent turns since the most-recent
    #   STEP_OUTPUT emission. Reset to 0 on any STEP_OUTPUT.
    turns_since_last_user_input = 0  # turn 1 will deliver via USER_INITIAL_STREAM
    turns_since_last_completion = 0  # incremented each turn, reset on emit
    ...
    for turn in range(1, max_turns + 1):
        ...
        # Capture THINKING bodies for the motivation activity-summary fallback.
        thinkings_this_turn = parse_thinking_blocks(raw)
        if thinkings_this_turn:
            recent_thinkings.extend(thinkings_this_turn)
            recent_thinkings = recent_thinkings[-6:]  # keep last few

        # --- Parse step_outputs and run LLM-judge per active DP ---
        step_outs = parse_step_outputs(raw)
        log.n_step_outs = len(step_outs)

        # If any STEP_OUTPUT emitted this turn, reset turns_since_last_completion
        # AT END of this turn (before starting next). We track here.
        ...
        # ---- Motivation update decision ----
        # Triggers:
        #   (a) post-compaction: a compaction fired this turn ("sleep
        #       consolidation" — natural moment).
        #   (b) periodic: turns since last update >= SH_MOTIVATION_PERIOD.
        # If both fire on the same turn, we still update once.
        # If neither, skip.
        ...
        if motivation_due:
            ...
            # iter10 Fix 2: pipe ACTUAL active sub-decisions into the
            # motivation generator's unresolved_goals slot so `anxious` can
            # fire on stale commitments. Each entry includes the age in turns.
            ...
            try:
                ...
                # Non-fatal; record event and keep state.
            ...
            # Queue the prefix for the NEXT followup.
            pending_motivation_prefix = _format_motivation_prefix(current_motivation)
        else:
            # No update this turn. If motivation is enabled, refresh the prefix
            # from the existing state so the NEXT followup still carries it
            # (i.e., directive persists between updates). If disabled, keep None.
            if not SH_DISABLE_MOTIVATION:
                pending_motivation_prefix = _format_motivation_prefix(current_motivation)
            else:
                pending_motivation_prefix = None

        def _wrap_followup(body: str) -> str:
            """Optionally prefix the followup body with the motivation
            directive line."""
            if pending_motivation_prefix:
                return pending_motivation_prefix + "\n\n" + body
            return body
        ...
        if done_this_turn and not stream_closed_announced:
            ...
            mt_messages.append(
                {
                    "role": "user",
                    "content": _wrap_followup(
                        USER_FOLLOWUP_PREMATURE_DONE.format(
                            turn=turn + 1,
                ...
                mt_messages.append(
                    {
                        "role": "user",
                        "content": _wrap_followup(
                            USER_FOLLOWUP_WITH_STREAM.format(
                                turn=turn + 1,
                                snippets=snippets_block,
                                n_probes_total=n_probes_total,
                                new_facts_this_turn=new_facts,
                ...
                mt_messages.append(
                    {
                        "role": "user",
                        "content": _wrap_followup(
                            USER_FOLLOWUP_STREAM_CLOSED.format(
                                turn=turn + 1,
                                snippets=snippets_block,
                                n_probes_total=n_probes_total,
                                new_facts_this_turn=new_facts,
                ...
                mt_messages.append(
                    {
                        "role": "user",
                        "content": _wrap_followup(
                            USER_FOLLOWUP_PROBE_ONLY.format(
                                turn=turn + 1,
                                snippets=snippets_block,
                                n_probes_total=n_probes_total,
                                new_facts_this_turn=new_facts,
        ...
        # Update user-input / completion counters for the NEXT motivation call.
        ...
        # Hard-truncate as last resort (also the bounding mechanism when
        # SH_DISABLE_COMPACTION is set).
    ...
    return {
        "scenario_id": sid,
        "variant": "mtmsg_compaction_motivation",
        "config": {
        ...
        # Open-sub-decisions tracker (iter10 Fix 1)
...
async def run_one_scenario(
    *,
    ...
    print(f"  [{sid}] starting variant=mtmsg_compaction_motivation", flush=True)
    ...
    print(
        f"  [{sid}] mtmsg_compaction_motivation: turns={agent_result['n_turns']} | "
        ...
        f"motiv_updates={agent_result['n_motivation_updates']} | "
        f"final_motiv={agent_result['final_motivation']['state']} | "
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR
        / f"eventmemory_shared_harness_v4_mtmsg_compaction_motivation{suffix}.sqlite3"
    )
        ...
        "scenarios": [
            {
                ...
                "n_motivation_updates": (r.get("agent_result") or {}).get(
                    "n_motivation_updates"
                ),
                "n_open_sub_decisions_final": (r.get("agent_result") or {}).get(
                    "n_open_sub_decisions_final"
                ),
                "final_motivation_state": (
                    (r.get("agent_result") or {}).get("final_motivation") or {}
                ).get("state"),
