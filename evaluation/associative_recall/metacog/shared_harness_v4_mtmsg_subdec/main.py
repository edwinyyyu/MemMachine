...
think through the task: what sub-decisions will you need to make as the \
work unfolds? Some sub-decisions have implicit constraints — facts the \
user shared previously that the task description doesn't mention but \
that materially shape the answer. (For example, planning a banquet \
...
the implicit-constraint facts behind each sub-decision.
"""
