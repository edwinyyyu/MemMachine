...
When the thread approaches the cap, the harness compacts older user \
messages by replacing their probe-result body with the placeholder text \
"(snippets dropped to fit context cap)" — the metadata header (turn \
number, probe count, dry-turn count) and your own assistant turns are \
preserved verbatim. So your THINKING from earlier turns remains, but \
the raw probe snippets you saw in those turns may be cut. This is why \
restating facts in THINKING matters.
...
SNIPPETS_DROPPED_PLACEHOLDER = "(snippets dropped to fit context cap)"

# Number of recent user-message turns to ALWAYS keep verbatim (full snippets).
# Older user messages get their snippet body cut to a placeholder, retaining
# only metadata so the model still sees probe-history context.
COMPACT_KEEP_RECENT_USER = 2


def _compact_user_message(content: str) -> str:
    """Replace the snippet body of an old user message with a placeholder,
    while keeping the surrounding metadata header/footer.

    User messages have shape:
      Turn N. New probe results from memory:\n---\n{snippets}\n---\n\n
      Probes used this run: ... New useful facts surfaced this turn: ...
      Consecutive turns with zero new useful facts: ...

    We keep everything outside the --- delimited block, replacing the inner
    snippet text with the placeholder. Verbatim cuts only — no paraphrasing.
    """
    if "---" not in content:
        return content
    parts = content.split("---")
    if len(parts) < 3:
        return content
    # parts[0] = header, parts[1] = snippet body, parts[2..] = trailing
    if SNIPPETS_DROPPED_PLACEHOLDER in parts[1]:
        return content  # already compacted
    parts[1] = "\n" + SNIPPETS_DROPPED_PLACEHOLDER + "\n"
    return "---".join(parts)


def truncate_thread(messages: list[dict[str, str]], cap: int) -> dict[str, Any]:
    """Compact-then-drop strategy.

    First pass: compact oldest user-message snippet bodies (verbatim cuts of
    the snippet text only; metadata header preserved). The model has already
    distilled load-bearing facts into its THINKING by the time the snippets
    are old, so dropping the raw snippet text loses nothing the model cared
    about.

    Second pass (fallback): if even after compacting all eligible user
    messages we're still over cap, drop oldest (user, assistant) pairs from
    the front (the original behavior).

    The system message (index 0) and the most recent COMPACT_KEEP_RECENT_USER
    user messages are always preserved verbatim.
    """
    stats = {
        "dropped_pairs": 0,
        "dropped_msgs": 0,
        "compacted_user_msgs": 0,
        "tokens_before": messages_tokens(messages),
        "tokens_after": messages_tokens(messages),
    }

    # Pass 1: compact oldest user messages (skipping the most recent
    # COMPACT_KEEP_RECENT_USER user messages).
    user_idxs = [i for i, m in enumerate(messages) if m["role"] == "user"]
    eligible_for_compact = (
        user_idxs[:-COMPACT_KEEP_RECENT_USER]
        if len(user_idxs) > COMPACT_KEEP_RECENT_USER
        else []
    )
    for idx in eligible_for_compact:
        if messages_tokens(messages) <= cap:
            break
        before = len(messages[idx]["content"])
        compacted = _compact_user_message(messages[idx]["content"])
        if len(compacted) < before:
            messages[idx]["content"] = compacted
            stats["compacted_user_msgs"] += 1

    # Pass 2: if still over cap, fall back to dropping oldest user/assistant
    # pairs.
