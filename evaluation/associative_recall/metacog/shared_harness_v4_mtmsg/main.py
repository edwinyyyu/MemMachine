"""Shared harness v4_mtmsg — bounded WM via continuous mt_messages thread,
hard-truncated at 10k tokens.

This variant strips the v3 architecture down: NO three-tier LRU, NO compression,
NO citations, NO structured JSON output, NO behavioral history blocks. Just a
plain multi-turn conversation thread (system + alternating user/assistant) where
the model emits THINKING:, PROBE:, STEP_OUTPUT:, and DONE lines verbatim. When
the thread crosses 10k tokens, the system hard-drops the oldest user/assistant
pair from the start of the thread (system prompt is preserved).

Hypothesis: SA-full's strength comes from continuous-thread reasoning, not the
elaborate three-tier LRU machinery. If a stripped-down thread at the same
10k token cap matches or beats v3, then v3's complexity isn't pulling weight.

Reference numbers (10 hard scenarios):
  v3 operator_lru:     cov 0.718 / R@5 0.302
  v1 operator_lru:     cov 0.581 / R@5 0.434
  SA-full champion:    cov 0.876 / R@5 0.646

This run: 2 scenarios only — `world-knowledge-bridge-01` and `multi-hop-banquet-01`.

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg/main.py
"""
...
async def main() -> None:
    # world-knowledge-bridge-01 is index 17, multi-hop-banquet-01 is index 10.
    SCENARIO_INDICES = [10, 17]
    K_list = SCORE_K_LIST
    ...
    sqlite_path = RESULTS_DIR / "eventmemory_shared_harness_v4_mtmsg.sqlite3"
    ...
    print("\n=== Cross-scenario means (v4_mtmsg, n=2) ===")
