"""Shared harness v4_mtmsg_subdec_boundaries_easy — decision-boundary marker variant.
...
DELTA vs subdec_split_easy: introduces TWO new output types
`DECISION_OPEN: <id>: <one-line summary>` and `DECISION_CLOSE: <id>`. The
model decides WHEN a new sub-decision begins. The harness intercepts each
NEW DECISION_OPEN (an id never opened before) and injects a system-style
directive into the NEXT USER_FOLLOWUP asking the model — for THAT specific
sub-decision — what implicit user-context facts apply. The directive fires
ONCE per decision boundary (not on every turn).

This differs from per-emission gating in two ways:
  (a) the model chooses WHEN a new decision begins (looser, less rigid),
  (b) the directive is harness-injected at boundary moments, not
      model-generated as a structural rule.

Hypothesis: boundary markers should match per-emission-gating's lift but
with FEWER redundant probes — the model only triggers the implicit-
constraint reflection when it itself recognizes a new decision.

Failure mode: model under-emits markers, missing decisions; symptom is cov
matching baseline but R@5 not lifting on implicit-decision-01 /
wedding-attire-01.
...
Per-scenario run1: banquet-01 cov=1.0 R@5=0.625;
                   implicit-decision-01 cov=0.75 R@5=0.75;
                   wedding-attire-01 cov=0.75 R@5=0.75.

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_mtmsg_subdec_boundaries_easy/main.py
    # smoke (3 critical scenarios with isolated DB):
    SH_SCENARIO_INDICES=1,5,8 SH_RESULTS_SUBDIR=results_smoke_s0 \
        SH_DB_SUFFIX=_smoke_s0 uv run python .../main.py
"""
...
# DECISION_OPEN: <id>: <one-line summary>
# id is anything up to the next colon (no colon, no newline, no dash).
DECISION_OPEN_RE = re.compile(
    r"^\s*DECISION_OPEN\s*:\s*([^\n:\-]+?)\s*[:\-]\s*(.+?)\s*$",
    re.MULTILINE | re.IGNORECASE,
)
# DECISION_CLOSE: <id>
...
DIRECTIVE_LINE_RE = re.compile(
    r"^\s*(THINKING|PROBE|STEP_OUTPUT|DECISION_OPEN|DECISION_CLOSE|DONE)\b",
    re.MULTILINE | re.IGNORECASE,
)
...
def parse_decision_opens(raw: str) -> list[dict[str, str]]:
    """Parse DECISION_OPEN: <id>: <summary> lines."""
    out: list[dict[str, str]] = []
    for m in DECISION_OPEN_RE.finditer(raw or ""):
...
DECISION_OPEN: <id>: <one-line summary>
  Purpose: mark that you are STARTING work on a new sub-decision. Use \
this when you recognize the work has shifted to a new sub-decision area \
that will eventually have its own deliverable. The id is a short stable \
handle (a small integer like `1`, `2`, `3`, OR a short string label like \
`catering`) that you will reuse on the matching STEP_OUTPUT and \
DECISION_CLOSE. The summary is a brief description of what this \
sub-decision must produce.
  Consequence: the harness records the boundary. The FIRST time it sees \
a given id opened, it will inject — into the next user message — a \
short system-style directive prompting you to identify the implicit \
user-context facts that apply to this specific sub-decision before you \
issue further probes for it.
...
id must match a previously-opened DECISION_OPEN.
  Consequence: the harness records the close. Closing is not strictly \
required (closing all decisions is implied by emitting DONE), but \
explicit closes help keep your bookkeeping clean.

STEP_OUTPUT
  Format: each line starts with `STEP_OUTPUT:` (note the colon attached \
to STEP_OUTPUT), followed by an id (matching a DECISION_OPEN id you \
previously emitted, or a small integer if you skipped opening), a \
...
When you recognize you're starting work on a new sub-decision, emit \
`DECISION_OPEN: <id>: <summary>` with a fresh id. Close it with \
`DECISION_CLOSE: <id>` when that sub-decision's deliverable is done. The \
id must match the STEP_OUTPUT id you'll later emit for that sub-decision. \
This lets the harness give you a brief, sub-decision-specific reminder at \
the moment a new boundary opens — useful precisely because the implicit \
constraints that matter most are the ones that only become visible once \
you've specified what THIS sub-decision must produce.

You may emit any combination of THINKING / PROBE / DECISION_OPEN / \
...
and either appends the next user message (with hits) or terminates if \
you emitted DONE.
"""
...
As work proceeds across later turns, formally mark sub-decision \
boundaries with the markers: emit `DECISION_OPEN: <id>: <one-line \
summary>` when you start work on a sub-decision, and `DECISION_CLOSE: \
<id>` when its deliverable is done. The id must match the eventual \
STEP_OUTPUT id. The harness will use these boundaries to give you a \
brief, sub-decision-specific reminder at the moment each new \
sub-decision opens.
"""
...
facts: {consecutive_dry_turns}.{boundary_directives}
"""


# Principle-level boundary directive injected ONCE per new DECISION_OPEN id.
# Kept neutral — no banquet/presentation examples, no specific failure modes.
BOUNDARY_DIRECTIVE_TEMPLATE = (
    '\n\n[on opening decision {decision_id} "{summary}": before any '
    "further probes for this sub-decision, identify the implicit "
    "user-context facts that materially shape the deliverable — facts "
...
class TurnLog:
    ...
    decision_opens: list[dict[str, str]] = field(default_factory=list)
    new_decision_opens: list[dict[str, str]] = field(default_factory=list)
    decision_closes: list[str] = field(default_factory=list)
    boundary_directives_injected: list[str] = field(default_factory=list)
...
async def run_agent_loop(
    *,
    ...
    # opened_decisions: ids ever opened (so we know what's "new")
    # currently_open_decisions: ids opened-but-not-closed
    # decision_summaries: id -> first-emitted summary (for directive text)
    opened_decisions: set[str] = set()
    currently_open_decisions: set[str] = set()
    decision_summaries: dict[str, str] = {}
    n_decision_opens_total = 0
    n_decision_closes_total = 0
    n_boundary_directives_total = 0
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        # Parse decision boundary markers FIRST so we can plan the directive
        # injection for the NEXT user followup.
        decision_opens = parse_decision_opens(raw)
        decision_closes = parse_decision_closes(raw)
        log.decision_opens = decision_opens
        log.decision_closes = decision_closes
        n_decision_opens_total += len(decision_opens)
        n_decision_closes_total += len(decision_closes)

        new_opens_this_turn: list[dict[str, str]] = []
        for dop in decision_opens:
            did = dop["id"]
            summary = dop["summary"]
            if did not in opened_decisions:
                opened_decisions.add(did)
                decision_summaries[did] = summary
                new_opens_this_turn.append({"id": did, "summary": summary})
            currently_open_decisions.add(did)
        log.new_decision_opens = new_opens_this_turn

        for did in decision_closes:
            currently_open_decisions.discard(did)
        ...
        # Build the boundary-directive block to APPEND to the next followup
        # message (one entry per NEW decision open in this turn).
        boundary_block = ""
        if new_opens_this_turn:
            parts: list[str] = []
            for nop in new_opens_this_turn:
                parts.append(
                    BOUNDARY_DIRECTIVE_TEMPLATE.format(
                        decision_id=nop["id"],
                        summary=nop["summary"],
                    )
                )
            boundary_block = "".join(parts)
            log.boundary_directives_injected = [nop["id"] for nop in new_opens_this_turn]
            n_boundary_directives_total += len(new_opens_this_turn)
            ...
            mt_messages.append(
                {
                    ...
                    "content": USER_FOLLOWUP.format(
                        ...
                        boundary_directives=boundary_block,
                    ),
                }
            )
        else:
    ...
    # Compliance metrics: count STEP_OUTPUTs whose raw_label was never an
    # opened decision id (orphans). raw_label is the model-emitted string, so
    # we compare against the same string set used for DECISION_OPEN ids.
    orphan_step_labels: list[str] = []
    for so in final_step_outputs:
        if so["raw_label"] not in opened_decisions:
...
def td_to_dict(t: TurnLog) -> dict[str, Any]:
    return {
        ...
        "decision_opens": t.decision_opens,
        "new_decision_opens": t.new_decision_opens,
        "decision_closes": t.decision_closes,
        "boundary_directives_injected": t.boundary_directives_injected,
...
async def run_one_scenario(
    *,
    ...
    print(f"  [{sid}] starting variant=mtmsg_capped_boundaries", flush=True)
    ...
    print(
        f"  [{sid}] boundaries: cov={cov} | full_R@5={full} | cond_R@5={cond} | "
        ...
        f"opens={agent_result['n_decision_opens_total']} | "
        f"unique_decisions={agent_result['n_unique_decisions_opened']} | "
        f"closes={agent_result['n_decision_closes_total']} | "
        f"directives_injected={agent_result['n_boundary_directives_injected']} | "
...
async def main() -> None:
    ...
    sqlite_path = (
        RESULTS_DIR
        / f"eventmemory_shared_harness_v4_mtmsg_boundaries{_DB_SUFFIX}.sqlite3"
    )
    ...
    async def _run_scen(scenario: dict) -> dict[str, Any]:
        async with SCEN_SEM:
            sid = scenario["scenario_id"]
            print(f"[run] {sid} (variant=mtmsg_capped_boundaries)", flush=True)
            try:
    ...
    for r in results:
        ...
        row = {
            ...
            "mtmsg_capped": {
                ...
                "n_decision_opens_total": agent_result.get(
                    "n_decision_opens_total", 0
                ),
                "n_decision_closes_total": agent_result.get(
                    "n_decision_closes_total", 0
                ),
                "n_unique_decisions_opened": agent_result.get(
                    "n_unique_decisions_opened", 0
                ),
                "n_boundary_directives_injected": agent_result.get(
                    "n_boundary_directives_injected", 0
                ),
    ...
    n_retrieves_grand = 0
    n_turns_total = 0
    n_decision_opens_grand = 0
    n_decision_closes_grand = 0
    n_unique_decisions_grand = 0
    n_boundary_directives_grand = 0
    n_orphans_grand = 0
    ...
    for r in results:
        ...
        n_decision_opens_grand += agent_result.get("n_decision_opens_total", 0)
        n_decision_closes_grand += agent_result.get("n_decision_closes_total", 0)
        n_unique_decisions_grand += agent_result.get("n_unique_decisions_opened", 0)
        n_boundary_directives_grand += agent_result.get(
            "n_boundary_directives_injected", 0
        )
    ...
    summary["per_variant_means"]["mtmsg_capped"] = {
        ...
        "n_decision_opens_total": n_decision_opens_grand,
        "n_decision_closes_total": n_decision_closes_grand,
        "n_unique_decisions_total": n_unique_decisions_grand,
        "n_boundary_directives_injected_total": n_boundary_directives_grand,
    ...
    print(
        f"\n=== Cross-scenario means (subdec_boundaries_easy, n={len(results)}, "
        f"out={_RESULTS_SUBDIR}) ==="
    )
    m = summary["per_variant_means"]["mtmsg_capped"]
    print(
        f"  boundaries: cov={m['coverage_mean']} | full_R@5={m['full_R@5_mean']} | "
        f"full_R@10={m['full_R@10_mean']} | cond_R@5={m['cond_R@5_mean']} | "
        f"opens={m.get('n_decision_opens_total')} | "
        f"closes={m.get('n_decision_closes_total')} | "
        f"unique={m.get('n_unique_decisions_total')} | "
        f"directives={m.get('n_boundary_directives_injected_total')} | "
