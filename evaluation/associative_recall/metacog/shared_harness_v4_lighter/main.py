"""Shared harness v4 LIGHTER — bounded WM agent with stripped scaffolding.

Variant C: drop ~2k tokens of metacognitive scaffolding per turn that v3
adds. Hypothesis: scaffolding consumes WM budget that would otherwise be
raw content; SA-full has none of this overhead and outperforms.

Kept (priming):
  - Saturation-requires-evidence
  - Drafts-not-commitments
  - Citation contract (LRU mechanism needs it)
  - Output schema

Dropped (priming):
  - Self-inspection block
  - Post-retrieval-reflection block (folded into citation contract)

Kept (per-turn user prompt):
  - TASK
  - WM contents (Tier 1 + Tier 2 + Tier 3)
  - Retrieval log: cue + hit_ids only, last 3 turns
  - STEP OUTPUTS SO FAR (count + brief listing)
  - "Respond with the JSON object now."

Dropped (per-turn user prompt):
  - Behavioral history block (per-turn action counts + cue excerpts)
  - Recent reasoning block (last 3 turns' reasoning excerpts)
  - Effort signals line

Mechanism (LRU + Tier system + citation parsing + transitive propagation)
is unchanged from v3.

Usage:
    uv run python evaluation/associative_recall/metacog/shared_harness_v4_lighter/main.py
"""
...
# Retrieval log shown to model: last 3 turns only.
RETRIEVAL_LOG_RECENT_N = 3
...
# ---------- Priming prompts (LIGHTER: only 4 sections) ----------
...
{
  "reasoning": "<free prose with inline citations like [chat-3].>",
  "actions": [
...
- TIER 2 (active LRU, verbatim, cap ~15): items you've recently CITED. The system tracks what you cite to keep relevant items active.
...
def build_user_prompt(
    *,
    ...
    parts.append("")
    parts.append(f"--- RETRIEVAL LOG (last {RETRIEVAL_LOG_RECENT_N}) ---")
    if retrieval_log:
        for entry in retrieval_log[-RETRIEVAL_LOG_RECENT_N:]:
            ids_part = ", ".join(entry.get("hit_ids", []))
            parts.append(f"  t{entry['turn']}: cue={entry['cue']!r} -> [{ids_part}]")
    else:
        parts.append("(empty)")
    parts.append("")
...
async def main() -> None:
    # Just two scenarios for quick exploration: world-knowledge-bridge-01 and multi-hop-banquet-01
    TARGET_SIDS = {"world-knowledge-bridge-01", "multi-hop-banquet-01"}
    K_list = SCORE_K_LIST
    variants = ["baseline_fifo", "operator_lru"]

    scenarios_all = load_scenarios()
    scenarios = [s for s in scenarios_all if s["scenario_id"] in TARGET_SIDS]
    ...
    sqlite_path = RESULTS_DIR / "eventmemory_shared_harness_v4_lighter.sqlite3"
    ...
    print("\n=== Cross-scenario means (v4 LIGHTER) ===")
    for variant in variants:
    ...
    print("\n=== Reference ===")
    print("  v3 op:    cov=0.718 | full_R@5=0.302  (10 hard scenarios)")
    print("  v1 op:    cov=0.581 | full_R@5=0.434  (10 hard scenarios)")
    print("  SA-full:  cov=0.876 | full_R@5=0.646  (10 hard scenarios)")
    print(f"\nWrote {summary_path}")
