...
async def main() -> None:
    ...
    print(
        f"  D:Alice->Bob  vs  D:Bob->Alice         = {c('D:Alice->Bob', 'D:Bob->Alice'):.3f}"
    )
    print("Query 'Who paid Alice?' (answer = 'Bob paid Alice'):")
    print(
        f"  -> D:Bob->Alice  (role-CORRECT)        = {c('Q:who-paid-Alice', 'D:Bob->Alice'):.3f}"
    )
    print(
        f"  -> D:Alice->Bob  (role-WRONG)          = {c('Q:who-paid-Alice', 'D:Alice->Bob'):.3f}"
    )
    print(
        f"  margin (correct - wrong)               = {c('Q:who-paid-Alice', 'D:Bob->Alice') - c('Q:who-paid-Alice', 'D:Alice->Bob'):+.3f}"
    )
    print("Query 'Who paid Bob?' (answer = 'Alice paid Bob'):")
    print(
        f"  -> D:Alice->Bob  (role-CORRECT)        = {c('Q:who-paid-Bob', 'D:Alice->Bob'):.3f}"
    )
    print(
        f"  -> D:Bob->Alice  (role-WRONG)          = {c('Q:who-paid-Bob', 'D:Bob->Alice'):.3f}"
    )
    print(
        f"  margin (correct - wrong)               = {c('Q:who-paid-Bob', 'D:Alice->Bob') - c('Q:who-paid-Bob', 'D:Bob->Alice'):+.3f}"
    )

    print("\n================ SURFACE (entity swap) ================")
    print(
        f"  D:squirrel  vs  D:raccoon              = {c('D:squirrel', 'D:raccoon'):.3f}"
    )
    print(
        f"  D:squirrel  vs  D:coffee-only          = {c('D:squirrel', 'D:coffee-only'):.3f}"
    )
    print("Query 'What did I see on my way to work today?':")
    print(
        f"  -> D:squirrel                          = {c('Q:saw-work', 'D:squirrel'):.3f}"
    )
    print(
        f"  -> D:raccoon                           = {c('Q:saw-work', 'D:raccoon'):.3f}"
    )
    print(
        f"  -> D:coffee-only (no animal)           = {c('Q:saw-work', 'D:coffee-only'):.3f}"
    )
