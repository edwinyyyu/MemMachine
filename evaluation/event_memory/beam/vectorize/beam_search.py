...
leak judge-side ground truth into the model's context at answer-generation
time.

This methodology is NOT comparable to the official BEAM paper. It is included
here so EventMemory can be benchmarked on Vectorize's setup for head-to-head
comparison against their published numbers.
"""
...
async def main():
    ...
    parser.add_argument(
        ...
        help=f"Answer-generation model (default: {_DEFAULT_VECTORIZE_ANSWER_MODEL})",
    )
    ...
    async def process_question(conv: BEAMConversation, question: BEAMQuestion):
        partition_key = _partition_key(conv.conversation_id)
        ...
        if collection is None:
            print(
                f"No collection for conversation {conv.conversation_id}; "
