...
- "google" — `google.genai.Client`, via `GEMINI_API_KEY` / `GOOGLE_API_KEY`.

The google path is loaded dynamically via `importlib` so this module can be
type-checked even when `google-genai` isn't installed.
"""
...
def _load_genai_modules() -> tuple[Any, Any]:
    """Return `(genai, types)` from `google.genai`, raising if not installed."""
...
def _build_gemini_schema(types_module: Any, schema_dict: dict[str, Any]) -> Any:
    type_map = {
        "string": types_module.Type.STRING,
        "boolean": types_module.Type.BOOLEAN,
        "integer": types_module.Type.INTEGER,
        "number": types_module.Type.NUMBER,
    }
    ...
    for name, spec in properties_dict.items():
        ...
        prop = types_module.Schema(
            type=type_map.get(spec_type, types_module.Type.STRING)
        )
...
class GoogleChatClient(ChatClient):
    def __init__(self, api_key: str | None = None) -> None:
        genai, types_module = _load_genai_modules()
        ...
        *,
        ...
        if response_format is not None:
            ...
            if kind == "json_schema":
                ...
                config_kwargs["response_schema"] = _build_gemini_schema(
                    types_module, schema
                )
        ...
        config = (
            types_module.GenerateContentConfig(**config_kwargs)
            if config_kwargs
            else None
        )
