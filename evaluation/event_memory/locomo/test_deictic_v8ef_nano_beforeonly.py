"""V8E and V8F prompts on gpt-5.4-nano @ low with BEFORE-ONLY neighbors.

V8E = single-pass / atemporal framing (eligibility checks describe
output properties, not workflow state).
V8F = step-by-step workflow framing (per-reference in-order loop).

Both prompts share the same WHEN TO EDIT eligibility bullets after
the recent updates (idiom rule, passage rule, "already resolved"
check, universal-reader obviousness questions) — the only structural
difference is whether the loop is implicit (V8E) or explicit (V8F).

Runs both segmenters concurrently per case.
"""
...
from probe_deictic_resolved_verbatim_v8e import DeicticResolvedV8ESegmenter
from probe_deictic_resolved_verbatim_v8f import DeicticResolvedV8FSegmenter
...
async def main():
    ...
    def _lm() -> OpenAIChatCompletionsLanguageModel:
    ...
    segs = {
        "V8E": DeicticResolvedV8ESegmenter(language_model=_lm()),
        "V8F": DeicticResolvedV8FSegmenter(language_model=_lm()),
    }
