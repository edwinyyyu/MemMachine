"""V8J prompt on gpt-5.4-nano at medium and high reasoning effort.

Same prompt and same 25 cases as test_deictic_v8j_nano_beforeonly.py;
only the reasoning_effort changes. Runs both tiers concurrently for
direct comparison vs the nano@low baseline.
"""
...
from probe_deictic_resolved_verbatim_v8j import (
    DatedSurroundingEvent,
    DeicticResolvedV8JSegmenter,
)
...
def _surrounding(dia_map, target_id, nb):
    ...
    return (
        [DatedSurroundingEvent(
            producer=dia_map[i]["speaker"],
            text=dia_map[i]["text"],
            timestamp=dia_map[i]["timestamp"],
        ) for i in before_ids],
        [],
    )
...
async def main():
    ...
    def _seg(effort: str) -> DeicticResolvedV8JSegmenter:
        lm = OpenAIChatCompletionsLanguageModel(
            OpenAIChatCompletionsLanguageModelParams(
                client=client, model="gpt-5.4-nano", reasoning_effort=effort,
            )
        )
        return DeicticResolvedV8JSegmenter(language_model=lm)
