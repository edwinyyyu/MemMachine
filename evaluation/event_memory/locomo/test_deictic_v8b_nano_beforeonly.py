"""V8b prompt on gpt-5.4-nano @ low with BEFORE-ONLY neighbors.

V8b differs from v8 only in metadata placement (SPEAKER label moved
to adjacent to MESSAGE, replacing the standalone SPEAKER: line at top).
Same 25 representative cases as v8 so we can read v8b vs v8 directly.
"""
...
async def run_one(seg, msg, before, after, label):
    ...
    print(f"V8B: {out[0].block.text if out else '(empty)'}", flush=True)
...
async def main():
    ...
    seg = DeicticResolvedV8BSegmenter(language_model=lm)
