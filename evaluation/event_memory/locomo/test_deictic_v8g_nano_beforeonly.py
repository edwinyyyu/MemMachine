"""V8G prompt on gpt-5.4-nano @ low with BEFORE-ONLY neighbors.

V8G = V8E + tightened vocative rule ("addressee is almost never the
speaker", vocative placed in same sentence containing "you", example
names both speaker (Bob) and addressee (Alice)).

Same 25 cases as v8e/v8f so we can read v8g vs v8e directly.
"""
...
from probe_deictic_resolved_verbatim_v8g import DeicticResolvedV8GSegmenter
...
async def main():
    ...
    seg = DeicticResolvedV8GSegmenter(language_model=lm)
