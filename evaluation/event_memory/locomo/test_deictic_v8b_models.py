"""V8B prompt across model tiers -- 25 manual cases only.

Tests whether the V8B prompt (speaker label adjacent to message) is
better followed at higher model tiers. The dominant remaining failures
at gpt-5.4-nano @ low were:
  - mid-sentence vocative ("don't you, Joanna, think?") despite the
    explicit "(not mid-sentence)" rule
  - "You? Anything new" → "Dave, anything new" despite the explicit
    "The original 'you' stays" rule

Tiers:
  - MINI@med = gpt-5-mini @ medium  (production-allowed ceiling)
  - FULL@med = gpt-5.4 @ medium     (one-off; not for production)

Before-only neighbors (8 prior, 0 future). Runs both models
concurrently per case.
"""
...
from probe_deictic_resolved_verbatim_v8b import DeicticResolvedV8BSegmenter
...
async def run_one(segs, msg, before, after, label):
    """Run all models concurrently, return list of (name, text)."""
    event = Event(
        uuid=uuid4(),
    ...
    results = await asyncio.gather(*(seg.segment(event) for seg in segs.values()))
    return [
        (name, out[0].block.text if out else "(empty)")
        for (name, _), out in zip(segs.items(), results)
    ]
...
async def main():
    ...
    def _seg(model: str, effort: str) -> DeicticResolvedV8BSegmenter:
        lm = OpenAIChatCompletionsLanguageModel(
            OpenAIChatCompletionsLanguageModelParams(
                client=client, model=model, reasoning_effort=effort,
            )
        )
        return DeicticResolvedV8BSegmenter(language_model=lm)

    segs = {
        "MINI@med": _seg("gpt-5-mini", "medium"),
        "FULL@med": _seg("gpt-5.4", "medium"),
    }

    # Schedule all cases concurrently; print serially in TARGETS order after.
    valid = [
        (cid, dia, label) for cid, dia, label in TARGETS
        if cid in dia_maps and dia in dia_maps[cid]
    ]
    case_results = await asyncio.gather(*(
        run_one(
            segs,
            dia_maps[cid][dia],
            *_surrounding(dia_maps[cid], dia, NB),
            label,
        )
        for cid, dia, label in valid
    ))

    for (cid, dia, label), results in zip(valid, case_results):
