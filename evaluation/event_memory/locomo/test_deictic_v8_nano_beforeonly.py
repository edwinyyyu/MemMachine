"""V8 prompt on gpt-5.4-nano @ low with BEFORE-ONLY neighbors.

Same 25 representative cases as test_deictic_v7_nano_beforeonly.py
so we can read v8 vs v7 directly.
"""
...
from probe_deictic_resolved_verbatim_v8 import DeicticResolvedV8Segmenter
...
async def run_one(seg, msg, before, after, label):
    ...
    print(f"V8 : {out[0].block.text if out else '(empty)'}", flush=True)
...
async def main():
    ...
    seg = DeicticResolvedV8Segmenter(language_model=lm)
