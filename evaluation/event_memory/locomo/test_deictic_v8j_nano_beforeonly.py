"""V8J prompt on gpt-5.4-nano @ low, BEFORE-ONLY neighbors.

V8J renders the surrounding context + target message as a chat log:

    Nate (February 24, 2024)
    Hey Joanna! Long time no talk, how's it going?

    Joanna (February 24, 2024)
    Hey Nate! Been quite a ride - had an audition yesterday for a writing gig.

    ...
    MESSAGE TO EDIT:
    Nate (February 24, 2024)
    Congrats! How did it go? Are you excited?

Same 25 cases as v8e/.../v8i for direct comparison.
"""
...
async def run_one(seg, msg, before, after, label):
    ...
    return out[0].block.text if out else "(empty)"
...
async def main():
    ...
    seg = DeicticResolvedV8JSegmenter(language_model=lm)
    ...
    for (cid, dia, label), text in zip(valid, case_results):
