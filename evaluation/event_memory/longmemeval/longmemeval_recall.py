"""Compute recall@k against LongMemEval gold turns from a search dump."""
...
def compute_recall(items: list[dict], max_k: int) -> dict:
    """Recall@k by scored segment context position (fast).
    ...
    for item in items:
        ...
        for rank, ssc in enumerate(query_result.scored_segment_contexts):
            for seg in ssc.segments:
...
def compute_recall_unified(items: list[dict], max_k: int) -> dict:
    ...
    For each segment budget k, runs build_query_result_context and
...
def main():
    ...
    parser.add_argument("--max-k", type=int, default=200, help="Maximum k for recall@k")
    parser.add_argument(
        "--unify",
        action="store_true",
    ...
    results: dict[str, Any] = {"mode": "unified" if args.unify else "ranked"}
