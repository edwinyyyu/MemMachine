...
Processes questions in batches (default 20). Each question is retried up to
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Max code-point length for segment chunks (default: 500)",
    )
    return parser.parse_args()


def _select_questions(
    all_questions: list[LongMemEvalItem],
...
async def _run_batches(
    questions: list[LongMemEvalItem],
    batch_size: int,
    ingest_with_retry,
) -> tuple[int, list[str]]:
    succeeded = 0
    ...
    for batch_idx in range(num_batches):
        ...
        for question, ok in zip(batch, results, strict=True):
            if ok:
                succeeded += 1
            else:
                failed_questions.append(question.question_id)

    return succeeded, failed_questions


async def main():
    args = _parse_args()

    all_questions = _select_questions(
        load_longmemeval_dataset(args.data_path), args
    )
    if args.question_id and not all_questions:
    ...
    deriver = SentenceTextDeriver() if args.derive_sentences else WholeTextDeriver()
    ...
    async def ingest_question(question: LongMemEvalItem):
        ...
        # Process sessions sequentially for deterministic ordering.
            ...
            events = [
                Event(
                    ...
                    context=ProducerContext(
                        producer="Assistant" if turn.role == "assistant" else "User"
                    ),
    ...
    succeeded, failed_questions = await _run_batches(
        all_questions, args.batch_size, ingest_with_retry
    )
