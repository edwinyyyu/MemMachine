"""LongMemEval dataset models for EventMemory benchmarking."""
...
class LongMemEvalItem(BaseModel):
    ...
    haystack_datetimes: dict[str, datetime] = {}
    ...
    def _post_init(self):
        ...
        for session_id in self.haystack_session_ids:
            ...
            self.haystack_datetimes[session_id] = session_datetime
            for i, turn in enumerate(session):
                self.turn_id_map[session_id][i] = turn
                turn.timestamp = session_datetime + timedelta(seconds=i)
                turn.index = i
                if turn.has_answer:
...
def load_longmemeval_dataset(file_path: str) -> list[LongMemEvalItem]:
    """Load the dataset from the given JSON file path."""
...
def get_datetime_from_timestamp(ts: str) -> datetime:
    """Convert timestamp string like '2023/04/10 (Mon) 23:07' to a UTC datetime."""
