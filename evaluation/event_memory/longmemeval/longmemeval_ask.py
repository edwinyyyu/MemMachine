...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Max segments after unification (default: 20)",
    )
    parser.add_argument(
        "--model",
        default="gpt-5-mini",
    ...
    segment_store_partition = await segment_store.open_or_create_partition(
        question.question_id, SegmentStorePartitionConfig()
    )

    # EventMemory requires a segmenter and deriver even when only querying.
    memory = EventMemory(
        EventMemoryParams(
