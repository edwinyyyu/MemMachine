"""LLM segmenter, Mode F-natural v32 -- structural-content preservation.

v30 fails on ASCII art across nano variants (30-70% failure rate over
10 reps). The model either drops the art as "decorative" or
paraphrases its whitespace/backslashes. Other structural content
(tables, code, arrows, morse) passes 10/10, so the failure is
specifically art-shaped non-prose with whitespace-heavy layout.

Two targeted additions, keeping the existing design principles
(short prompt, general rule, minimal examples):

  Rule 1 (VERBATIM): adds "preserve whitespace, newlines, and
  special characters within a segment exactly" -- closes the
  whitespace-normalization paraphrase loophole.

  Rule 2 (KEEP side): adds "distinctive non-prose blocks (code,
  tables, ASCII art, encoded text)" to the enumeration of specific
  content -- gives the model an explicit hook so it doesn't classify
  visually-heavy distinctive content as boilerplate.

Both changes are scoped to address the structural-preservation gap.
No new rules; no new examples beyond the parenthetical.
"""
...
states tied to events, constraints, distinctive phrasing, and \
distinctive non-prose blocks (code, tables, ASCII art, encoded text). \
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V32.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V32.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
