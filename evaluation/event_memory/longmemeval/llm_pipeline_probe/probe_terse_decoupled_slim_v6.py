"""Terse-decoupled segmenter v6 -- slim_v3 + model-robust objective rules.

Prompt-only change vs slim_v3. The slim_v3 LLM-robustness matrix
({gpt-5-nano, gpt-5.4-nano, gpt-5-mini} x {low, medium}, nb8, K=10)
showed accuracy was robust (~1.5pp spread) but SEGMENT VERBOSITY was
not: gpt-5-nano wrote 26.7 tok/terse vs gpt-5.4-nano's 21.4 (+25%) and
produced 18% more segments -- blowing the 350-token budget at K=10
(382 tok/q vs 310).

Diagnosed from segment dumps -- two prompt-ambiguity failures, not model
weakness:
  1. Over-segmentation. The keep-rule was a CATEGORY LIST ("emit nothing
     for greetings, thanks, ..."). Weak models saw a non-empty message
     and emitted an item anyway (kept "James apologizes for not getting
     back sooner, asks how it's going"). Fix: an objective GATE -- an
     item is earned ONLY IF the message states >=1 particular.
  2. Verbose terse. "fewest words that stay unambiguous" is subjective.
     Weak models restated one point several ways ("a rewarding
     challenge" + "a total win" + "seeing the smiles is a win"). Fix:
     an objective anti-redundancy rule (state each point ONCE) plus a
     deletion framing for `terse` (delete from `memory`, never
     paraphrase upward / add words).

Everything else (date section, neighbours, particular definition, the
3-field {memory, terse, queries} output) is byte-identical to slim_v3.
"""
...
# Slimmed prompt v6 (slim_v3 + objective keep-gate + objective
# anti-redundancy/deletion rules for cross-model consistency).
PROMPT_SLIM_V6 = """\
...
topic; most messages raise just one.

A message earns an item ONLY IF it states at least one PARTICULAR \
(defined next). Apply this as a gate: if the message states no \
particular -- a bare greeting, sign-off, thanks, acknowledgement, \
reassurance, pleasantry, generic reflection, or generic question -- it \
earns nothing, however many words it has.
...
is the main failure to avoid. State each particular, fact, feeling, \
and evaluation EXACTLY ONCE -- never restate the same point in other \
words; restatement is the main way an item bloats.
...
this/that/there to what they refer to. Keep every particular; state \
each one once, plainly.
...
(B) "terse" -- the shortest faithful form of (A), and the ONLY text \
retrieval shows a reader, so it must stand on its own. Form it by \
DELETING from (A): remove every word that carries no particular and is \
not needed for grammar -- articles, hedges, intensifiers ("really", \
"such a", "definitely"), scene-setting, and any clause that only \
re-states a point already made. Do not paraphrase and do not add words \
(A) lacks. The result is one plain, readable sentence -- not a headline \
or telegraphic fragment; keep every particular and the same date \
handling.
...
class TerseDecoupledSegmenter(Segmenter):
    """Terse-decoupled segmenter, slim_v6 model-robust prompt."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_SLIM_V6,
        chunk_size: int = 1500,
        max_attempts: int = 3,
