"""LLM rewriting segmenter v31 — v28 with multi-axis trailing topic tag.

v28 appends a single canonical category noun ("[category: tournament
win]") which provides a shared retrieval cue across instances of the
same class. But many cat 1 enumeration queries hinge on a
sub-category-specific axis: "tournaments WON" vs "tournaments
PARTICIPATED IN", "screenplays REJECTED" vs "screenplays WRITTEN".
A single trailing tag can only point to one axis. v26's
multi-paraphrase fix regressed -7pp because additional segments
collapsed event coverage under K=7.

v31 keeps v28's one-statement-per-event invariant and replaces the
single-tag rule with a SHORT LIST of 2-4 hyphenated noun axes —
e.g., "[topics: tournament-win, video-game-tournament, regional,
competition]". Each axis is a stand-alone single-token-ish noun
phrase so BM25 lights up on direct word match; the additional axes
expand retrieval paths without growing segment count, so event
coverage at K=7 is preserved.
"""
...
- When the event is an instance of a recurring class -- a \
recommendation, a tournament outcome, a hike, a meal, a purchase, \
a visit, a workout, a movie watched, a book read, a meeting -- \
APPEND a trailing tag listing the queryable AXES that future \
enumeration questions could ask about this specific event. \
Each axis is a single hyphenated noun phrase (no spaces inside an \
axis, no verbs, no adjectives standing alone). Use 2-4 axes when \
the event has multiple distinct queryable angles (class, outcome, \
sub-class, location/scope, medium, participant role); use 1 axis \
when only the class is queryable. Order: class first, then \
outcome/result, then sub-class or medium, then scope/location. \
Use a consistent vocabulary across instances of the same class so \
they share retrieval cues. The trailing tag NEVER replaces or \
precedes the particulars; entity name, date, and other concrete \
particulars must appear in the body of the statement first. \
Format: " [topics: <axis-1>, <axis-2>, ...]". Examples: \
"Joanna watched Eternal Sunshine of the Spotless Mind on \
2022-04-10 with Nate. [topics: movie-watched, film, drama, \
romance]" \
"Nate placed 2nd at the regional video-game tournament on \
2022-04-25. [topics: tournament-participation, tournament-loss, \
video-game-tournament, regional-tournament, competition]" \
"Joanna recommended the book Project Hail Mary to Nate on \
2022-09-10. [topics: book-recommendation, recommendation, \
science-fiction]" \
"Nate's screenplay Dust Drifts was rejected by Studio Lumen on \
2022-06-03. [topics: screenplay-rejection, screenplay-submission, \
screenplay, studio-submission]" \
"Joanna bought a hiking backpack from REI on 2022-08-15. \
[topics: purchase, gear-purchase, hiking-gear, retail-store]"
...
class RewriteSegmenter(Segmenter):
    """v31 — v28 with multi-axis trailing topic tag for enumeration cues."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V31,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
