"""LLM rewriting segmenter v9 — v7 + tighter filler drop.

v8 confirmed that aggregation helps low-K but hurts high-K (+1.7pp @
l8, -0.9pp @ l160; +21% tokens). v9 takes a more targeted lever:
keep v7's granular ONE-TOPIC-PER-STATEMENT rule (preserves high-K
wins), but adopt v8's tighter rule 9/10 (drop bare questions, "Bye",
"Take care", "I'm here for you"). Expected: -5 to -10% tokens at all
limits, parity or better accuracy.
...
her", "{speaker} asked the other party about their day", "{speaker} \
wondered what they thought") produce no statement. If the message \
contains incidental personal facts inside a question or request \
("Have you tried the new bakery on 4th Street?"), extract those \
facts as a statement.

10. DROP PURE FILLER. Greetings ("Hi", "Hey", "How are you"), \
sign-offs ("Bye", "Take care", "Talk soon"), bare reassurances ("I'm \
here for you", "You got this", "You're amazing"), and bare \
acknowledgments ("Sounds good", "Thanks", "Got it", "Yeah", "Cool") \
that carry no specifics produce no statement. A message whose every \
part is interchangeable filler -- no name, date, number, decision, \
plan, preference, opinion, event, hobby, relationship detail, or \
attached media description -- emits an empty list.
...
class RewriteSegmenter(Segmenter):
    """v9 — v7 prompt + tighter filler/meta-commentary drop."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V9,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
