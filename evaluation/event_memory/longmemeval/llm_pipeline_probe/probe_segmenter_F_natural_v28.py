"""LLM segmenter, Mode F-natural v28 -- all-ASCII (v27) + flush rule list.

v27 replaced em-dashes with '--'. v28 also removes the gratuitous
2-space indent before each numbered rule -- it doesn't help and adds
noise.
"""
...
Interchangeable content is stock framing that appears across many \
passages regardless of subject -- greetings, sign-offs, polite \
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V28.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V28.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
