"""v32 deriver — filler rule integrated into goal paragraph.

v29/v31 had a dangling "Otherwise" / "All other segments" closing that
came after a sentence about content-bearing affirmatives. The referent
was ambiguous.

v32 unifies: state the general rule ("at least one per segment") with
the filler EXCEPTION immediately, then the filler sub-exception
(content-bearing affirmative). One coherent statement, no dangling
references.

Form:
  "Emit at least one derivative per segment — UNLESS the segment is pure
   filler ('yes', 'no', ...), in which case emit ZERO. An affirmative
   that ALSO carries concrete content ('ok, leaving Tuesday at 5') is
   NOT pure filler — emit derivatives for the content."
"""
...
coverage. Emit at least one derivative per segment — UNLESS the segment \
is PURE FILLER (content-free reactions or affirmations only meaningful \
with the prior message: \"yes\", \"no\", \"ok\", \"sounds good\", \
\"will do\", \"thanks\", \"lol\", \"great point\", \"omg yes!!\", \
\"acknowledged\"), in which case emit ZERO derivatives. An affirmative \
that ALSO carries concrete content (\"ok, leaving Tuesday at 5\", \
\"yes, I'll bring the wine\") is NOT pure filler — emit derivatives for \
the content.
...
if __name__ == "__main__":
    print(
        f"v32 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
