"""v46 deriver -- v45 with the ONE central subject list replaced by an
open principle.

User feedback: v45's "ONE central subject -- a person, place, event,
project, process, product, relationship, preference, opinion, or
decision" misses topics, concepts, theorems, definitions, and anything
else not in the list. Explicit lists are fragile -- the next missing
type breaks the rule.

v46 replaces with a principle + non-exhaustive examples:
  v45: "ONE central subject -- a person, place, event, project,
       process, product, relationship, preference, opinion, or
       decision -- ..."
  v46: "ONE central subject -- whatever single thing the segment is
       about, whether an entity (person, place, project), a process or
       event, a concept (theorem, definition, topic), a mental state
       (preference, opinion, decision), or any other coherent subject
       -- ..."

The "or any other coherent subject" escape hatch tells the model: the
list is illustrative, not exhaustive. The test is "is every sentence
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v46 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
