"""LLM segmenter, Mode F-natural v26 — remove redundant clause.

v25's rule 3 had: "When such a response ALSO carries content ('ok,
leaving Tuesday at 5', ...), it remains KEPT — emit the whole
utterance, not a fragment." This is redundant:
  - Rule 2 already says KEEP what is specific to this passage (covers
    any response that carries content).
  - Rule 1 (VERBATIM, contiguous quote at sentence/clause boundaries)
    prevents fragmenting.

v26 drops the clause.
"""
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V26.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V26.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
