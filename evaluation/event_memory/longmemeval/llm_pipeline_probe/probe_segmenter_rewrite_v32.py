"""LLM rewriting segmenter v32 — v22 + anti-speech-act rule (no tags).

v30 (= v22 + trailing category tag + anti-speech-act) hit g3 c124 = 90.96%
@ 320 tokens, but disambiguation showed:
- ~+5.3pp of the gain came from BM25 keyword overlap on the in-text tag,
  which depends on LLM+user converging on canonical nouns and only bridges
  morphological variants (lemmatization), not true synonyms.
- ~+0.5pp came from the answerer seeing the tag.
- The anti-speech-act rule's retrieval-quality benefit is independent of
  the tag mechanism.

The in-text tag is benchmark-specific (the correct production direction
is a side-channel keyword store). v32 keeps the generalizable
anti-speech-act rule WITHOUT the tag, isolating its standalone contribution
on a clean v22 baseline.
"""
...
class RewriteSegmenter(Segmenter):
    """v32 — v22 + anti-speech-act rule, no tags."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V32,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
