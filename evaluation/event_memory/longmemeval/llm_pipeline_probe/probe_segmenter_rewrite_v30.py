"""LLM rewriting segmenter v30 — v28 (trailing category tag) + v27q anti-speech-act.

v28 hits g3 c124 = 89.89% @ 350 tokens/q by appending a trailing
" [category: <noun>]" tag for recurring-class events on top of the
v22 baseline; this lifts cat 1 multi-hop +10.8pp without hurting
cat 2 / cat 4.

A separate v27q ablation showed that "speech-act echo" segments
("X asked Y about Z", "What Y's question to X was") are toxic for
retrieval: their wh-tokens collide with future user queries on
cosine similarity and displace answer-bearing segments. v27q
suppressed those, lifting cat 1 +2.7pp and cat 2 +7.5pp over v27,
though v27's own Q-style framing hurt cat 4 split-facts so v27q
finished below v28 overall.

v30 hypothesis: speech-act echoes pollute retrieval regardless of
base prompt; combining v28's trailing-tag retrieval cue with
v27q's anti-speech-act rule should give an ADDITIVE gain on cat 1
(more answer-bearing segments crowding into top-K) without
re-introducing v27's cat 4 fragmentation, since v30 keeps v22's
"one statement per event" structure intact.
"""
...
fact and MUST be dropped. Emitting any statement of the form "X \
asked Y about Z", "X asked Y what/whether/if ...", "Y's question \
to X was ...", "X wanted to know about Y" is a FAILURE. Only the \
ANSWER to such a question becomes a stored fact, and only if the \
answer carries specific content. If the speaker asks a question \
that itself contains a specific plan, opinion, or decision \
("Should we still meet at the Blue Door at 7pm?"), keep ONLY the \
embedded specific content as a fact ("The planned meeting is at \
the Blue Door at 7pm on {date}."), not the act of asking.
...
recommendation, a tournament win, a hike, a meal, a purchase, a \
visit, a workout, a movie watched, a book read, a meeting -- \
APPEND a canonical category noun as a trailing tag in the form \
" [category: <noun>]" so all instances of the same class share a \
retrieval cue. The trailing tag never replaces or precedes the \
particulars; entity name, date, and other concrete particulars must \
appear in the body of the statement first. Use a single, consistent \
category noun across all instances of the same class. Examples: \
"Joanna watched Eternal Sunshine of the Spotless Mind on \
2022-04-10 with Nate. [category: movie watched]" \
"Nate placed 2nd at the regional tournament on 2022-04-25. \
[category: tournament win]" \
"Joanna recommended the book Project Hail Mary to Nate on \
2022-09-10. [category: book recommendation]"
...
class RewriteSegmenter(Segmenter):
    """v30 — v28 trailing category tag + v27q anti-speech-act rule."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V30,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
