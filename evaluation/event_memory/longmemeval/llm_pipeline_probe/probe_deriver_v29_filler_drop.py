"""v29 deriver — v28 + filler-drop rule.

Architectural insight from user: segmenter must KEEP short affirmatives
("yes", "no", "ok", "sounds good", "lol") because they're needed for
memory reconstruction at read time. But the DERIVER should DROP them
because they have no search-target value — a query for "yes" doesn't make
sense.

v29 changes from v28:
  - Add explicit FILLER rule: pure reactive filler with no concrete
    particulars → emit ZERO derivatives.
  - Relax "Emit at least one derivative per segment" → "Emit at least one
    derivative per non-filler segment".
  - Show borderline ("yes, I'll do that") in the rule so the model
    distinguishes content-bearing affirmatives from pure filler.
"""
...
plumbing that need the prior message to be meaningful — emit ZERO \
derivatives. Examples: \"yes\", \"no\", \"ok\", \"sounds good\", \"will \
do\", \"thanks\", \"lol\", \"great point\", \"omg yes!!\", \
\"acknowledged\". They carry no searchable content on their own. \
HOWEVER: if the segment combines an affirmative with concrete content \
(\"ok, leaving Tuesday at 5\", \"yes, I'll bring the wine\"), it is NOT \
pure filler — emit derivatives for the content. Otherwise emit at least \
one derivative per segment.
...
if __name__ == "__main__":
    print(
        f"v29 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
