"""v38b deriver -- principle preserved as first COUNTING bullet, unified format.

Tests: if dropping the principle (v38a) regresses, does keeping it as a
bullet rule recover the same performance as v37 while gaining the format
consistency win?

Changes from v37:
  - Header was: "How many derivatives? Count the distinct searchable things
    from a future user's perspective, NOT the number of sentences or named
    entities:"
    Now: "COUNTING:" + first bullet:
    "- Each derivative covers one distinct searchable thing -- NOT one
       sentence and NOT one named entity."
  - NON-PROSE section converted to bullet list
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v38b length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
