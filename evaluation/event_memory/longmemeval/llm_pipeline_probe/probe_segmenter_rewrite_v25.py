"""LLM rewriting segmenter v25 — v22 + canonical-category-noun rule.

v22 cat 1 (multi-hop) g3 failures are dominated by enumeration
questions ("how many tournaments has Nate won?", "what things has
Nate recommended?", "how many movies have Joanna and Nate watched?").
The answerer can only count what retrieval surfaces; at K=7 it sees
~7 instances but the true count may be higher. The segmenter helps
by giving same-class events a shared retrieval cue so they cluster
in vector space.

v25 adds one rule: when an event is one of a recurring class (a
recommendation, a tournament win, a meal, a hike, a purchase, a
visit, etc.), include the canonical CATEGORY NOUN as a leading
phrase in the statement so all events of the same class share a
retrieval cue.
"""
...
include the canonical CATEGORY NOUN explicitly in the statement so \
every event of the same class shares a retrieval cue. "Nate's \
tournament win on 2022-04-25: 2nd place at the regional" beats \
"Nate placed 2nd at the regional on 2022-04-25". "Nate's \
recommendation to Joanna: the dragon book series" beats "Nate told \
Joanna about a great dragon book series". Use a single, consistent \
category noun across all instances of the same class so that a \
future query for the class retrieves all instances together.
...
class RewriteSegmenter(Segmenter):
    """v25 — v22 + canonical-category-noun rule for enumeration cues."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V25,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
