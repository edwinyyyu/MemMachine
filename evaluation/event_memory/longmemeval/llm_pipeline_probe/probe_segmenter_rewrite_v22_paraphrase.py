"""v22-paraphrase -- v22 with memory + paraphrase in embedding text.

Hypothesis: two phrasings of the same content increase embedding \
robustness (different wordings cover different lexical neighborhoods) \
without query-form bias. Diff from qkey: both items are declarative \
...
PROMPT_REWRITE_V22_PARAPHRASE = """\
Rewrite the MESSAGE into a JSON list of memory items. Each item \
contains a third-person memory statement plus a paraphrase. A future \
user querying any specific content in the message should find at \
least one item whose memory OR paraphrase contains that content.
...
(B) "paraphrase": a different phrasing of the same memory (10-25 \
words). Use different word choice (synonyms, reordered clauses, \
nominalizations vs. verbalizations). Preserve all concrete particulars \
from (A); add no new content. The paraphrase MUST share fewer than \
half its content words with the memory.
...
{{ "memory": "...", "paraphrase": "..." }} objects. Use an empty list \
...
class RewriteSegmenter(Segmenter):
    """v22-paraphrase -- memory + paraphrase in embedding."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V22_PARAPHRASE,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
    ...
    def _build_embed_text(
        memory: str, paraphrase: str, original_chunk: str, speaker: str
    ) -> str:
