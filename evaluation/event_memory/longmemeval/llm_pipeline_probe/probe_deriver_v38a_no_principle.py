"""v38a deriver -- SHIPPED AS v38. Drops the "count distinct searchable
things..." principle line AND unifies the three sections to parallel
HEADER: + bullet-list format.

Empirical justification (3-pass sweep, 3 models x 2 reasoning x 10 cases):
  v37  (with principle, mixed format):  160/180 = 88.9%
  v38a (no principle, unified format):  161/180 = 89.4%
  v38b (principle as bullet, unified):  148/180 = 82.2%

Principle line was ceremony, not load-bearing. D1 atomization on
gpt-5.4-nano happens intermittently regardless of prompt (model
flakiness, not prompt-fixable). v38b's bullet-form principle regressed
B1 self-containment.

Changes from v37:
  - Header was: "How many derivatives? Count the distinct searchable
    things from a future user's perspective, NOT the number of sentences
    or named entities:"
    Now: "COUNTING:"
  - NON-PROSE section converted from semicolon-separated prose to bullet
    list
  - All three sections now: HEADER: + bullet list (parallel structure)
  - 3080 chars vs v37's 3200
"""
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v38a length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
