"""v52 deriver -- v51 with v45's "per-sentence derivatives" wording
restored. The co-mentioned particulars rule and the atomization
FAILURE rule become separate concerns (as in v45), not merged.

v51 4-pass: 56/60 every pass (Δ0 variance) but -3pp vs v45.
D1 Mustang failed all 4 passes -- removing "per-sentence" from the
FAILURE wording lost the specific cue gpt-5.4-nano needs to avoid
per-sentence atomization on long narratives.

v52 structure:
  - Two-sentence subject + open list (from v48-v51, addresses
    "subject repeats 3x" critique)
  - Co-mentioned particulars rule (separate sentence, positive: they're
    ATTRIBUTES of the topic). Does NOT try to causally connect to
    per-sentence atomization (which was v50's mistake).
  - Atomization FAILURE rule (separate sentence, specific wording from
    v45: "per-sentence derivatives" -- protects D1).

The three statements are co-located guidance for "ONE central topic ->
ONE derivative", not causally chained. User's question "what do
sentences have to do with co-mentioned particulars?" -- they're
separate concerns, addressed separately.
"""
...
ATTRIBUTES of that topic, not new topics. Atomizing a single-topic \
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v52 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
