"""LLM segmenter, Mode F-natural v31 -- add 'generic courtesies' anchor.

v30 trimmed rule 2 to just "bare greetings, sign-offs" as examples of
interchangeable framing. That left no anchor for stock courtesy
closers like "let me know if any questions" (not a greeting, not a
sign-off, but content-free), which survived on all 3 models.

v31 adds a third class: "generic courtesies". Still higher-abstraction
than listing the literal phrase, but anchors the model on the family
that includes "let me know if any questions", "happy to help",
"hope this helps", etc.
"""
...
(e.g., bare greetings, sign-offs, generic courtesies).
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V31.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V31.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
