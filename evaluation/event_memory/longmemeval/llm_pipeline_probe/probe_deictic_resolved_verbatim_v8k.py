"""Deictic-resolved verbatim segmenter -- v8k.

V8J's prompt + structured JSON output. Instead of asking the model
to regenerate the full edited message, we ask it to return a list of
{original, replacement} edit pairs. The segmenter applies each pair
to the ORIGINAL message string. The message text is never regenerated
by the model.

Properties guaranteed by this approach:
  - Content cannot be dropped (we never rewrite the message; we apply
    edits to it).
  - Format artifacts cannot appear (no "Joanna: Joanna:" doubling).
  - First-person voice can only be replaced if the model EXPLICITLY
    emits {"original": "I", "replacement": "Audrey"} -- visible and
    filterable.
  - Over-substitution-repetition requires the model to emit a pair
    per occurrence; the segmenter only replaces the first match.
  - Mid-sentence vocative requires the model to emit the wrong-position
    form explicitly; visible if it happens.

The model retains responsibility for the JUDGMENT calls (which edits
to propose); the structural risk is contained.

A minimal programmatic check drops only structurally-broken pairs:
  - Edit whose `original` is not present in the message.
  - Edit where original == replacement.
First-person preservation is handled by prompt only — adding a code
catch was premature; the JSON-edit framing plus the WHAT STAYS
UNCHANGED rule + an explicit OUTPUT FORMAT example are kept to do
the work. Add code-level filtering only if the prompt fails
empirically.
"""
...
class DeicticResolvedV8KSegmenter(Segmenter):
    def __init__(
        self,
        *,
        client: AsyncOpenAI,
        ...
        prompt_template: str = PROMPT_DEICTIC_RESOLVED_V8K,
        chunk_size: int = 1500,
    ) -> None:
        self._client = client
        self._model = model
