"""LLM rewriting segmenter v22 — v20 + explicit no-per-sentence-split anchor.

v20 over-fragments (1223 g3 segments vs v14 822). Inspecting v20
output shows the model splits an event's elaboration across
sentences ("subject + reason + outcome" → 3 statements). v22 keeps
v20's neighbor context and KEEP enumeration, but reframes the event
rule so the unit of granularity is the EVENT not the SENTENCE.

The rule names a concrete failure mode (per-sentence splitting) with
FAILURE framing, parallel to how v33 names dropping multi-line
