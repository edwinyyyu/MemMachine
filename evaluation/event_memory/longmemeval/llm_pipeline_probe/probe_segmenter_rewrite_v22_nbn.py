"""LLM rewriting segmenter v22-nbn — v22 minus all neighbor references.

Identical to v22 except the prompt has no NEIGHBORING TURNS section and
no `{neighbors_block}` placeholder. For clean "no-neighbors" ablation
paired with `--neighbor-window 0` / `--neighbor-direction none`.
"""
...
class RewriteSegmenter(Segmenter):
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_REWRITE_V22_NBN,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
