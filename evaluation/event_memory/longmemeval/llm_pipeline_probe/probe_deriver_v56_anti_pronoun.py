"""v56 deriver -- v55 with explicit anti-pronoun rule in self-containment.

User feedback: the actual failure mode in atomized derivatives isn't
that atomization is wrong -- it's that atomized siblings use pronouns
('it', 'they', 'we did X') in place of the central anchor, making each
derivative unsearchable for queries about the scope.

Examples of the failure:
  - "It took me about 8 months working evenings and weekends." [no
    anchor for 'clock restoration']
  - "We finished in the late afternoon and her dog wouldn't stop
    barking at it." [no anchor for 'treehouse']

The fix: explicit rule that pronouns referring to the central scope
must be replaced with the anchor noun in every sibling derivative.

v56 changes only EACH DERIVATIVE bullet 3 (self-containment):

  v55: "Stands alone as a search result. When sibling derivatives
       share a scope (a trip, project, period, conversation, artifact)
       and the central co-actors of that scope, EVERY sibling must
       include both -- otherwise a query mentioning the scope or a
       co-actor returns only the introductory sibling, not the ones
       describing what actually happened."

  v56: "Stands alone as a search result. EVERY sibling derivative must
       name the central scope (project, trip, artifact, person, etc.)
       and central co-actors explicitly in its own text -- NEVER
       substitute 'it', 'they', or 'we did X' for the searchable
       subject. A derivative like 'It took 3 years' or 'We finished at
       5' is invisible to queries for the scope: replace pronouns
       referring to the central scope with the explicit anchor noun
       in every sibling."
"""
...
the query-binding.
- Stands alone as a search result. EVERY sibling derivative must name \
the central scope (the project, trip, artifact, person, etc.) and \
central co-actors explicitly in its own text -- NEVER substitute "it", \
"they", or "we did X" for the searchable subject. A derivative like \
"It took 3 years" or "We finished at 5" is invisible to queries for \
the scope: replace pronouns referring to the central scope with the \
explicit anchor noun in every sibling.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v56 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
