"""v22-listfaith RewriteSegmenter -- production v22 prompt + LIST FAITHFULNESS rule.

Hypothesis
----------

The LIST FAITHFULNESS rule added to v22-fp-min v1->v2 lifted g4 cat1 by
+6.46pp empirically. The mechanism is model-level: LLMs default to
compressing enumerable items into vague phrases (``various``, ``several``,
a partial sample) regardless of voice (1p vs 3p) or prompt structure
(minimalist vs full v22).

Transfer hypothesis: adding the same rule, unconditional form, to the
full production v22 prompt should also lift cat1 retrieval, since v22's
``Preserves every CONCRETE PARTICULAR`` clause is general and does not
specifically forbid list-compression.

v22-listfaith
-------------

Identical to PROMPT_REWRITE_V22 except for one additional bullet inserted
into the EACH STATEMENT list (placed adjacent to the concrete-particular
preservation bullet, where the model is already attending to verbatim
preservation). No new examples; the rule alone is the intervention.

Everything else (RewriteContext + dual-text embed, SurroundingEventsContext,
KEEP/DROP enumeration, one-event-per-statement, speech-act-only-when-event,
date resolution discipline) is unchanged from production v22.
"""
...
- LIST FAITHFULNESS. When the raw message contains an enumeration \
(a list of three or more comparable items: names, books, places, \
foods, brands, hobbies, languages, dates, etc.), the statement MUST \
preserve EVERY listed item verbatim. Summaries that collapse the \
list into ``various``, ``several``, ``a few``, ``many``, ``some``, \
or a partial sample are FAILURES. If the list itself is the \
queryable content, the statement is a single enumeration -- not \
multiple statements. Two comma-separated phrases that describe the \
SAME event are not an enumeration; only three-or-more comparable \
items count.
...
class RewriteSegmenter(Segmenter):
    ...
    Identical to the production v22 RewriteSegmenter except the prompt
    adds a LIST FAITHFULNESS bullet to the EACH STATEMENT rules.
    ...
    Args:
        ...
        prompt_template: A ``.format(speaker=..., date=..., passage=...,
            ...
            Defaults to the v22-listfaith prompt.
        chunk_size: Maximum characters per deterministic chunk before
            calling the LLM. Defaults to 1500.
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_REWRITE_V22_LISTFAITH,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
