"""v10 deriver probe — v9 with 3 surgical fixes.

v9 (2849 chars) was too aggressive. Regressions vs v8:
  - O1: 0/5 wins (v8: 3/5) — fragments embed worse than full sentences
  - C2: lodging derivative was telegraph-style fragment
  - C4 markdown table: no artifact-description derivative
  - C6 code: kept code syntax (':' '(node)') instead of prose
  - F2 chess: fragment "chess move 25.g4 gaining space..."

v10 fixes the three lost lines:
  (a) preamble: "Each derivative is a full grammatical sentence"
  (b) rule 6: "ALWAYS emit at least one ARTIFACT-DESCRIPTION"
  (c) rule 6: "NEVER keep pipes, code syntax, or brackets — render in prose"

Target: ~3200 chars (still 36% shorter than v8, 68% shorter than v5).
"""
...
under that scope. Co-actors and per-clause attributes do not propagate.
  6. NON-PROSE SURFACES GET REPLACED WITH PROSE. For encoded text, code, \
tables, lists of raw numbers, log lines, or markup: decode recognizable \
encodings (cipher, base64); ALWAYS emit at least one \
ARTIFACT-DESCRIPTION derivative — a full sentence naming what the \
segment IS (e.g., \"Python function returning max value in a binary \
tree\", \"benchmark table comparing GPT-4, Claude, and Gemini on Task A \
and Task B\") and what it is ABOUT; for multi-row tables, optionally \
emit one prose derivative per row when each is independently \
answerable. NEVER keep pipes, code syntax, or brackets — render values \
in prose.
