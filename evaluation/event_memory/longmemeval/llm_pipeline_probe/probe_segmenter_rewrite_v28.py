"""LLM rewriting segmenter v28 — v22 + trailing category-noun tag.

v25 (category-noun in LEAD position) lifts cat 1 multi-hop +10.8pp on
g3 but loses cat 2 (-5.0pp) and cat 4 (-1.8pp): the lead-position
category-noun displaces the date/entity from the high-signal
embedding region. Net c124 is a wash.

v28 keeps v22's full statement structure (entity/date/particulars at
the lead) and APPENDS a canonical category noun as a trailing tag
when the event is one of a recurring class. The lead position
preserves cat 2/4 retrieval cues; the trailing category tag still
provides a shared BM25 and partial embedding signal for cat 1
enumeration queries.
"""
...
class RewriteSegmenter(Segmenter):
    """v28 — v22 + trailing category-noun tag for enumeration cues."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V28,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
