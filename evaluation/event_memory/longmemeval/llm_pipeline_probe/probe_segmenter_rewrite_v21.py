"""LLM rewriting segmenter v21 — v20 + fold one-event + completeness bullets.

v20 over-fragmented (1223 segs on g3 vs v14's 822). The model
interpreted "covers every concrete particular" as a per-particular
emission trigger rather than a per-event completeness constraint
against the immediately preceding "exactly one event" bullet.

v21 folds the two bullets into a single statement-scope rule that
treats event-unity and particular-completeness as one principle. The
FAILURE marker is restated on splitting (the actual failure mode
observed) rather than on dropping particulars (covered by KEEP).
"""
...
EACH STATEMENT:
- Describes exactly one event and contains EVERY concrete particular \
the message gives about that one event -- subject, action, time, \
place, attendees, motivation, outcome, attached media. An event is \
one occurrence, decision, plan, observation, state, or preference \
at one point or span in time. Distinct events (different times, \
different occasions, different actions) each get their own \
statement, even when they share participants or topics. Splitting \
one event's particulars across multiple statements is a FAILURE; \
each particular is an ATTRIBUTE of its event, not its own event. \
Merging two distinct events into one statement is a FAILURE.
...
class RewriteSegmenter(Segmenter):
    """v21 — v20 + tighter event-unity rule."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V21,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
