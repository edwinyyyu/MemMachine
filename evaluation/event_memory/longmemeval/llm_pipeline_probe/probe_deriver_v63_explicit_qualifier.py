"""v63 deriver -- SHIPPED. v60 with three user-requested clarity fixes.

4-pass sweep on expanded 13-case bench (78 calls/pass, 312 calls/prompt):
  v60: 74+75+76+76 = 301/312 = 96.5%
  v63: 75+73+75+74 = 297/312 = 95.2%

Within noise (-1.3pp). B1 binding (the key pronoun-related case)
stayed tied at 3 fails. FIL1 improved (3->1).

Three user-requested fixes addressed:
  (a) Removed "sibling" jargon -- "In every sibling derivative" ->
      "When you emit multiple derivatives for one segment, each must..."
  (b) Plural "derivatives" in consequence (grammatical correctness:
      a query matches multiple anchored derivatives, not just one)
  (c) Broader qualifier "for the scope or anyone/anything involved"
      replaces narrow "standing in for the central subject" -- covers
      pronoun substitutions for any of the listed items, not just the
      central one



v62 attempt regressed B1 (1 -> 4 fails) by changing "standing in for
the central subject" to vague "referring to them". The qualifier
needs to be EXPLICIT about what the pronouns are standing in for.

v63 changes from v60:
  (a) "In every sibling derivative" -> "When you emit multiple
      derivatives for one segment, each..." (removes "sibling" jargon)
  (b) "standing in for the central subject" -> "for the scope or
      anyone/anything involved" (broader qualifier, mirrors the listed
      items in the rule, replaces vague "referring to them")
  (c) "finds only the derivative where they were named" ->
      "finds only the derivatives where they were named" (plural;
      multiple anchored derivatives can match a query)
"""
...
as pronouns ("it", "they", "we") for the scope or anyone/anything \
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v63 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
