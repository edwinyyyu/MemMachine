"""v36 deriver -- trim filler examples + diversify identifying-relation examples.

User feedback on v35:
  1. Filler list had too many variants ("omg yes!!" ~ "yes", "acknowledged"
     ~ "ok", "sounds good" / "will do" ~ "ok"). Keep one example per
     distinct category.
  2. Identifying-relations examples were all people (mentor / team lead /
     neighbor). Add a place and a project to show the rule applies to
     any name + role binding, not just human roles.

Changes from v35:
  - Filler examples: "yes", "no", "ok", "sounds good", "will do",
    "thanks", "lol", "great point", "omg yes!!", "acknowledged"
    -> "yes", "no", "ok", "thanks", "lol", "great point" (6 examples
    covering: affirmation, negation, agreement, gratitude, humor, praise)
  - Identifying relations: "my mentor Alice", "team lead Bob",
    "neighbor Charlie"
    -> "my mentor Alice", "the cafe Bluebird", "the project Apollo"
...
- Keeps identifying relations atomic: "team lead Alice", "the \
engineering library on campus" -- name and relations appear TOGETHER, \
never split.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v36 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
