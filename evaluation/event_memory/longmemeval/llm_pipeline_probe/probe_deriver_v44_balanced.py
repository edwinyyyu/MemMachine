"""v44 deriver -- v43 with regression-causing trims restored + table
generalization per user request.

v43 (3001 chars) regressed ~3pp vs v41 (3495 chars):
  - P1 preference came back (1 fail) — was caused by removing the
    "Atomizing single-subject is a FAILURE." callout
  - B1 binding self-containment worse (3/3 passes) — was caused by
    compressing the query-side self-containment reasoning
  - FIL1 doubled (minor / variance)

v44 restores the two regression-causing trims and addresses user
feedback: generalize "multi-row tables" -> any non-prose aggregate with
multiple distinct entries (table rows, JSON array elements, log
batches).

Keeps v43's safe trims (encodings, exactly-ONE, particulars examples,
goal paragraph trim).

Target: match v41 robustness with smaller char budget than v41's 3495.
"""
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v44 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
