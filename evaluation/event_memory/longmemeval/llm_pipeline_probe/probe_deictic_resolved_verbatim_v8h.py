"""Deictic-resolved verbatim segmenter -- v8h.

V8G + explicit negative examples to counteract two failure modes
observed at nano@low:

  - Speaker-self-vocative: "Are you excited, Nate?" / "Hey, Nate, we
    should go together..." where Nate is the speaker. Negative
    example added to the WHAT EDITS TO MAKE "you" bullet showing the
    speaker's own name as a WRONG vocative.
  - First-person voice replaced with addressee/speaker name: V8G's
    D18:6 ("Audrey organized a playdate with Audrey's furry friends",
    where Audrey is the speaker), D28:3 ("Here's a pic of Audrey's
    fur kids" where Audrey is the speaker). Negative example added
    to the WHAT STAYS UNCHANGED first-person bullet showing first-
    person voice stays even when a name substitution would be
    grammatically plausible.

V8I is the alternative: prompt ends with "<speaker>: " to seed the
output context with the speaker label.
"""
...
- A "you" addressed to a specific person — the addressee, who is \
identifiable from the surrounding context and is almost never the \
speaker — gains a vocative with the addressee's name, placed at \
the start or end of the same sentence containing the "you". For \
example, if the speaker is Bob and the addressee is Alice, "Do \
you like pizza?" becomes "Do you like pizza, Alice?". The \
following are WRONG: "Do you like pizza, Bob?" (Bob is the \
speaker, not the addressee), "Do you, Alice, like pizza?" \
(vocative mid-sentence). The original "you" stays.
...
speaker. Do NOT replace first-person voice with a name. If the \
speaker is Audrey, "I organized a playdate with my friends" stays \
as "I organized a playdate with my friends", NOT "Audrey \
organized a playdate with Audrey's friends".
...
class DeicticResolvedV8HSegmenter(Segmenter):
    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_DEICTIC_RESOLVED_V8H,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
