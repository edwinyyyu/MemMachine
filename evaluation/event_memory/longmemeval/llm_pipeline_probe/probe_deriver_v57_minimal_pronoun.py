"""v57 deriver -- SHIPPED. v55 + minimal anti-pronoun parenthetical added
to existing self-containment rule.

4-pass sweep on expanded 13-case bench (78 calls/pass; 312 calls/prompt):
  v55: 75+77+75+74 = 301/312 = 96.5% (range Δ3)
  v57: 77+76+78+77 = 308/312 = 98.7% (range Δ2)

v57 ties or beats v55 on every case. The "(BY NAME, not as 'it',
'they', or 'we' standing in for the searchable subject)" parenthetical
addresses pronoun-substitution AND also improves B1 binding by ~50%
(the anti-pronoun signal generalized to "name co-actors explicitly").

Failure shift vs v55 (4 passes each):
  B1 binding:   4 -> 2 (improved by 50%)
  FIL1:         2 -> 0 (improved)
  PR3 apple:    1 -> 0 (pronoun fix worked)
  P1 pref:      1 -> 0 (improved)
  PR2 tree:     1 -> 1 (tied)
  D1 mustang:   1 -> 1 (tied)

v56 (full rewrite of self-containment) regressed P1/B2/C5 from rule
verbosity. v57's minimal parenthetical avoids that distraction while
still delivering the anti-pronoun signal.



v56 (full rewrite of self-containment rule): 0 pronoun-anchor failures
across 4 passes, but ~5 regressions elsewhere (B1, P1, B2, C5) from
rule verbosity and distraction.

v57 keeps v55's self-containment wording verbatim, only adds a brief
parenthetical "(names, not pronouns like 'it' or 'they')" inline to
the existing rule.

  v55: "EVERY sibling must include both -- otherwise a query
       mentioning the scope or a co-actor returns only the
       introductory sibling, not the ones describing what actually
       happened."

  v57: "EVERY sibling must include both BY NAME (not as 'it', 'they',
       or 'we' standing in for the searchable subject) -- otherwise a
       query mentioning the scope or a co-actor returns only the
       introductory sibling, not the ones describing what actually
       happened."

Adds ~80 chars. Surgical change targets pronoun-substitution without
adding new sentences or changing the rule's structure.
"""
...
central co-actors of that scope, EVERY sibling must include both BY \
NAME (not as "it", "they", or "we" standing in for the searchable \
subject) -- otherwise a query mentioning the scope or a co-actor \
returns only the introductory sibling, not the ones describing what \
actually happened.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v57 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
