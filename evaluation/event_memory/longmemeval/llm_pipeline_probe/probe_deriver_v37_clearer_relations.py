"""v37 deriver -- clearer "compound identifier" rule (same 2 examples).

User noted v36's "Keeps identifying relations atomic: 'team lead Alice',
'the engineering library on campus' -- name and relations appear
TOGETHER, never split" is unclear with just two raw phrases. The model
has to guess what the rule actually means from the examples.

v37 changes ONLY the relations bullet:
- Names the concept ("compound identifier")
- Defines its structure (name + role/qualifier/descriptor that
  disambiguates it from similar things)
- Explains the WHY (splitting loses the binding that makes future
  retrieval specific)
- Same two examples retained
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v37 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
