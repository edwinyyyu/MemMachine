"""LLM segmenter, Mode F-natural v45 -- rhetorical-unit framing.

Hypothesis: segments are rhetorical units, defined by RST-style
relations between sentences. Sentences in elaboration, evidence,
explanation, restatement, reaction, qualification, or sub-aspect
relation to one another form one unit. A new unit begins when the
relation is none of those -- a new claim, a new event, a new question.

This grounds segmentation in established discourse-structure theory.
Continuation relations are enumerated objectively; no outcome bias.
"""
...
their source quotes.
6. SEGMENT BY RHETORICAL UNIT. A segment is one rhetorical unit. \
Two consecutive sentences belong to the same rhetorical unit when \
the second stands in one of these relations to the first: \
elaboration, evidence, explanation, exemplification, restatement, \
reaction, intensification, qualification, contrast on the same \
subject, consequence, or sub-aspect. Two consecutive sentences \
belong to different rhetorical units when the relation is none of \
those -- the second introduces a different claim, a different \
event, a different question, or carries an explicit discourse marker \
of departure ("by the way", "anyway", "speaking of", "moving on").
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V45.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V45.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
