"""v22-nomsgdate RewriteSegmenter -- v22 baseline + drop redundant message date.

Hypothesis
----------

v22 baseline produces segments like:
  "Maria was busy volunteering at the homeless shelter on 2022-12-17."
where 2022-12-17 IS the message date. The framework's segment-context
renderer ALREADY prepends ``[<timestamp>] `` (the message date) to every
retrieved segment, so the inline ``on 2022-12-17`` is REDUNDANT.

The redundancy:
  - Wastes ~10-15 tokens per message-time segment (a substantial fraction
    of v22's 328t mean question budget).
  - Doesn't add retrieval signal: the framework header already supplies
    the message date for BM25 and embedding.
  - Risks date confusion when an answering model sees two timestamp-like
    strings (the header `[Mon, Dec 17, 2022, ...] ` plus the inline
    `on 2022-12-17`).

min3p and fp-min-v2 both already drop the redundant date and tie or
near-tie baseline accuracy at lower token counts. Applying the rule to
v22 baseline (which is otherwise a strong configuration) should
produce a TOKEN-PARETO improvement: same/similar accuracy at fewer
tokens.

Change vs v22 baseline
----------------------

ONLY the EVENT-DATE-ANCHORING rule is modified. v22 baseline says:
...
v22-nomsgdate replaces this with explicit DROP semantics:
    "When the event's resolved date EQUALS the message date {date}
    (the event coincides with the message-sending moment), the
    statement contains NO date and NO relative time phrase -- the
    framework's automatic [<timestamp>] prefix carries it. When the
    resolved date DIFFERS from {date}, weave it as ``on YYYY-MM-DD``
    in natural prose and DELETE the original relative phrase. One
    event date per statement."

Everything else (KEEP/DROP, anti-fragmentation FAILURE clauses,
RewriteContext + dual-text embed, neighbors block, WholeTextDeriver
pipeline) is unchanged from v22 baseline.

Expected behavior
-----------------

- Tokens-per-question mean: 280-300 (v22 baseline 328t -> 10-15% drop).
- c124 accuracy: ~baseline (89.13%) ± 1pp (token drop is mechanical;
  retrieval signal preserved via framework header).
- If accuracy holds: TOKEN-PARETO WIN over v22 baseline. Same accuracy
  at fewer tokens.

Anticipated failure modes
-------------------------

1. The "DROP date when equals message date" rule may also cause the
   model to drop the date when it DIFFERS from message date (failure
   to distinguish). Mitigation: explicit pair of conditional clauses
   (EQUALS -> drop; DIFFERS -> include) with a forbidden-form list.
2. Tokens may go down too much without proportional retrieval-quality
   benefit, hurting at marginal scale. Mitigation: this is a pure
   token-efficiency play; we measure both before deciding.
"""
...
- The framework prepends the MESSAGE timestamp {date} automatically \
to each statement when surfacing it for retrieval. Therefore: when \
the event's resolved date EQUALS the message date {date} (the event \
coincides with the message-sending moment), the statement contains \
NO date and NO relative time phrase -- the framework's automatic \
prefix carries the date. When the event's resolved date DIFFERS \
from {date}, weave it inline as ``on YYYY-MM-DD`` in natural prose \
and delete the original relative phrase. The redundant inline \
message date ``on {date}`` is a FAILURE; the framework prefix \
already shows it. One event date per statement; split multi-date \
messages into multiple statements.
...
class RewriteSegmenter(Segmenter):
    """v22 baseline + drop-redundant-message-date rule.

    Block.text and context architecture identical to v22 baseline:
        block.text = 3p_rewrite
        context = RewriteContext(text_to_embed=f"{rewrite}\\n{speaker}: {raw_chunk}")
    """
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_REWRITE_V22_NOMSGDATE,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
