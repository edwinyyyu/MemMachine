"""Deictic-resolved verbatim segmenter -- v8g.

V8E with a tightened vocative rule that explicitly distinguishes the
addressee from the speaker. V8E's remaining hard failures at
nano@low were both speaker-self-vocative (D6:3 "Nate!" inserted as
standalone vocative where speaker IS Nate; D11:16 "Hey, Nate, we
should go together..." where speaker IS Nate, addressee is Joanna).

Changes from V8E:
  - WHAT EDITS TO MAKE "you" bullet: explicit "the addressee is
    identifiable from the surrounding context and is never the
    speaker"; example now names BOTH speaker (Bob) and addressee
    (Alice) so the speaker/addressee distinction is anchored.
  - Strengthened placement: vocative goes at start or end of the
    SAME sentence containing the "you" — closes the loophole that let
    "Nate!" appear as a standalone sentence between others.
"""
...
class DeicticResolvedV8GSegmenter(Segmenter):
    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_DEICTIC_RESOLVED_V8G,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
