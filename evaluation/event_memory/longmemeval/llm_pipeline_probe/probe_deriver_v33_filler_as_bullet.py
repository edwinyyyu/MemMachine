"""v33 deriver — filler rule moved into the granularity bullet list.

v32 had the filler rule in the goal paragraph with "Emit at least one
derivative per segment — UNLESS the segment is PURE FILLER..." User
pointed out this is a granularity rule (how many?), not a goal statement.

v33 moves filler into the bullet list as the FIRST case (the 0-derivative
case). Each bullet answers the question "how many derivatives?" with a
quantity:

- PURE FILLER → 0 derivatives
- ONE topic → ONE derivative
- Several independent → one per
- Single focused → ONE near-original
- Bare term → ONE with just the term
- List of items → ONE with items inline

Removes the redundant "Emit at least one derivative per segment" from the
goal paragraph since the bullet list covers all cases.
"""
...
- PURE FILLER — content-free reactions or affirmations only meaningful \
with the prior message (\"yes\", \"no\", \"ok\", \"sounds good\", \"will \
do\", \"thanks\", \"lol\", \"great point\", \"omg yes!!\", \
\"acknowledged\") → ZERO derivatives. An affirmative that ALSO carries \
concrete content (\"ok, leaving Tuesday at 5\", \"yes, I'll bring the \
wine\") is NOT pure filler — emit derivatives for the content.
...
if __name__ == "__main__":
    print(
        f"v33 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
