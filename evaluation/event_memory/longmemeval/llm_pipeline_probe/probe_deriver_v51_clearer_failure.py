"""v51 deriver -- v50 with the muddled per-sentence/per-particular
phrase fixed.

User feedback on v50:
  "Co-mentioned particulars ... splitting them into per-particular or
  per-sentence derivatives is a FAILURE" doesn't make sense -- the
  co-mentioned particulars rule is about per-particular splitting;
  per-sentence is a different failure mode. Merging them in one phrase
  is confusing.

v51 separates the two concerns cleanly:
  - Co-mentioned particulars rule (about particulars-as-attributes):
    states the POSITIVE behavior (particulars belong in the topic's
    derivative). No per-X confusion.
  - Atomization failure: stated generically as "Atomizing a
    single-topic passage into multiple derivatives is a FAILURE."
    Covers both per-sentence and per-particular mechanisms without
    naming either one.
...
ATTRIBUTES of that topic and belong in its one derivative, not as \
separate derivatives. Atomizing a single-topic passage into multiple \
derivatives is a FAILURE.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v51 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
