"""v39a deriver -- rename COUNTING -> PER SEGMENT.

Tests whether parallel scoping headers ("PER SEGMENT" / "EACH DERIVATIVE")
read more clearly than the mechanical "COUNTING". The two sections
become structurally parallel:
  - PER SEGMENT:    decisions made for the whole segment (how many)
  - EACH DERIVATIVE: constraints on each derivative (form, scope, etc.)
  - NON-PROSE SURFACES: special-case override
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v39a length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
