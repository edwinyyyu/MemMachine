"""SIDECAR-QUOTE: SIDECAR + per-item verbatim source-span shown to the answerer.

Hypothesis
----------

Diagnosis of why raw-events beats SIDECAR for cat2: the answerer's
offset arithmetic ("last Friday" relative to a header date) is more
reliable when the line LOOKS like dialog -- speaker name + first-person
phrasing in conversational register -- than when the same relative
phrase appears inside a compressed third-person summary. SIDECAR's
``terse`` keeps the relative phrase verbatim but loses the conversational
scaffolding.

Past FP variants (probe_terse_decoupled_slim_v3_fp.py and v22_fp* family)
tried to fix this by REPLACING the third-person rewrite with first-person.
All regressed -- third-person LoCoMo queries lost lexical/semantic match
against first-person memories, and "I" became ambiguous across thousands
of segments.

This variant tests the augmentation direction NONE of the FP variants
covered: keep third-person ``memory``/``terse`` driving BOTH retrieval
channels (so query-side match is preserved), and additionally show the
answerer a verbatim source span -- preserving the conversational register
and the speaker's natural first-person voice as a per-item rendering.

The verbatim source turn is ALREADY present in SIDECAR's
``text_to_embed`` channel (``{speaker}: {chunk_stripped}`` is appended to
every item's embed text). What this variant adds is making a per-item
slice of that source visible to the answerer too. Asking the LLM to
extract a span keeps each item's quote targeted (not the full chunk
repeated N times for N items) and keeps the token budget close to plain
SIDECAR.
...
A single new field per item:
  ...
  - ``quote``: the shortest contiguous span from the source MESSAGE that
    establishes this item -- a clause or short sentence in the speaker's
    own words. Verbatim (no paraphrasing). One per item.
...
------------------
  ...
  - vs SIDECAR: ONLY the answerer sees more; both retrieval channels are
    byte-identical. Any delta is answerer-side (offset reasoning,
    transcript framing).
  - vs FP-replace variants: retrieval channels remain 3p; we are NOT
    testing 1p-at-storage again.
  - vs rawev ceiling: rawev shows the FULL message turn per event;
    SIDECAR-quote shows a per-ITEM span. Closer in spirit but cheaper.
"""
...
(C) "quote" -- the SHORTEST contiguous span from the MESSAGE that \
establishes THIS item, copied VERBATIM in {speaker}'s own words. A \
clause or short sentence; NEVER paraphrase, NEVER summarize, NEVER \
substitute words. Keep the speaker's own pronouns ("I", "my", "you", \
"we"). Keep the speaker's relative time phrases as written. If two \
items come from the same message, each item's quote should focus on its \
own topic span (different parts of the message). Quote nothing the \
speaker did not write.
...
only; never emit items for them, and never draw quotes from them.

Return JSON: {{"items": [{{"memory": "...", "terse": "...", \
"quote": "...", "date_resolutions": [...], "queries": ["...", "..."]}}, \
...
class SidecarQuoteSegmenter(Segmenter):
    """SIDECAR with a per-item verbatim source-span shown to the answerer.

    block.text = ``{terse}\n> {speaker}: "{quote}"``

    Retrieval channels match base SIDECAR byte-for-byte, so any delta
    is answerer-side.
    """
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_SIDECAR_QUOTE,
        chunk_size: int = 1500,
        max_attempts: int = 3,
                ...
                case TextBlock(text=text):
                    ...
                    for chunk in chunks:
                        ...
                        for item in items:
                            ...
                            quote = item.quote.strip() if item.quote else ""
                            # block.text shows ONLY the verbatim quote when
                            # available; terse is used for retrieval only.
                            # Quote replaces terse so the answerer sees a
                            # single conversational rendering, not two views
                            # of the same item (the dual rendering hurt c2
                            # in v1 -- two surfaces of "last Friday"
                            # confused offset reasoning).
                            if quote:
                                display = f'{speaker}: "{quote}"'
                            else:
                                display = terse
