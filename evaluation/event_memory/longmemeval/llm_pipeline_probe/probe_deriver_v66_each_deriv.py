"""v66 deriver -- v65 with completeness rule MOVED from DERIVATIVES
COVERAGE to EACH DERIVATIVE.

User feedback on v65: 'why isn't the new rule under "EACH DERIVATIVE"?'

The completeness rule is about a single derivative's CONTENT (what
specific details it includes), not about derivative COUNT (how many to
emit). Logically it belongs in the EACH DERIVATIVE section alongside
"preserve concrete particulars" -- not in DERIVATIVES COVERAGE.

v66 changes:
  - DERIVATIVES COVERAGE bullet for ONE central subject: reverted to
    v64 (without the completeness clause).
  - EACH DERIVATIVE bullet 1 (concrete particulars): extended with the
    completeness clarification.

v66 EACH DERIVATIVE bullet 1:
  "Is a full grammatical sentence (except bare-entity), using the
  segment's wording verbatim for CONCRETE PARTICULARS -- names,
  places, dates, numbers, identifiers, decisions, plans, preferences,
  opinions, relationships, emotional states tied to events,
  constraints, and distinctive phrasing. When a single derivative
  covers a multi-sentence passage, include every concrete particular
  from the passage -- not just the first sentence, not a generic
  summary. Drop generic abstractions or stock phrases."

If v66 maintains v65's 0/40 content-drop fix at gpt-5-nano @ low, the
placement change is win-win (cleaner organization + same robustness).
"""
...
distinctive phrasing. When a single derivative covers a multi-sentence \
passage, include every concrete particular from the passage -- not \
just the first sentence, not a generic summary. Drop generic \
abstractions or stock phrases.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v66 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
