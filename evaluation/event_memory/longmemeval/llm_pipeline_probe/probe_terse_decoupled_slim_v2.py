"""Slimmed terse-decoupled segmenter v2 -- redundancy-removed + granularity fix.

Identical architecture and date-alias logic to terse-decoupled-v2; only
the prompt changes. v2's prompt had accreted to ~75 lines (the
"particular" list 3x, the retrieval objective 3x, a 19-line date section
with two redundant enumerations, "FAILURE" 7x).

slim_v1 removed that redundancy but ALSO dropped v2's anti-fragmentation
guardrail ("ONE item per EVENT, not per sentence and not per
particular") -- and over-segmented badly: ~9.1k-11k segments across
models vs v2's 5.2k (one item per *clause*; see the segmentation diff in
the session notes). slim_v2 restores the anti-fragmentation rule as an
objective principle: the unit is a TOPIC, not a sentence or a particular
-- every sentence and detail about one topic stays in one item.

Load-bearing behaviour preserved: keep every particular; ONE item per
topic (never split by sentence/particular); drop interchangeable filler;
...
message's own date; terse = readable prose, not fragments; 1-3 queries.
...
# Slimmed prompt v2: redundancy-removed + objective anti-fragmentation
...
class TerseDecoupledSegmenter(Segmenter):
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_SLIM_V2,
        chunk_size: int = 1500,
        max_attempts: int = 3,
