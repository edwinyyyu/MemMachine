"""bo-natural with autoregressive date-review fields.

Bo-natural's prompt instructs the LLM to resolve relative time phrases
("last week", "the other day", "two weekends after") to absolute dates,
but inspection shows ~15% of segments still contain unresolved phrases.
The LLM ignores the resolve instruction even with WRONG/RIGHT examples
(see temporal variant — failed -0.97pp).

This variant uses a **schema-level** fix: the Pydantic field order forces
the LLM to enumerate relative phrases and their resolutions BEFORE
writing memory. Field order is JSON-schema property order is
autoregressive generation order — the LLM literally cannot skip the
resolution step. By the time it writes `memory`, the resolved dates are
already in context.

Schema (note field order):
  - relative_phrases: list[str] — exact phrases from source ("last week", ...)
  - date_resolutions: list[str] — absolute date per phrase, in same order
  - memory: str (3p, with resolved dates woven in)
  - terse: str
  - queries: list[str]

Empty lists mean no relative phrases in source. The terse/memory/queries
fields are unchanged from bo-natural in semantics.

Defaults match bo-natural: BM25-only date aliases, natural date format.
"""
...
class _MemoryItem(BaseModel):
    # NOTE: field order matters — Pydantic preserves it, JSON schema does
    # too, and the LLM generates in this exact order. The review fields
    # come first so the LLM resolves dates BEFORE writing memory.
...
class ReviewSegmenter(Segmenter):
    """bo-natural variant with autoregressive date-review schema fields."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REVIEW_NATURAL,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
