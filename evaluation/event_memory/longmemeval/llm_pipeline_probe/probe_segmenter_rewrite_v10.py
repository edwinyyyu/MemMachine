"""LLM rewriting segmenter v10 — same-event aggregation, not same-subject.

v8 disagreement diagnosis vs v7 at l160 showed v8 wins on resolved
relative-time (rule 5/6 fires more often when one statement carries
the resolved date) but loses on *same-subject conflation*: when a
chunk mentions "Calvin & Frank Ocean" in two separate events (Aug
2022 collab + Aug 2023 festival), v8's rule 7 merges them into one
statement and the answerer picks one date.

v10 narrows rule 7: aggregate facts about the same EVENT, INCIDENT,
or SINGLE-NARRATIVE within the chunk, not same SUBJECT-ENTITY. A
person/place/object referenced across separate events stays as
separate statements per event. This keeps v8's time-resolution wins
while avoiding cross-event conflation.
...
7. AGGREGATE FACTS WITHIN A SINGLE EVENT. When multiple sentences \
describe the SAME event, incident, occasion, or continuous narrative \
beat (one trip, one promotion, one project milestone, one \
conversation thread on one topic), combine their facts into ONE rich \
statement. But if the same person, place, or thing appears in TWO \
SEPARATE events (two different trips, a past meeting and a future \
plan involving the same friend, two collaborations spanning \
different years), emit ONE statement PER event -- not one statement \
combining them. Conflating separate events of the same subject is a \
FAILURE: the answerer will pick one date and lose the other.
   Examples of correct EVENT aggregation:
   - 3 sentences about "our camping trip last weekend" -> ONE \
statement.
   - "I got promoted to assistant manager at Shopify last week after \
two years" -> ONE statement.
   Examples of correct EVENT separation:
   - "Calvin met Frank Ocean at a festival in 2022 and they're \
recording together in 2023" -> TWO statements (2022 meeting + 2023 \
recording).
   - "I went to Bali last year and I'm planning to go again next \
month" -> TWO statements (past trip + future plan).
...
fact that it was said. Bare questions that solicit information \
without carrying any of their own ("{speaker} asked Mel who supports \
her", "{speaker} asked the other party about their day") produce no \
...
Street?"), extract those facts as a statement.

10. DROP PURE FILLER. Greetings ("Hi", "Hey", "How are you"), \
sign-offs ("Bye", "Take care", "Talk soon"), bare reassurances ("I'm \
here for you", "You got this"), and bare acknowledgments ("Sounds \
good", "Thanks", "Got it", "Yeah", "Cool") that carry no specifics \
produce no statement. A message whose every part is interchangeable \
filler -- no name, date, number, decision, plan, preference, \
opinion, event, hobby, relationship detail, or attached media \
description -- emits an empty list.
...
class RewriteSegmenter(Segmenter):
    """v10 — event-scoped aggregation with RewriteContext dual embed."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V10,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
