"""bo-natural with strengthened temporal resolution examples.

Adds explicit WRONG/RIGHT examples for the failure modes observed in
bo-natural's mini-classic eval:
  - "last week", "the other day" left unresolved.
  - "two weekends after [date]" partially resolved with ISO format
    instead of natural language.

The natural-date block is otherwise identical to bo-natural. Schema is
identical. Defaults match bo-natural (BM25-only date aliases, natural
date handling).

Kept independent from probe_terse_decoupled_slim_v3.py so bo-natural is
preserved unchanged.
"""
...
# Same as bo-natural's PROMPT_SLIM_V3_NATURAL_DATES, with WRONG/RIGHT
# examples appended to the date block. The examples target the specific
# failure modes observed: unresolved relative phrases, ISO format in chat.
PROMPT_TEMPORAL_NATURAL = """\
...
"two weekends after", "next month", "a while ago", "earlier this \
week", "the day before" -- to an absolute date anchored at {date}.
...
Examples — these are the failure modes to avoid:
  WRONG (left unresolved): "James adopted a pup last week."
  RIGHT (resolved + natural format): "James adopted a pup in early \
April 2022."

  WRONG (left unresolved): "Andrew adopted another dog the other day."
  RIGHT (resolved): "Andrew adopted another dog around November 18, \
2023."

  WRONG (partial resolution with ISO): "Nate is organizing a gaming \
party two weekends after 2022-06-03."
  RIGHT (full resolution + natural format): "Nate is organizing a \
gaming party the weekend of June 17-18, 2022."

  WRONG (ISO in chat context): "James adopted a pup on 2022-04-08."
  RIGHT (natural for chat): "James adopted a pup on April 8, 2022."
...
class TemporalNaturalSegmenter(Segmenter):
    """3p bo-natural variant with strengthened temporal-resolution examples."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_TEMPORAL_NATURAL,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
                ...
                case TextBlock(text=text):
                    ...
                    for chunk in chunks:
                        ...
                        for item in items:
                            ...
                            # bo-natural composition: BM25-only date aliases.
