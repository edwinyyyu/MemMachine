"""LLM rewriting segmenter v12 — hyper-aggregated per-message statements.

Hypothesis: Mem0's 91.3% at top-10 / 382 tok comes from cross-message
add-only summaries that pack many facts per memory. A single-event
segmenter can't match cross-message aggregation, but it CAN maximize
density per message. v12 pushes rule 7 to its limit: produce 1
statement per message by default, splitting only when the message
covers two truly unrelated subjects.

Each statement becomes a complete "scene": who, what, when, where,
why, with whom, with what details. The answerer LLM then has the full
context within one retrieval slot.

Architecture identical to v7: RewriteContext dual-surface embed,
deterministic 1500-char split. The prompt change is rule 7 going from
v8's "merge same-subject" → v12's "1 statement per message".
"""
...
7. ONE STATEMENT PER MESSAGE BY DEFAULT. Pack every concrete \
specific from the message into ONE rich statement, no matter how \
many sentences or how many facts. The statement is a complete \
"scene" answering who, what, when, where, why, and with what \
details. Only emit a SECOND statement when the message clearly \
covers two unrelated subjects (e.g., the message talks about a job \
promotion AND then switches to discussing a pet -- two statements). \
A single subject's many facets (a trip and its food, sights, \
people, emotions) stay in ONE statement. A single decision and its \
reasoning stay in ONE statement. A single event with multiple \
attendees, motivations, and outcomes stays in ONE statement. \
Emitting multiple statements about the same event/subject is a \
FAILURE: it scatters the answer across retrieval slots.
...
her", "{speaker} asked about their day") produce no statement. If \
the message contains incidental personal facts inside a question or \
request ("Have you tried the new bakery on 4th Street?"), extract \
those facts into the message's statement.

10. DROP PURE FILLER. Greetings ("Hi", "Hey", "How are you"), \
sign-offs ("Bye", "Take care", "Talk soon"), bare reassurances \
("I'm here for you", "You got this"), and bare acknowledgments \
("Sounds good", "Thanks", "Got it", "Yeah", "Cool") that carry no \
...
list of strings. Most messages produce 1 statement. Empty list if \
the message has no extractable specifics.
...
class RewriteSegmenter(Segmenter):
    """v12 — hyper-aggregation: 1 statement per message by default."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V12,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
