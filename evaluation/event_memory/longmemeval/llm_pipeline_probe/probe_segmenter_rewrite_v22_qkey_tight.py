"""v22-qkey-tight -- v22-qkey with ONE compact query per memory.

What this iterates from v22-qkey
--------------------------------

v22-qkey appended 1-3 hypothetical queries to text_to_embed per memory.
At K=7 it scored 88.31% c1234 @ 375t — TIE on c1234 vs v22 baseline
(88.38% @ 350t) with cat3 +4.2pp BUT 25 tokens over budget. The
overhead pushed retrieval toward longer events.

Hypothesis: the query overhead matters. Restricting to ONE short query
phrase (5-10 words, not a full sentence) per memory should preserve
the cat3 gain while keeping retrieval anchored to v22's compact
statement form.

Architecture
------------

Same as v22-qkey but the prompt asks for exactly ONE short query per
memory, phrased as a query (not a question with answer-content).
text_to_embed becomes:
...
QUERY: For each statement, also produce ONE short query (a query \
phrase of 3-10 words, NOT a full sentence) that a future user might \
use to retrieve this statement. The query should capture the \
SINGLE most queryable angle on the statement (a who/what question \
phrasing, or a noun phrase naming the queried particular). The \
query MUST NOT contain the answer it is asking about. Queries that \
exactly restate the statement are a FAILURE; queries longer than 10 \
words are a FAILURE; multiple queries are a FAILURE.
...
{{ "memory": "<the statement>", "query": "<the short query>" }} \
...
class RewriteSegmenter(Segmenter):
    """v22-qkey-tight -- v22 + a single short query per memory."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V22_QKEY_TIGHT,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
    ...
    def _build_embed_text(
        memory: str, query: str, original_chunk: str, speaker: str
    ) -> str:
