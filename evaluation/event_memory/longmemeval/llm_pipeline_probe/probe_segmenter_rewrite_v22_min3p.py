"""v22-min3p RewriteSegmenter — minimalist THIRD-person rewrite prompt.

Hypothesis
----------

v22-fp-min (1p, minimal ~918-token prompt, ProducerContext) won g3 c124
at 87.77% (+1.60pp vs v22 baseline 86.17%). v22-fp v1 (1p, FULL prompt,
ProducerContext) regressed to 81.91% (-4.26pp). The minimal prompt also
produced ~1.10 segments per turn vs v22-fp v1's 1.57 -- fewer-but-
tighter segments.

Open question: did the win come from MINIMIZATION (shorter prompt →
tighter segments → better retrieval) or from FIRST-PERSON (1p prose
embeds better)?

This variant isolates the question: keep the min variant's STRUCTURE
(same length, same KEEP/DROP, same one-event-per-statement, same date
discipline, same forbidden-format list, same neighbors-as-context rule,
same 5-example shape, same ProducerContext class wrapper) and flip
ONLY the person rule from 1p to 3p.

  - if min-3p beats v22 baseline (86.17%): the win is from minimization
  - if min-3p ties or loses to v22 baseline: first-person contributes
    (likely via cat3 inferential phrasing that benefits from 1p prose)

The most likely failure mode is that 3p output regresses the cat3
inferential gain that 1p prose was the source of -- 3p "Alice was
eating tonkotsu and found it incredible" embeds differently from 1p
"I am eating tonkotsu and find it incredible" against query phrasings
that mirror the asker's introspective frame.
"""
...
class RewriteSegmenter(Segmenter):
    """Third-person minimalist rewrite segmenter.
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_REWRITE_V22_MIN3P,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
