"""v34 deriver — generalize filler wording to cover negations.

User noted v33's "An affirmative that ALSO carries concrete content" excludes
negations. "no, I changed my mind about Tuesday" should emit content but
falls outside "affirmative".

v34 changes:
  - "content-free reactions or affirmations" → "content-free short responses"
    (covers affirmations + negations + exclamations + acknowledgments)
  - "An affirmative that ALSO carries..." → "When such a response ALSO
    carries..." (refers back to the filler category, not just affirmations)
  - Add a negation example: "no, I changed my mind about Tuesday"
"""
...
\"acknowledged\") → ZERO derivatives. When such a response ALSO carries \
concrete content (\"ok, leaving Tuesday at 5\", \"no, I changed my mind \
about Tuesday\"), it is NOT pure filler — emit derivatives for the \
content.
...
Bob\", \"neighbor Charlie\" — name and role appear TOGETHER, never split.
...
if __name__ == "__main__":
    print(
        f"v34 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
