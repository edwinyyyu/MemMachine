"""LLM segmenter, Mode F-natural v3 (iteration artifact).

Change: F1 fix v2: referent-borrow elaboration — REGRESSED, rolled back

This file preserves the v3 prompt body as drafted in this session, per the
...
Rules:
  ...
  2. DROP generic scaffolding by simply not including it in any segment: "Hi!", "What a great question!", "I hope this helps!", "Let me know if you have any other questions", a polite restatement of what the other party just asked, AND reactive filler ("I love that!", "great point", "I love your reasoning") that echoes the prior turn without adding new specifics. Bare "I love/hate [X]!" or "[X] is great/awful!" is reactive filler when [X] was just named by another party and the speaker offers no new fact, evidence, or commitment beyond the reaction. KEEP opinion statements that bring their own subject or elaboration ("I love jazz" followed by examples; "I think Sleep is overrated"; "I hated the dentist visit. They charged $400.").
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V3.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V3.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
