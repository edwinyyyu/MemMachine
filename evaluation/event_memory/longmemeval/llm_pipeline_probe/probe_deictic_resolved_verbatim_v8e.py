"""Deictic-resolved verbatim segmenter -- v8e.

V8D with the "already resolved" eligibility bullet reworded to be
atemporal / single-pass: rather than asking the model to check
whether it has already resolved an earlier occurrence (which assumes
working memory across autoregressive steps), the rule describes the
output property as a static constraint — "at most one resolution per
...
class DeicticResolvedV8ESegmenter(Segmenter):
    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_DEICTIC_RESOLVED_V8E,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
