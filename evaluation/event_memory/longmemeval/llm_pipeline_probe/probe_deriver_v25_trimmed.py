"""v25 deriver — v22 trimmed of redundant hedges and combined bullets.

Changes from v22 (no rule changes, just wording):
  - Drop "(sometimes 2-3 if too rich for one sentence)" hedge — model exercises
    judgment without it.
  - Drop "Per-clause mentions (a passerby at one event, a brand in only one
    clause) stay where they are" — implicit from "central to that scope".
  - Combine "full grammatical sentence" + "verbatim concrete particulars"
    into one bullet — they describe the same property.
  - Simplify self-containment to one sentence (no "(the trip, project,
    period, person)" enumeration in body since it appears just above).
  - Slightly trim non-prose section.

Goal: ~10% shorter at same qualitative pass rate.
"""
...
if __name__ == "__main__":
    print(
        f"v25 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
