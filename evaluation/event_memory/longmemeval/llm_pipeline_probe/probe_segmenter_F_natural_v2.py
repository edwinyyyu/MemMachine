"""LLM segmenter, Mode F-natural v2 (iteration artifact).

Change: F1 fix: reactive filler clause (referent-less examples)

This file preserves the v2 prompt body as drafted in this session, per the
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V2.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V2.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
