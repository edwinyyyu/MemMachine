...
# REFRAMING A -- "absolute date specifier": enumerate valid date forms and
# name the bad class (deictic). Defines what the output must NOT contain
# rather than describing the substitution procedure. No EQUALS rule
# (which masked LLM math errors by silently dropping the date entirely
# when the LLM incorrectly resolved to {date}).
_DATE_BLOCK_SPECIFIER = """Dates in the statement: every date in the statement must be an ABSOLUTE CALENDAR SPECIFIER -- either a year alone ("2024"), a year + month ("March 2024"), or a year + month + day ("March 15, 2024"). NEVER write a DEICTIC time word -- words like "yesterday", "last week", "three years ago", "next Friday", "today", "recently", "now", "just", "the other day", "this morning". Deictic words depend on when the speaker wrote, and the reader of this statement does not have that context.

For each deictic word in the source, compute the absolute calendar specifier using {date} as the anchor. The message's own date ({date}) is attached automatically when this memory is surfaced, so the statement text itself must never contain {date}. If the speaker referred to {date} itself ("today", "now") drop the deictic without replacement -- the attached date already conveys it. For any other deictic, write the absolute specifier in the prose at the precision the speaker stated. Match the source register: ISO ("2024-03-15") only if the source itself uses ISO; natural ("on March 15, 2024", "in March 2024", "in 2024") for chat or prose. Never write a date as a bracketed, parenthetical, or sentence-prefixed tag."""

# REFRAMING B -- "standalone-readable": frames the requirement as a
# property of the output ("readable without knowing when it was
# written") rather than a procedure. Engages perspective-taking.
_DATE_BLOCK_STANDALONE = """Dates in the statement: this statement must be READABLE WITHOUT KNOWING WHEN IT WAS WRITTEN. Imagine a reader opening this statement ten years from now, cold: they should know what date each event refers to, from the text alone.

Words like "yesterday", "last week", "three years ago", "next Friday", "today", "recently", "now", "just", "the other day", "this morning" are NOT readable standalone -- they require knowing when-this-was-written. Replace every such word with the absolute calendar date it refers to, anchored at {date} (the actual write date, attached automatically when surfaced). The statement text itself must never contain {date}; if the deictic referred to {date} itself, drop it (the attached date conveys it). Use natural format ("March 15, 2024", "March 2024", "2024") for chat/prose; ISO ("2024-03-15") only if source is ISO. Match the precision the speaker stated. Never write a date as a bracketed/parenthetical tag."""

# REFRAMING C -- "journalistic convention": leans on the LLM's familiarity
# with newspaper writing. Journalists never write "yesterday" without
# also giving the date.
_DATE_BLOCK_JOURNAL = """Dates in the statement: write dates using a journalist's convention. A journalist never writes "yesterday" or "last week" or "three years ago" without also giving the calendar date -- a newspaper reader has no way to know when the article was written, so vague time references aren't allowed.

Apply the same rule: replace every relative time word ("yesterday", "last week", "three years ago", "next Friday", "today", "recently", "now", "just", "the other day", "this morning", etc.) with the absolute calendar date it refers to, anchored at {date} (the message date, attached automatically when surfaced). The statement text itself must never contain {date}; if the relative word referred to {date} itself, drop it (the attached date covers it). Use natural format ("March 15, 2024", "March 2024", "2024") for chat/prose; ISO ("2024-03-15") only if the source is ISO. Match the precision the speaker stated. Never write a date as a bracketed/parenthetical/sentence-prefixed tag."""

PROMPT_SLIM_V3_SPECIFIER_DATES = PROMPT_SLIM_V3.replace(
    _DATE_BLOCK_RESOLVE, _DATE_BLOCK_SPECIFIER
)
PROMPT_SLIM_V3_STANDALONE_DATES = PROMPT_SLIM_V3.replace(
    _DATE_BLOCK_RESOLVE, _DATE_BLOCK_STANDALONE
)
PROMPT_SLIM_V3_JOURNAL_DATES = PROMPT_SLIM_V3.replace(
    _DATE_BLOCK_RESOLVE, _DATE_BLOCK_JOURNAL
)
for label, candidate, marker in [
    ("specifier", PROMPT_SLIM_V3_SPECIFIER_DATES, _DATE_BLOCK_SPECIFIER),
    ("standalone", PROMPT_SLIM_V3_STANDALONE_DATES, _DATE_BLOCK_STANDALONE),
    ("journal", PROMPT_SLIM_V3_JOURNAL_DATES, _DATE_BLOCK_JOURNAL),
]:
    assert marker in candidate, f"Date-block swap failed for {label}"
...
class TerseDecoupledSegmenter(Segmenter):
    ...
    def __init__(
        ...
        *,
        ...
        if date_handling not in _ALLOWED:
            raise ValueError(f"date_handling={date_handling!r} not in {_ALLOWED}")
        ...
        elif prompt_template is PROMPT_SLIM_V3 and date_handling == "specifier":
            prompt_template = PROMPT_SLIM_V3_SPECIFIER_DATES
        elif prompt_template is PROMPT_SLIM_V3 and date_handling == "standalone":
            prompt_template = PROMPT_SLIM_V3_STANDALONE_DATES
        elif prompt_template is PROMPT_SLIM_V3 and date_handling == "journal":
            prompt_template = PROMPT_SLIM_V3_JOURNAL_DATES
