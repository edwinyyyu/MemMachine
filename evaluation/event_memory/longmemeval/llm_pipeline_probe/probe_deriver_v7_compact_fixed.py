"""v7 compact deriver probe — v6 with targeted regression fixes.

v6 (3864 chars) regressed against v5 on:
  - C1: "The speaker went to Tokyo..." wrapper + I->their first-person flip
  - F3 Mario: 4 derivs (v5 did 1-2) — atomized one-subject prose
  - D1 Mustang: 9 derivs (v5 did 1-3) — worst single-subject atomization
  - FOCUS-F2 git commit: split focused multi-clause into 2 derivs
  - O1: 2/5 reps win vs v5's 4/5 — scope-propagation weakened

The compressed rule-3 lost two load-bearing lines from v5's P8/P9/P10:
  (a) explicit `"X is P. X is Q. X has R."` collapse instruction
  (b) explicit ban on `"the speaker said X"` / first-person rewrites
  (c) explicit `P10 does NOT override P8/P9` deference clause

v7 surgically restores those three without re-bloating the prompt.
Target: ~4400 chars (still ~55% shorter than v5's 9937).
"""
...
\"actually,\"), self-attribution wrappers (\"I said that...\"), trailing \
meta-comments; do NOT paraphrase the predicate, re-order the clause, add \
\"the speaker said X\" / \"the user said X\" wrappers, or split the \
single topic into multiple derivatives. For a bare entity or term, emit \
just that term unmodified.

5. NO NEAR-CLONES. Drop any derivative that shares the same predicate \
and key entities with another. Items differing only by an \
identifier+value (versions of one model, quarters of one year, rows of \
one set) collapse into ONE derivative naming the parent and listing the \
items inline; per-item derivatives are warranted only when each carries \
its own distinct cluster of facts.
...
\"server error log line\") and what it is ABOUT, listing the entities or \
labels it covers; (c) for multi-row tables/lists, you may also emit one \
prose derivative per row when each row is itself an answerable fact; \
(d) render values in prose — never keep pipes, code syntax, or brackets.
...
CRITICAL_CASES: list[tuple[str, str, str]] = [
    (
        ...
        "1 derivative close to original; preserves Tokyo, Anne, March, first-person.",
    ),
    (
        ...
        "~3 derivatives: Tokyo trip with Anne; Park Hyatt $400/5nt in Tokyo; ramen at Ichiran/Shibuya in Tokyo. Tokyo SHOULD propagate.",
    ),
    (
        "C3 caesar cipher",
...
async def main() -> None:
    ...
    print("# DERIVER v7 COMPACT-FIXED PROBE")
