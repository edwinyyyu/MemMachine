"""DRAFT: autoregressive scaffold -- verbatim_excerpt anchors before memory/terse.

The user's insight (Q2): "memory then terse did better than just terse"
-- the second step writes better when it has a source-faithful draft
already in its autoregressive context. Extending that pattern by adding
ANOTHER upstream step:

  source -> verbatim_excerpt -> memory (resolved) -> terse (compressed)

The LLM writes a source-faithful third-person draft FIRST, which keeps
every particular and the speaker's natural phrasing (including relative
phrases). Then memory transforms it: resolves relatives to absolute
dates and tightens prose. Then terse compresses. The verbatim_excerpt
field is pure scaffolding -- it never enters retrieval or display, only
the autoregressive context that the LLM uses to write memory + terse.

Memory + terse are RESOLVED (like bo-natural). The only change vs
bo-natural is the extra upstream verbatim field.

Schema (autoregressive order matters):
  - verbatim_excerpt: third-person source-faithful draft (relatives kept)
  - memory: resolved natural prose (like bo-natural)
  - terse: compressed resolved prose
...
Each item has four fields, written in this exact order. Earlier fields \
...
as close to the speaker's actual wording as third-person grammar allows. \
Replace "I"/"my" with {speaker}, "you" with the addressee, and \
this/that/there with what they refer to -- but keep every other choice \
of word, every relative time phrase ("yesterday", "last week", "next \
Friday", "the other day", "recently"), and every stated detail exactly \
as the speaker put it. This field is your source-faithful anchor.

(B) "memory" -- field (A) refined: same content, same particulars, but \
resolved and polished prose. Drop "{speaker} said that ..." wrappers \
unless the communicative act IS the point. Resolve every relative time \
reference from (A) to an absolute date anchored at {date}.
...
absolute date woven into the sentence. Match the source's register: \
natural ("April 8, 2022", "March 2024", "2024") for chat or prose; ISO \
("2024-03-15") only if the source itself uses ISO. Match the precision \
the speaker stated. Never write {date} itself, never write a date as a \
...
"terse": "...", "queries": ["...", "..."]}}, ...]}}. Return an empty \
list if the message has no topic worth remembering.
...
class DraftSegmenter(Segmenter):
    """Autoregressive scaffold: verbatim_excerpt anchors memory/terse."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_DRAFT,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
                ...
                case TextBlock(text=text):
                    ...
                    for chunk in chunks:
                        ...
                        for item in items:
                            ...
                            # verbatim_excerpt is pure scaffolding -- it
                            # influences the LLM's writing of (B)/(C) via
                            # autoregression, but never enters retrieval
                            # or display.
