"""Deictic-resolved verbatim segmenter -- v8b.

Identical to v8 except the SPEAKER label is placed adjacent to MESSAGE
rather than before SURROUNDING CONTEXT. The hypothesis: in v8 the
speaker line was far enough from the passage that the model lost the
speaker→target binding and re-inferred it from neighbors (manifested as
the speaker-self-vocative failure at D6:3, "Are you excited, Nate?"
where Nate is the speaker).

Only the trailer of the template changes; all instructional sections
(WHEN TO EDIT / WHAT EDITS TO MAKE / WHAT STAYS UNCHANGED) are byte-
identical to v8 so any quality delta is attributable to the metadata-
placement change.
"""
...
When the referent has a clear owner, attribute from the speaker's \
perspective:
  - If the speaker owns the referent, use "my X".
...
class DeicticResolvedV8BSegmenter(Segmenter):
    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_DEICTIC_RESOLVED_V8B,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
