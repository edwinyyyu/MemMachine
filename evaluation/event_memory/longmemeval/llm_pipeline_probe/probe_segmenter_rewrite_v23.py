"""LLM rewriting segmenter v23 — v22 + tightened speech-act folding rule.

v22 still emits speech-act fragments: e.g. three Feb-25 segments from
Nate to Joanna ("said good luck", "said crossing fingers", "said
hoped her screenplay finds a fan"). v22's speech-act rule lives
inline at the end of EACH STATEMENT bullets and reads as advisory.

v23 promotes "fold related speech-acts" to its own bullet near the
event-unity rule, with a FAILURE marker named after the observed
pattern: same-occasion, same-speaker, same-addressee, same-topic
utterances must be ONE statement.
"""
...
- Folds related utterances on the same occasion. When the speaker \
makes multiple short statements on one occasion to one addressee \
about one topic (a wish and the reason for it, a question and its \
hedge, a compliment and the elaboration), these collapse into ONE \
statement that mentions every concrete particular from the set. \
Emitting one statement per utterance for such a cluster is a \
FAILURE.
...
class RewriteSegmenter(Segmenter):
    """v23 — v22 + speech-act-folding rule."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V23,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
