"""v22-fp-min-v2 RewriteSegmenter — minimalist 1p rewrite + LIST FAITHFULNESS.

Hypothesis
----------

v22-fp-min v1 wins on g3 (+1.60pp c124) but regresses on g4 cat1 by -19pp
(25/31 -> 19/31). Sampled g4 cat1 failures all share one mechanism: the
terse first-person prose tempts the model to compress enumerations into
natural-sounding summaries that drop list items.

  - "Which endorsement deals has John been offered?" -> v1 emitted
    "I have been offered no specific endorsements" while the raw turn
    listed Nike, Gatorade, and Moxie.
  - "What books has Tim read?" -> v1 said "no record" while the raw
    turn enumerated 5+ titles.
  - "How many times has John injured his ankle?" -> v1 said 1x while
    the raw turn mentioned 2 separate sprains.

v2 fix
------

Add ONE explicit LIST FAITHFULNESS rule that forbids the model from
collapsing enumerations into ``various`` / ``several`` / a partial
sample. Add 2 neutral examples (Alice/Bob with puppy hobbies / book
club / climbing-style domains -- never Tim/John/Audrey/Wolves/Mustang)
that demonstrate keeping every list item verbatim.

Everything else (KEEP/DROP, one-event-per-statement, speech-act-only-
when-event, date discipline, 1p voice, ProducerContext, neighbors block)
is unchanged from v22-fp-min v1.
"""
...
class RewriteSegmenter(Segmenter):
    """First-person rewrite segmenter (minimalist-v2 list-faithful variant).
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_REWRITE_V22_FP_MIN_V2,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
