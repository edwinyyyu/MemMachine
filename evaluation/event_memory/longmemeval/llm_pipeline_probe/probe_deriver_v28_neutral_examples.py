"""v28 deriver — v25 with prompt examples disjoint from bench cases.

User feedback: v25's identifying-relations examples ("my wife Anne",
"manager Sarah", "CFO Casey") use names and relations that LITERALLY
appear in bench tests B1 and B2. The bare-entity example "Tokyo" matches
C5. The focused-statement example "chess move" matches F2. This is
overfitting at the example level — the bench could be testing memorization
rather than principle abstraction.

v28 swaps prompt examples to placeholder ABCDE names and disjoint
instances:
  - "my wife Anne" → "my mentor Alice"
  - "manager Sarah" → "team lead Bob"
  - "CFO Casey" → "CTO Charlie"
  - "Tokyo" (bare entity) → "Paris"
  - "chess move" (focused statement) → "a function signature"
  - "commit message" stays (no current bench overlap)
  - "definition" stays

This is a STRONGER test: if v28 still passes the bench, the model has
truly abstracted the principle. If v28 regresses, the prompt was leaning
on example memorization.

Same rule structure as v25 — only example tokens changed.
"""
...
if __name__ == "__main__":
    print(
        f"v28 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
