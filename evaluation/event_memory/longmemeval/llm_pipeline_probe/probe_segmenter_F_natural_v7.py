"""LLM segmenter, Mode F-natural v7 (iteration artifact).

Change: F2 fix v3: remove Hey, from greetings, allow softener+content

This file preserves the v7 prompt body as drafted in this session, per the
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V7.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V7.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
