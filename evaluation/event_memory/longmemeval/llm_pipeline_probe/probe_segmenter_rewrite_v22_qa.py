"""v22-qa -- v22 with Q+A pair embedding format.

Hypothesis: LongMemEval/LoCoMo queries are questions, so embedding text \
that mirrors Q+A document structure should cluster closer to the query \
distribution than plain statements. Diff from qkey: explicit "Q: ... \
A: ..." document framing rather than "Queries: ..." appendix.

text_to_embed = "Q: {question}\nA: {memory}\n{speaker}: {raw_chunk}"

block.text remains the memory statement (used for BM25 + display fallback).
Display under --answer-with-raw-events is the raw event.
...
PROMPT_REWRITE_V22_QA = """\
Rewrite the MESSAGE into a JSON list of memory items. Each item \
contains a third-person memory statement plus a hypothetical \
question. A future user querying any specific content in the message \
should find at least one item whose statement OR question contains \
that content.
...
{{ "memory": "...", "question": "..." }} objects. Use an empty list \
...
class RewriteSegmenter(Segmenter):
    """v22-qa -- Q+A pair embedding format on top of v22 memory body."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V22_QA,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
    ...
    def _build_embed_text(
        memory: str, question: str, original_chunk: str, speaker: str
    ) -> str:
        q = (question or "").strip()
        if q:
