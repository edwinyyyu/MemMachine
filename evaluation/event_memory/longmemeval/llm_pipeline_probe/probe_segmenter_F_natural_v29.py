"""LLM segmenter, Mode F-natural v29 -- raise abstraction in rule 2.

v28's rule 2 listed concrete categories ("greetings, sign-offs, polite
restatements, envelope phrases, bylines, chapter prefaces, license
headers"). Three were weak:
  - "polite restatements" of the prior question carry the topic words
    and are a good retrieval anchor -- they should NOT be in the drop
    list.
  - "envelope phrases" is not a commonly understood term (coinage).
  - "bylines" is too domain-specific (news only).

v29 drops those three, reframes the category at higher abstraction
("the framing around a passage rather than its substance"), and keeps
the four cross-domain examples that span conversation, textbook, code.
"""
...
Interchangeable content is the framing around a passage rather than \
its substance -- e.g., greetings, sign-offs, chapter prefaces, license \
headers.
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V29.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V29.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
