"""v48 deriver -- v47 with cleaner ONE central subject phrasing.

User feedback on v47:
  1. "(person, place, project)" is weird; canonical English is
     "person, place, thing"
  2. "ONE central subject -- whatever single thing the segment is
     about" is redundant
  3. Nested parenthesized lists "(entity (person, place, project), a
     process or event, a concept (theorem, definition, topic), a
     mental state (preference, opinion, decision))" is bloated

v48 rewrites the subject bullet as a clean two-sentence structure:
  - First sentence: the rule (ONE central subject -> ONE derivative)
  - Second sentence: what counts as a central subject (flat list +
    escape hatch)

Other content kept from v47 (open-form CONCRETE PARTICULARS, all
robustness fixes from v41-v45).
"""
...
- A passage centered on ONE central subject across any number of \
sentences -> ONE derivative. The central subject can be a person, \
place, thing, process, event, concept, preference, opinion, decision, \
or any other coherent subject. Co-mentioned particulars (a city, a \
friend, a brand) are ATTRIBUTES, not new topics. Atomizing a \
single-subject narrative is a FAILURE.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v48 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
