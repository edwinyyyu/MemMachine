"""v22-fp-min RewriteSegmenter — minimalist first-person rewrite prompt.

Hypothesis
----------

v22-fp's prompt is ~290 lines and ~3000 tokens. An audit at gpt-5.4-nano @
low and @ medium on 24 LoCoMo turns showed:

  - person-form discipline: 0/35 violations (clean)
  - forbidden date formats: 0/35 (clean)
  - msg_date_leak: ~5.6%
  - bare_relative (relative phrase un-stripped after resolution): 14-20%

The clean rules are spending model attention that the violated rules
need. This variant aggressively cuts everything the model already
follows for free, keeping ~half the prompt length so the date-relative
rules sit closer to the model's working attention.

Major cuts
----------

  - the long "specific content" / "interchangeable" enumeration
  - the "polarity preservation" rule (model handles for free)
  - the "inferred attributes are FAILURES" rule (rare violation)
  - the multi-bullet "EACH STATEMENT" sub-list (consolidated to a line)
  - the long speech-act explanation (single line)
  - per-rule "WHY" justifications

Kept
----

  - KEEP/DROP principle (one short paragraph)
  - one-event-per-statement with anti-fragmentation phrasing
  - first-person preservation + cross-speaker 3p (one short paragraph)
  - date discipline: don't echo {date}; resolve relatives; emit
    ``on YYYY-MM-DD`` only when different; strip the relative phrase
  - forbidden formats listed once, terse
  - 5 examples covering: message-time, resolved-different-date,
    explicit-different-date, speech-act-as-event, empty-filler
"""
...
class RewriteSegmenter(Segmenter):
    """First-person rewrite segmenter (minimalist prompt variant).
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_REWRITE_V22_FP_MIN,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
