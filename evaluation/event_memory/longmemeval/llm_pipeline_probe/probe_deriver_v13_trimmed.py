"""v13 deriver probe — v10 rules with trimmed wording and plainer headings.

v12 (2760 chars, merged 7→6 rules) regressed: O1 0/5, C2 1 deriv, C4 1 deriv.
Lesson: rule COUNT and BODY FORCE matter for behavior; rule WORDING and
HEADINGS can be trimmed.

v13 keeps all 7 rules; trims verbose example lists and renames headings
to plainer terms.

Heading changes (v10 → v13):
  VERBATIM TOKENS                          → KEEP NAMES VERBATIM
  COUNT CENTRAL SUBJECTS, NOT SENTENCES    → ONE TOPIC, FEW DERIVATIVES
  ATOMIC FACTS GET ONE DERIVATIVE EACH     → EACH DISTINCT FACT ONCE
  NO NEAR-CLONES                           → NO DUPLICATES
  CARRY SCOPE WHEN SPLITTING               → EACH DERIVATIVE STANDS ALONE
  NON-PROSE SURFACES GET REPLACED ...      → DESCRIBE NON-PROSE
  BRIDGE FORM GAPS                         → INCLUDE BOTH FORMS

Body trims: shorten example lists, drop hedge words, keep the load-bearing
imperatives (EXACTLY ONE / STOP / ALWAYS / NEVER).
"""
...
Each derivative is a full grammatical sentence — no telegraph fragments, \
no formatting artifacts.

Rules:
  1. KEEP NAMES VERBATIM. Named entities, places, brands, dates, amounts, \
distinctive concept words, technical terms, identifiers, and quoted \
speech with attribution appear verbatim in any derivative covering them. \
Never invent content the segment lacks.

  2. ONE TOPIC, FEW DERIVATIVES. If most sentences describe ONE thing \
...
brand) are attributes of that thing, not new things to split on.

  3. EACH DISTINCT FACT ONCE. For multiple facts that would each answer \
a different query, emit one self-contained derivative per fact. For a \
single focused fact (a commit message, a chess move, a definition), \
emit EXACTLY ONE near-original derivative and STOP. For a bare \
standalone term, emit just that term.

  4. NO DUPLICATES. Drop any derivative that would match the same query \
as another. Items differing only by an id+value (model versions, table \
rows) collapse into ONE derivative listing items inline. An acronym \
and its expansion belong in ONE derivative.

  5. EACH DERIVATIVE STANDS ALONE. When splitting into siblings, repeat \
the shared scope (the trip, project, period, person these facts are \
ABOUT) in each sibling whose fact lives under it. Per-clause attributes \
(a brand in one clause, a person in one clause) do not propagate.

  6. DESCRIBE NON-PROSE. For encoded text, code, tables, lists of raw \
...
brackets — render values in prose.
  ...
  7. INCLUDE BOTH FORMS. When likely query tokens differ from the \
segment (heavy acronyms like JFK/POTUS/CMC, or text in a different \
script/language than queries will use), one derivative includes the \
...
async def main():
    ...
    print("# DERIVER v13 TRIMMED PROBE (7 rules, plainer headings)")
