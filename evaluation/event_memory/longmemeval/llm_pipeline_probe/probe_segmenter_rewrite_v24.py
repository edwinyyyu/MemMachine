"""LLM rewriting segmenter v24 — v22 + compact phrasing instruction.

v22 emits verbose wrap-around: "On 2022-02-25, Nate said that he
hoped Joanna's screenplay finds a fan and is given its due." Most of
that is wrapper. The same content compresses to "Nate (2022-02-25):
hoped Joanna's screenplay finds a fan and is given its due."

v24 adds an "EACH STATEMENT IS COMPACT" rule that bans speech-act
wrappers and prefers terse anchoring. Hypothesis: same accuracy at
fewer tokens improves Pareto frontier at every budget.
"""
...
- IS COMPACT. State content directly; drop framing words that add no \
searchable particular. A leading "On <date>, X said that X felt Y" \
becomes "X felt Y on <date>". A "X told Y that X is doing Z" becomes \
"X is doing Z on <date>", or, when the addressee is essential, "X \
told Y: X is doing Z". The date anchor sits at the end of the \
sentence unless leading it improves clarity. Particles that add no \
searchable content ("that" as complementizer, "really", "actually", \
"just", "kind of") are DROPPED.
...
class RewriteSegmenter(Segmenter):
    """v24 — v22 + compact-phrasing rule."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V24,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
