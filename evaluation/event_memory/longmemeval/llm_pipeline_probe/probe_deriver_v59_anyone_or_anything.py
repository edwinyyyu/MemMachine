"""v59 deriver -- v58 with "anyone involved" -> "anyone or anything
involved" to cover pets and non-people (Mochi, etc.).

In English, "anyone" implies people; "anything" implies non-people.
The bench includes Mochi (a dog) in B1 and the clock/treehouse/pie
artifacts in PR1/PR2/PR3 -- "anyone or anything" covers both
categories explicitly.

v59 = v58 with two replacements in self-containment rule:
  - "anyone involved in it" -> "anyone or anything involved in it"
  - "scope or anyone involved" -> "scope or anything involved in it"
"""
...
scope (a trip, project, period, conversation, artifact) and anyone or \
anything involved in it, EVERY sibling must include both BY NAME (not \
as "it", "they", or "we" standing in for the searchable subject) -- \
otherwise a query mentioning the scope or anything involved returns \
only the introductory sibling, not the ones describing what actually \
happened.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v59 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
