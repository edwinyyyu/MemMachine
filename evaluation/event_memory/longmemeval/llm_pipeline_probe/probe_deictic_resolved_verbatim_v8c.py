"""Deictic-resolved verbatim segmenter -- v8c.

V8B + a new HOW TO EDIT section that addresses failure modes observed
across nano@low / mini@med / full@med on 25-case manual probe:

  - (1) Subtle topic changes principle: the model conflated movies and
    books in D9:14 because both involved "this trilogy" / "this series"
    spanning a topic pivot. Principle wording rather than a strict rule
    because topic-change detection isn't binary.
  - (2) Ownership inference guard: D28:10 ("your car, Calvin"),
    D25:5 FULL ("my recovery"), D23:22 MINI ("your place"), D8:5
    MINI ("your photo, Dave") all infer ownership from demonstratives
    ("this", "that") or from "speaker shared a photo of X" without an
    explicit "my X" / "your X" / "their X" claim.
  - (4) Vocative position negative example: "(not mid-sentence)" in
    parens was being ignored. Hoist + show WRONG-form example.
  - (5) Output the full edited message: D27:1 FULL dropped most of the
    message content.
  - (6) Autoregressive co-reference: once an entity is named in the
    edited output, subsequent same-message references to it stay as
    the original pronouns -- trust the reader follows the noun phrase
    just established.

HOW TO EDIT placed BETWEEN "what edits" and "what stays" so the model
reads "when -> what -> how -> stays" as a clean pipeline.
"""
...
A WRONG form is "Do you, Alice, like pizza?". The original "you" \
stays.
...
- Once an entity is named in your edited output, the reader sees \
the noun phrase and will follow subsequent co-references on their \
own. The first occurrence of a pronoun referring to a given entity \
may need substitution; the rest of the same message can keep the \
original pronouns. Trust the reader, and trust your earlier \
substitution.
...
class DeicticResolvedV8CSegmenter(Segmenter):
    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_DEICTIC_RESOLVED_V8C,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
