"""Deictic-resolved verbatim segmenter -- v8j.

V8E with the surrounding context and target message rendered using
EventMemory's internal segment-header format:

    [Saturday, February 24, 2024 at 11:15:00 AM] Nate: "Hey Joanna!..."
    [Saturday, February 24, 2024 at 11:16:00 AM] Joanna: "Hey Nate!..."

This matches `EventMemory._segment_header` (FormatOptions default:
date_style="full", time_style="long") so the prompt presents events
in the same form they're rendered elsewhere in the system.

Locally extends SurroundingEvent with an optional timestamp field via
DatedSurroundingEvent (test-pipeline subclass; production
...
# Mirror EventMemory's FormatOptions defaults:
#   date_style="full", time_style="long", locale="en_US"
_DATE_STYLE = "full"
_TIME_STYLE = "short"
_LOCALE = "en_US"
...
class DatedSurroundingEvent(SurroundingEvent):
    ...
    formatter. Production SurroundingEvent is unchanged; this subclass
    is local to the v8j probe."""

    timestamp: datetime | None = None


PROMPT_DEICTIC_RESOLVED_V8J = """\
...
def _format_timestamp(ts: datetime | None) -> str:
    """Match EventMemory._format_timestamp with default FormatOptions
    (full date + long time, en_US locale, no timezone normalization)."""
    if ts is None:
        return ""
    ...
    if not date_string:
        return time_string
    # Connector follows the more verbose of the two styles.
...
def _format_event_line(producer: str, text: str, ts: datetime | None) -> str:
    """Inline `[timestamp] producer: text` per turn, matching the header
    shape EventMemory._segment_header uses but without the JSON quoting
    EventMemory applies for multi-segment accumulation. That JSON quoting
    is unnecessary for the prompt and induces output artifacts (the model
    echoes the surrounding quotes; a leading inline vocative gets read as
    a second producer label)."""
...
class DeicticResolvedV8JSegmenter(Segmenter):
    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_DEICTIC_RESOLVED_V8J,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
