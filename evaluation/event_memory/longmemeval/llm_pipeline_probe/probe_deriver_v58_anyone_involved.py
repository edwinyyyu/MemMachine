"""v58 deriver -- SHIPPED. v57 with jargon "co-actors" replaced by plain
"anyone involved".

4-pass sweep on expanded 13-case bench (78 calls/pass, 312 calls/prompt):
  v57 ("co-actors"):       298/312 = 95.5%
  v58 ("anyone involved"): 299/312 = 95.8%

Statistically tied. Plain-English wording works equally well as jargon.
B1 and D1 slightly improved in v58, FIL1 and PR3 slightly regressed --
all within noise.



User feedback: "co-actor" is jargon. Plain-English alternatives
considered: "participant" (slightly formal, single word),
"people involved" (excludes pets/things), "anyone involved"
(broadest, stays simple).

v58 = v57 with two replacements in self-containment rule:
  - "the central co-actors of that scope" -> "anyone involved in it"
  - "scope or a co-actor" -> "scope or anyone involved"
"""
...
involved in it, EVERY sibling must include both BY NAME (not as "it", \
"they", or "we" standing in for the searchable subject) -- otherwise \
a query mentioning the scope or anyone involved returns only the \
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v58 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
