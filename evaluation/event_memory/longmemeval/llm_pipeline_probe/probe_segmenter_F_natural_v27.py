"""LLM segmenter, Mode F-natural v27 — all-ASCII (em-dashes -> --).

Identical to v26 with the 8 em-dashes replaced by '--'.
"""
...
merge unrelated ones.
  7. STANDALONE -- each segment reads on its own. If a quote depends \
on a referent introduced earlier ("the trip" for "the Tokyo trip in \
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V27.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V27.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
