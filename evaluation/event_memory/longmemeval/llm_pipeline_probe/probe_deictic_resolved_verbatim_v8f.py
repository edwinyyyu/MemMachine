"""Deictic-resolved verbatim segmenter -- v8f.

V8D with an explicit step-by-step WORKFLOW added at the top of the
instruction body. The workflow iterates over pronouns and demonstratives
in order, one at a time, applying the WHEN/WHAT/HOW rules per reference.
The "only resolve each entity once" constraint is embedded as an
eligibility check in WHEN TO EDIT ("has an earlier reference been
resolved?") rather than upfront grouping, because per-reference iteration
fits autoregressive generation more naturally.

V8E is the "consider everything in one go" framing.
V8F is the step-by-step workflow framing.
"""
...
1. For each pronoun or demonstrative in the MESSAGE, in order from \
first to last:
   a. Identify which entity (if any) it refers to in the surrounding \
context.
   b. Check the current reference against WHEN TO EDIT below. If it \
is not eligible, leave it unchanged.
   c. If eligible, apply the matching pattern from WHAT EDITS TO \
MAKE, following the principles in HOW TO EDIT.
2. Output the full edited message — every sentence and clause of \
the original, with any substitutions applied.

WHEN TO EDIT — for the current reference, check whether it is \
...
- Has an earlier reference in the message to the same entity \
already been resolved? If yes, leave the current reference \
unchanged — the entity is already named in your output and the \
reader will follow.
...
WHAT EDITS TO MAKE — apply the matching pattern:
...
HOW TO EDIT — apply these principles to every edit:
...
WHAT STAYS UNCHANGED — everything in the message outside the chosen \
...
class DeicticResolvedV8FSegmenter(Segmenter):
    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_DEICTIC_RESOLVED_V8F,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
