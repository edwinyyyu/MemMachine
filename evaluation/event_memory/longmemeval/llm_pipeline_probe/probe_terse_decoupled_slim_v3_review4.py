...
Date handling in (C) — applies ONLY if (A) is non-empty:

  IF (A) HAS ITEMS — substitute each phrase in (A) with its \
corresponding date from (B), woven into the prose naturally. The final \
(C) must contain ZERO occurrences of any phrase in (A); each (B) \
resolution must appear in (C) where its (A) phrase would have. \
Special case: if a resolution in (B) equals the event date ({date}), \
drop the phrase entirely with no replacement. Never write {date} \
itself, never write a date as a bracketed / parenthetical / \
sentence-prefixed tag.

  IF (A) IS EMPTY — there are no relative phrases to substitute. Write \
(C) as a natural third-person statement of the topic; no date \
transformations are needed. Never write {date} itself.
...
class Review4Segmenter(Segmenter):
    """REVIEW v3: stronger imperatives on memory-field date substitution."""
    ...
    def __init__(
        ...
        *,
        ...
        prompt_template: str = PROMPT_REVIEW4,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
