"""v49 deriver -- v48 with two phrases restored.

v48 vs v45 regressed 1.7pp. Likely culprits: dropped two phrases that
were emphasizing key behavior:
  1. "is ONE searchable thing" -- conveys the count rationale
  2. "into per-sentence derivatives" in the FAILURE line -- specifies
     the failure mode

v49 keeps v48's cleaner subject bullet structure (two-sentence,
"person, place, thing", open list with "or any other coherent
subject") but restores the two emphasizing phrases.
"""
...
sentences is ONE searchable thing -> ONE derivative. The central \
subject can be a person, place, thing, process, event, concept, \
preference, opinion, decision, or any other coherent subject. \
Co-mentioned particulars (a city, a friend, a brand) are ATTRIBUTES, \
not new topics. Atomizing a single-subject narrative into per-sentence \
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v49 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
