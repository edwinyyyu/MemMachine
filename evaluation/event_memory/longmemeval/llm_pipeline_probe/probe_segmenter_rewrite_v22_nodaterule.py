"""v22-nodaterule -- v22 with the date-handling rules REMOVED entirely.

What this iterates from v22
---------------------------

v22 has two prompt rules dedicated to dates:
  ...
  - Resolves every relative time reference into an absolute date or
    ...
    backward; ...; The original relative phrase MUST NOT appear
    after resolution.
...
Hypothesis: these rules may be noise. The model naturally handles
dates reasonably; the explicit "resolve to absolute date" rule
fabricates dates from imprecise references ("next month" -> a single
day) and the "anchored to the date" rule forces an inline date even
when the framework's prepended `[<timestamp>]` header already
provides one.

This variant DELETES both rules with NO replacement. The model is
left to handle dates however it naturally does — typically: keep
explicit absolute dates verbatim, keep natural relative phrasing
(yesterday, last week), don't add inline anchors unless useful.

This is distinct from:
  - naturaltime-v2 (drop ALL inline anchors + FORBID translating
    relative phrases) -- too aggressive, lost recall
  - min3p (drops redundant same-day inline date) -- conditional,
    fragile
  - this variant (no rule at all) -- model defaults

Architecture
------------

block.text = clean 3p rewrite (model's default behavior, no date rule)
context = RewriteContext(text_to_embed = "{rewrite}\\n{speaker}: {raw_chunk}")

Same dual-text embed as v22 baseline. Same KEEP/DROP rules. Only
difference: the two date rules are gone.
"""
...
class RewriteSegmenter(Segmenter):
    """v22 with the two date rules removed; no replacement."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V22_NODATERULE,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
