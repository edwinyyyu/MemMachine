"""v42 deriver -- v41 trimmed toward <=3000 chars (round 1: 3166 chars).

Trims vs v41:
  - Goal paragraph: "searching for anything" -> "querying anything";
    drop "-- every extra slot wastes index space" parenthetical
  - PURE FILLER: drop "it is NOT pure filler --" redundancy
  - ONE central subject: drop "Atomizing... is a FAILURE." (redundant
    with goal paragraph's "atomizing single-topic narrative are
    FAILURES"); compress "even across many sentences" -> "across any
    number of sentences"
  - Compound identifiers: "every derivative that references it --
    splitting the name from its qualifier loses the binding a future
    query relies on" -> "every time -- splitting them loses the
    query-binding"
  - Stands alone: compressed self-containment rule by half while
    keeping concrete query-side reasoning

Goal: same 97.5% mean and tight variance with ~3000 chars.

SUPERSEDED by v43 (further trims).
"""
...
narrative are FAILURES, not coverage.
...
ONE derivative. Co-mentioned particulars (a city visited, a friend who \
appeared, a brand of a component) are ATTRIBUTES of the central \
subject, not new topics.
...
a definition) -> exactly ONE near-original derivative. Do NOT split \
its clauses; do NOT paraphrase.
...
keep the whole phrase together every time -- splitting them loses the \
query-binding.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v42 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
