"""v40b deriver -- SHIPPED. Renames v38a's COUNTING -> DERIVATIVES COVERAGE.

Mirrors the goal paragraph's "cover every searchable thing" framing.
Bullets describe how derivatives cover each kind of segment. The word
DERIVATIVES anchors the bullets to the action being decided (cf. v40a
bare "DERIVATIVES" regressed -5.6pp; v40c bare "COVERAGE" regressed
-2.8pp).

Empirical results (3 prompts x 3 sweep passes, 540 LLM calls):
  Across two separate 3-pass sweeps comparing alternatives:
    v38 (COUNTING)              335/360 = 93.1%
    v39a (PER SEGMENT)          165/180 = 91.7%
    v39b (GRANULARITY)          165/180 = 91.7%
    v40a (DERIVATIVES)          157/180 = 87.2%  REGRESSES (atomization)
    v40b (DERIVATIVES COVERAGE) 168/180 = 93.3%  TIED with v38
    v40c (COVERAGE)             162/180 = 90.0%  borderline regression
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v40b length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
