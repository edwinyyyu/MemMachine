"""v62 deriver -- v61 with two wording fixes.

User feedback:
  1. "finds only the derivative where they were named" should be plural
     ("derivatives") -- a query matches all anchored derivatives, not
     just one.
  2. "standing in for the central subject" is too narrow -- pronouns
     can refer to either the scope OR anyone/anything involved. The
     rule should apply to any pronoun substitution for the items
     listed.

v62 changes self-containment rule:
  v61: "...never as pronouns ('it', 'they', 'we') standing in for the
       central subject. A query for the scope or anyone/anything
       involved finds only the derivative where they were named..."

  v62: "...never as pronouns ('it', 'they', 'we') referring to them. A
       query for the scope or anyone/anything involved finds only the
       derivatives where they were named..."

Changes:
  - "standing in for the central subject" -> "referring to them"
    ("them" = the scope AND anyone/anything else involved listed in
    the preceding clause; broader and grammatically correct)
  - "derivative" -> "derivatives" (plural, since a query may match
    multiple anchored derivatives)
"""
...
as pronouns ("it", "they", "we") referring to them. A query for the \
scope or anyone/anything involved finds only the derivatives where \
they were named, not the ones using pronouns.
...
if __name__ == "__main__":
    ...
    if non_ascii:
        ...
        print("All ASCII OK")
    print(
        f"v62 length: {len(PROMPT_DERIVER)} chars / {len(PROMPT_DERIVER.split())} words"
    )
