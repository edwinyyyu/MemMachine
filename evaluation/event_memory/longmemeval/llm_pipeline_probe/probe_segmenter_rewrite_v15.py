"""LLM rewriting segmenter v15 — v14 + participant-context cross-trails.

Hypothesis: v14 g3 multi-hop is the worst category (24/37 = 64.9% l8,
32/37 = 86.5% l40). Multi-hop fails when the query needs to bridge
two separate events. A cross-trail rule — include incidental
participant attributes referenced in the same message inside the
event statement — converts some multi-hop queries to single-hop:

  Original message: "I went to the concert with Sarah, my college
  roommate, last night."
  v14: 2 statements (concert event + roommate fact)
  v15: 1 statement that covers the concert AND notes Sarah is the
  college roommate. A query for "who is the college roommate" finds
  this statement directly.

Crucially, the cross-trail must only attach attributes that the
SAME message mentions — never inferred. v14's anti-fabrication rule
(rule 8) still holds. This is rule 7 amendment, not a new rule.
...
SPECIFIC, ATTACH IN-MESSAGE PARTICIPANT CONTEXT. An EVENT is a \
single occurrence, decision, action, or narrative beat with one \
core happening (a trip, a promotion, a purchase, a conversation \
thread on one topic, a planned outing). Details that elaborate the \
SAME event -- subject, action, time, place, attendees, motivation, \
outcome, sub-details, attached media -- all go in ONE rich statement \
covering every concrete specific from those details (every name, \
date, number, quoted phrase, proper noun, attached media \
description). ONE statement is one searchable prose string, not a \
short summary that drops particulars.

PARTICIPANT CONTEXT (cross-trail): when a person, place, or \
organization in the event is described in the SAME message with an \
incidental attribute -- their role, relationship, location, \
occupation, ownership, or recent activity ("my college roommate \
Sarah", "Frank Ocean the artist", "her dog Toby", "Anne, who works \
at Stripe") -- include that attribute INSIDE the event statement, \
attached to the named participant. Example: "On 2023-XX-XX, \
{speaker} went to the concert with Sarah, {speaker}'s college \
roommate" (one statement, both facts). This lets a query for \
"who is {speaker}'s college roommate" find the event statement \
directly. Only attach attributes that the SAME message states \
explicitly -- never infer.

SEPARATE EVENTS that share a subject must be SEPARATE statements: \
"Calvin met Frank Ocean at a festival in August 2022 and they're \
recording together in August 2023" becomes TWO statements (2022 \
meeting + 2023 recording). "{speaker} went to Bali last year and \
is planning to return next month" becomes TWO statements (past trip \
+ future plan). Same subject does NOT mean same event. Splitting \
one event into multiple statements is a failure; merging two \
distinct events into one statement is also a failure.
...
class RewriteSegmenter(Segmenter):
    """v15 — v14 + participant-context cross-trail rule (rule 7 amended)."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V15,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
