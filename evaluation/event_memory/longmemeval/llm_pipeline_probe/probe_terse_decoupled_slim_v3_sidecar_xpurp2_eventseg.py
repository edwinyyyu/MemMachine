            ...
            match block:
                case TextBlock(text=text):
                    ...
                    for chunk in chunks:
                        ...
                        if not items:
                            continue
                        # EVENTSEG: ONE segment per chunk. The LLM's N
                        # memory items become N derivative components on
                        # the segment, picked up by MultiComponentDeriver
                        # (--deriver multi). block.text = raw chunk;
                        # text_to_embed/bm25 store one item's worth as a
                        # fallback for derivers that don't read embed_N.
                        embed_components: list[str] = []
                        bm25_components: list[str] = []
                        # All-items date aliases for a single combined sidecar.
                        all_memory = " ".join(
                            it.memory.strip() for it in items if it.memory
                        )
                        event_aliases = _date_aliases_verbose(
                            event.timestamp, all_memory
                        )
                        for item in items:
                            ...
                            embed_components.append("\n".join(embed_parts))
                            bm25_item = memory
                            if dates_line:
                                bm25_item = f"{bm25_item}\nDates: {dates_line}"
                            bm25_components.append(bm25_item)
                        # Concatenated embed/bm25 for fallback consumers
                        # (e.g. the `whole` deriver) and for the segment's
                        # single text_to_score_bm25 contribution.
                        embed_text = "\n\n".join(embed_components)
                        bm25_text = "\n\n".join(bm25_components)
                        # Pack components for MultiComponentDeriver.
                        properties = dict(event.properties)
                        properties[EMBED_COMPONENT_COUNT_KEY] = str(
                            len(embed_components)
                        )
                        for i, comp in enumerate(embed_components):
                            properties[f"{EMBED_COMPONENT_KEY_PREFIX}{i}"] = comp
                        # IMPORTANT: prepend speaker so the answerer
                        # sees "Caroline: I went..." not "I went..."
                        # without speaker attribution. DecoupledRetrieval-
                        # Context's segment header renders just [date],
                        # not "[date] producer:", so the speaker prefix
                        # must live in block.text for raw display.
