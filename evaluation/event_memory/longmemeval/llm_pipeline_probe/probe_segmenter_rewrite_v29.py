"""LLM rewriting segmenter v29 — v28 with compact trailing tag.

v28 (trailing "[category: <noun>]" tag) beat v22 on g3 c124 by +1.6pp
(89.89% vs 88.30%, mem0-bench K=7 OLD stack). Cat 1 +10.8pp; cat 2
preserved; cat 4 only -0.9pp. But avg tokens grew to 350/q (10 over
the 340 budget), since "[category: <noun>]" adds ~3 tokens per
statement.

v29 tightens the trailing tag to a bare bracketed hyphenated noun
phrase, e.g. "[tournament-win]" instead of "[category: tournament
win]". Same shared retrieval cue (BM25 + embedding cluster), fewer
tokens. Goal: preserve v28's cat 1 gain at lower token cost so the
configuration fits under 340 tokens.
"""
...
APPEND a compact bracketed category tag at the end of the \
statement, e.g. " [movie-watched]", " [tournament-win]", \
" [book-recommendation]". Use a single, consistent hyphenated \
category for every instance of the same class so a future query \
for the class retrieves all instances together. The trailing tag \
never replaces or precedes the particulars; entity name, date, and \
other concrete particulars must appear in the body of the \
statement first. Examples: \
"Joanna watched Eternal Sunshine of the Spotless Mind on \
2022-04-10 with Nate. [movie-watched]" \
"Nate placed 2nd at the regional tournament on 2022-04-25. \
[tournament-win]" \
"Joanna recommended the book Project Hail Mary to Nate on \
2022-09-10. [book-recommendation]"
...
class RewriteSegmenter(Segmenter):
    """v29 — v28 with compact trailing hyphenated category tag."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V29,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
