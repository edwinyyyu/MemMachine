"""LLM segmenter, Mode F-natural v24 — strip retrieval-architecture meta.

v23 had parentheticals explaining the segment-vs-derivative role
split ("even if it would not be findable on its own (search uses
other indexes)", "even though they would not be findable on their
own"). Those are meta-reasoning about retrieval architecture — the
model doesn't need to understand WHY, just WHAT.

v24 removes them. The rules just state what to keep and what to drop,
without justifying via retrieval mechanics.
"""
...
answer or reaction the reconstructed memory needs to show. When such a \
response ALSO carries content ("ok, leaving Tuesday at 5", "no, I \
changed my mind about Tuesday"), it remains KEPT — emit the whole \
utterance, not a fragment.
...
async def segment(client, model, text, reasoning="low", window_chars=WINDOW_CHARS):
    if len(text) <= window_chars:
        prompt = PROMPT_F_NATURAL_V24.format(passage=text)
    ...
    sub_results = await asyncio.gather(
        *(
            call(client, model, PROMPT_F_NATURAL_V24.format(passage=w), reasoning)
            for w in windows
        )
    )
    flat = []
