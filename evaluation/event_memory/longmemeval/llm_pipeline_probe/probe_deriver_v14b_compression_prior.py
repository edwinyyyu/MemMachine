"""v14b deriver probe — v14 goal-first + explicit compression prior.

v14 (2417 chars) regressed: D1 atomized to 13, F3 to 3, C5 emitted 0 derivs,
C3/C4 missing description, F2 split, FOCUS-F2 paraphrased.

Diagnosis: goal-first prompt presented a "menu" of granularity options
without a strong prior. The model defaulted to atomization (its training
default for "extract facts from passage" tasks). v10's seemingly-redundant
anti-fragment wording was actually encoding this prior implicitly.

v14b fix: state COMPRESSION as the prior. Frame the task as "fewest
derivatives that cover the segment", with atomization explicitly
characterized as a failure mode (wastes index slots).
"""
...
it via cosine similarity to the segment OR to one of your derivatives. \
Generate the FEWEST derivatives that achieve this goal — every extra \
derivative slot wastes index space and dilutes ranking. \
Splitting a focused thought, paraphrasing the same fact, or atomizing a \
single-topic narrative are FAILURES, not coverage.
...
Do NOT split its clauses. Do NOT paraphrase.
...
lines, JSON, markup, lists of raw numbers):
  - ALWAYS emit one DESCRIPTION derivative — a sentence saying what the \
segment IS (e.g., \"Python function returning max in a binary tree\", \
\"benchmark table comparing GPT-4, Claude, and Gemini on Task A and B\") \
and what it is ABOUT.
  - Decode recognizable encodings (Caesar, base64) into prose.
  - For multi-row tables, also emit per-row prose sentences when each \
row is independently searchable.
...
async def main():
    ...
    print("# DERIVER v14b GOAL-FIRST + COMPRESSION-PRIOR PROBE")
