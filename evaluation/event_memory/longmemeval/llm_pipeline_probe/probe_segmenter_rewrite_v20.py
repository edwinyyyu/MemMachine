"""LLM rewriting segmenter v20 — neighbor context + v33/v65 enumeration style.

Reads SurroundingEventsContext (when present) to resolve second-person
addressees and prior references; falls back to "the other party" when
only ProducerContext is available.

Prompt structure follows v33/v65 conventions: a single enumerated list
of KEEP categories, FAILURE markers on key drop cases, and terse
parenthesized triggers — no inline input→output examples and no
segment-count biases.
"""
...
EACH STATEMENT:
- Refers to the speaker by name. First-person self-references resolve \
to the speaker's name; second-person references resolve to the \
addressee's name when one is known; demonstrative and ambiguous \
...
- Describes exactly one event. An event is one occurrence, decision, \
plan, observation, state, or preference at one point or span in \
time. Distinct events (different times, different occasions, \
different actions) each get their own statement, even when they \
share participants or topics. Splitting one event across multiple \
statements is a FAILURE. Merging two distinct events into one \
statement is a FAILURE.
- Covers every concrete particular the message gives about that one \
...
class RewriteSegmenter(Segmenter):
    """v20 — reads SurroundingEventsContext for neighbor-aware rewriting."""

    def __init__(
        self,
        *,
        language_model: LanguageModel,
        prompt_template: str = PROMPT_REWRITE_V20,
        chunk_size: int = 1500,
        max_attempts: int = 3,
    ) -> None:
