"""v22-origemb — same as v22 but embeds only "speaker: original".

For embed-content ablation. segment.block.text remains the third-person
rewrite (displayed), but the indexed string is just the verbatim original
prefixed by speaker.
"""
...
class RewriteSegmenter(Segmenter):
    ...
    def _build_embed_text(rewrite: str, original_chunk: str, speaker: str) -> str:
        return f"{speaker}: {original_chunk}"
