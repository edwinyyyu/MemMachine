...
Every referring expression in the rewrite -- pronouns ("I", "you", \
"we", "they", "it"), demonstratives ("this", "that", "those"), \
definite descriptions ("the project", "the workshop"), elliptical \
predicates ("will do it", "come over", "go together"), and short \
response acts ("yes", "sounds good") -- must resolve to something \
the rewrite itself names or that exists in the world independently. \
Use the prior turns to resolve referents the message actually uses, \
but do not import topics the message itself does not engage with. \
The test: a reader who has only the rewrite -- not the source \
message, not the surrounding turns -- must be able to identify the \
same people, places, things, events, and intents the speaker meant. \
Relative time references ("yesterday", "last week", "tomorrow") are \
the one exception: keep them verbatim, since the date prefix \
resolves them separately. Keep every particular.
