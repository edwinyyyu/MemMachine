"""Single-pass unified-envelope temporal extractor (v3).

The v2.1 extractor uses two passes:
  PASS1: find candidate surfaces (returns [{surface, context_hint}])
  PASS2: resolve each surface to a temporal envelope
This is 1 + N LLM calls per doc, where N = number of surfaces.

v3 collapses both into a SINGLE call: input the passage + ref_context,
output a list of {surface, earliest, latest, granularity, confidence}
directly. Saves 1+N->1 call per doc.

Quality risks:
- Combining a find+resolve task may confuse the LLM, producing fewer
  or worse extractions.
- PASS2's JSON schema constraint was per-surface; v3 uses an array
  schema. Strict-mode JSON schema may be harder to satisfy.

Mitigation: same principle-based skip rules as v2.1 (policy/rule/
template skip, refuse-to-fabricate on missing anchor). Qualitative
validation on the 22 critical cases must pass before quantitative bench.

Cache directory: cache/temporal_retrieval_v3/.
"""
...
SINGLE_PASS_SYSTEM_V3 = """You are a temporal-reference extractor.
...
- For confidence: 1.0 when the phrase pins unambiguously; 0.4-0.7
  when underspecified ("some time ago"); 0 when uninterpretable.

# Refuse to fabricate

If you cannot place a surface on the calendar without falling back
to ref time as a fabricated anchor, set confidence=0.0 and emit any
placeholder envelope (ref_time as both edges is fine). The
downstream pipeline drops references below confidence 0.5.
...
In those cases, do not invent an envelope around ref_time.
...
class TemporalExtractorV3:
    """Single-pass unified-envelope extractor.

    Wire via TemporalRetriever's `extractor` constructor param.
    Output: list[TimeEnvelope] — one envelope per emitted reference.
    """
    ...
    def __init__(
        ...
        self.cache = _LLMCache(cd / "single_v3.json")
        # Compat with TemporalRetriever which calls these on the extractor
    ...
    async def _call(self, text: str, ref_time: datetime) -> list[dict]:
        ...
        if cached is None:
            resp = await self.client.responses.create(
                model=self.model,
                input=[
                    {"role": "system", "content": SINGLE_PASS_SYSTEM_V3},
                    {"role": "user", "content": user},
                ],
                text={"format": {"type": "json_schema", **V3_JSON_SCHEMA}},
            )
    ...
    def _to_envelope(env: dict, ref_time: datetime) -> TimeEnvelope | None:
        # NOTE: v3.1 emits a `confidence` field in the LLM JSON output,
        # but TimeEnvelope no longer carries one (removed when v3.2
        # shipped as the production extractor; see
        # extractor_v3_2_validation.json). We ignore the LLM's
        # confidence value here. v3.1 is kept as a historical
        # reference; the v3.2 skip-don't-emit contract means
        # `confidence_floor` is no longer needed downstream — but if
        # you're using v3.1, the LLM may emit unanchorable
        # placeholders and there's no longer a filter to drop them.
        try:
