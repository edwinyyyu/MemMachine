...
def evaluate_dnf_match(
    ...
    For "disjoint" leaves, the leaf factor is `1 - containment` where
    containment is in [0, 1]. `notin_aggregate=False` (default) uses
    `excluded_containment` (max-over-pairs / strict): a doc with any one
    interval fully inside the excluded window gets containment = 1 and
    leaf factor = 0. `notin_aggregate=True` uses
    `excluded_containment_aggregate`: total fraction of doc-time inside
    the excluded window — multi-interval docs whose intervals are mostly
    outside the excluded window keep most of their score.

    For "intersect"/"after"/"before" leaves we use the binary
...
# Back-compat alias.
evaluate_dnf_mask = evaluate_dnf_match
