"""Ablate the `interval` and `recurrence` TE kinds to measure their
retrieval contribution.

Production cache (16k pass2 entries) shows the kind distribution:
- instant 74.7%, interval 14.9%, recurrence 6.5%, duration 3.9%

Question: do `interval` and `recurrence` actually move retrieval recall?

Variants (all share the same extracted TEs; only flatten_intervals' kind
filter differs):
  full        - skip_kinds=None (current production)
  no_interval - skip_kinds={"interval"}
  no_recur    - skip_kinds={"recurrence"}
  no_both     - skip_kinds={"interval", "recurrence"}

Tests across the 7 standing benches.

Run from ``evaluation/``::

    uv run python -m temporal_retrieval.research._validate_kind_ablation
"""
...
VARIANTS = {
    "full": {"skip_kinds": None},
    "no_interval": {"skip_kinds": frozenset({"interval"})},
    "no_recur": {"skip_kinds": frozenset({"recurrence"})},
    "no_both": {"skip_kinds": frozenset({"interval", "recurrence"})},
}
...
async def evaluate(
    ...
    retriever = TemporalRetriever(
        embed_fn=embed_fn,
        rerank_fn=rerank_fn,
        skip_kinds=variant_kw["skip_kinds"],
    )
    await retriever.index(docs)

    # Count intervals across all docs (signal of how much was masked out)
...
async def main() -> None:
    ...
    out_path = ROOT / "kind_ablation_validation.json"
