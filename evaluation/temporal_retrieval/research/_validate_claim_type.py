"""Validate v3.4 (claim_type) + claim-type-dispatched scoring vs baseline.

Architecture:
  - Use v3.4 extractor (emits earliest/latest/claim_type per ref) to
    extract intervals for both docs and query anchors.
  - Compare two scoring variants:
      baseline: binary intersect + binary after/before (current shipped)
      claim_type: event→doc-frac, state/habit→query-frac;
                  after/before linear decay with floor
  - Score is base_cosine + match + recency (additive, weights 1.0).

Runs across the 35 standard benches + the new fractional_intent bench
which specifically exercises state/event distinction.

Run from ``evaluation/``::

    uv run python -m temporal_retrieval.research._validate_claim_type
"""
...
def _baseline_match(plan, doc_ivs, resolver) -> float:
    """Binary intersect + binary after/before + fractional disjoint."""
...
async def evaluate_bench(bench, embed_fn, rerank_fn):
    ...
    # === Index: extract doc refs (with claim_type) ===
    ...
    variant_stats = {
        "baseline":       {"n_eval": 0, "n_r1": 0, "n_r5": 0, "all_r5": []},
        "ct_evbinary":    {"n_eval": 0, "n_r1": 0, "n_r5": 0, "all_r5": []},
        "ct_evbin_binab": {"n_eval": 0, "n_r1": 0, "n_r5": 0, "all_r5": []},
        "ct_evdocf_binab":{"n_eval": 0, "n_r1": 0, "n_r5": 0, "all_r5": []},
        "frac_min_binab": {"n_eval": 0, "n_r1": 0, "n_r5": 0, "all_r5": []},
    }
    ...
    for q in queries:
        ...
        # Resolve anchors (extract on leaf phrase; produces refs, drop claim_type for anchors)
        ...
        # === Score each variant ===
        for vname in ("baseline", "ct_evbinary", "ct_evbin_binab",
            ...
            for did in pool:
                ...
                if not d_refs:
                    # Production empty_doc_match=1.0 for ALL variants for
                    ...
                    if vname == "baseline":
                        ...
                        # event=binary, state=min-norm, proximity after/before
                        match_scores[did] = claim_type_scoring(
                            plan, d_refs, resolver, event_binary=True)
                    elif vname == "ct_evbin_binab":
                        # event=binary, state=min-norm, BINARY after/before
                        match_scores[did] = claim_type_scoring(
                            plan, d_refs, resolver, event_binary=True,
                            binary_ab=True)
                    elif vname == "ct_evdocf_binab":
                        # event=DOC-FRAC, state=min-norm, BINARY after/before
                        # — the "right" asymmetric scoring per claim_type intent.
                        match_scores[did] = claim_type_scoring(
                            plan, d_refs, resolver, event_docfrac=True,
                            binary_ab=True)
                    else:  # frac_min_binab: uniform min-norm intersect, binary after/before
...
async def main():
    ...
    print("\n" + "=" * 130, flush=True)
    print(f"{'bench':28s} "
          + " ".join(f"{v[:8]:>10s}" for v in variants_all)
          + "   "
          + " ".join(f"Δ{v[:6]:>6s}" for v in variants_all[1:]), flush=True)
    ...
    out = ROOT / "claim_type_validation.json"
