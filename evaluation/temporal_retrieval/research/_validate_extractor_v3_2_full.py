"""Full-bench A/B of v3.1 vs v3.2 across every temporal bench we have.

Earlier validation used a 7-bench representative subset. This sweeps
the comprehensive set (34 benches) to ensure v3.2 has no regressions
anywhere — not just on the convenient subset.

Skips `real_benchmark` / `real_benchmark_small` (older variants of
realq) to avoid double-counting; everything else included.

Run from ``evaluation/``::

    uv run python -m temporal_retrieval.research._validate_extractor_v3_2_full
"""
...
async def main():
    ...
    # Summary
    print("\n" + "=" * 110, flush=True)
    print(f"{'bench':28s} {'v3.1 R@1':>9s} {'v3.2 R@1':>9s} {'ΔR@1':>7s}   {'v3.1 R@5':>9s} {'v3.2 R@5':>9s} {'ΔR@5':>7s}", flush=True)
    print("-" * 110, flush=True)

    macro = {v: {"R@1": [], "R@5": [], "all_R@5": []} for v in ("v3_1", "v3_2")}
    regressions: list[str] = []
    improvements: list[str] = []
    for bench in BENCHES:
        try:
            r1 = next(x for x in rows if x["bench"] == bench and x["variant"] == "v3_1" and "error" not in x)
        ...
        d1 = r2["R@1"] - r1["R@1"]
        d5 = r2["R@5"] - r1["R@5"]
        print(f"{bench:28s} {r1['R@1']:>9.3f} {r2['R@1']:>9.3f} {d1:>+7.3f}   {r1['R@5']:>9.3f} {r2['R@5']:>9.3f} {d5:>+7.3f}", flush=True)
        for k in ("R@1", "R@5", "all_R@5"):
            macro["v3_1"][k].append(r1[k])
            macro["v3_2"][k].append(r2[k])
    ...
    out_path = ROOT / "extractor_v3_2_full_validation.json"
