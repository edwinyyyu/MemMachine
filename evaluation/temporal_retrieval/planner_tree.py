"""Boolean-tree LLM query planner (variant of DNF planner).

A plan is a recursive node tree:
    Leaf(phrase, relation)         — a single temporal constraint
    And(children)                  — conjunction
    Or(children)                   — disjunction
    Not(child)                     — negation of a sub-tree

Relation values: "intersect" | "after" | "before" | "disjoint", same as
the DNF planner. Negation can either be expressed as `disjoint` on a
Leaf (preferred for simple cases) OR via a `Not` node wrapping a more
complex sub-expression.

Match evaluator: recursive — min over `and`, max over `or`,
`1.0 - eval(child)` for `not`, per-relation factor for `leaf`.
"""
...
NODE. Each node has one of four `type` values:
  ...
  - "and"   : conjunction. `children` is a non-empty list of sub-nodes.
              Doc must satisfy all children.
  - "or"    : disjunction. `children` is a non-empty list of sub-nodes.
              Doc satisfies the node if it satisfies any child.
...
not used by its type:
  ...
  - "and"   : phrase=null, relation=null, children=[<node>, ...], child=null
  - "or"    : phrase=null, relation=null, children=[<node>, ...], child=null
  - "not"   : phrase=null, relation=null, children=[], child=<node>

99% of queries fold to a single leaf. Use `and` / `or` ONLY when the
...
OTHER constraints in the SAME and-group, NOT against `Reference time`.

  "in 2024 not in summer"
    -> and(leaf("2024","intersect"), leaf("summer 2024","disjoint"))

  "in 2024 excluding the spring semester"
    -> and(leaf("2024","intersect"), leaf("spring 2024","disjoint"))

  "in Q1 2023 outside of February"
    -> and(leaf("Q1 2023","intersect"), leaf("February 2023","disjoint"))

  Already-qualified phrases need no composition:
  "What I did since 2022 outside of Q1 2023"
    -> and(leaf("2022","after"), leaf("Q1 2023","disjoint"))
...
{{"expr":{{"type":"and","phrase":null,"relation":null,"children":[{{"type":"leaf","phrase":"2024","relation":"intersect","children":[],"child":null}},{{"type":"leaf","phrase":"summer 2024","relation":"disjoint","children":[],"child":null}}],"child":null}},"extremum":null}}

Query: "What did I do in Q3 2023 after the launch?"
{{"expr":{{"type":"and","phrase":null,"relation":null,"children":[{{"type":"leaf","phrase":"Q3 2023","relation":"intersect","children":[],"child":null}},{{"type":"leaf","phrase":"the launch","relation":"after","children":[],"child":null}}],"child":null}},"extremum":null}}
...
{{"expr":{{"type":"or","phrase":null,"relation":null,"children":[{{"type":"leaf","phrase":"Q1 2023","relation":"intersect","children":[],"child":null}},{{"type":"leaf","phrase":"Q4 2023","relation":"intersect","children":[],"child":null}}],"child":null}},"extremum":null}}
...
# ---------------------------------------------------------------------------
# JSON schema (recursive via $defs / $ref, strict-mode-compatible)
# ---------------------------------------------------------------------------
...
def _parse_node(d: Any) -> Node | None:
    ...
    if t in ("and", "or"):
        ...
        if not kids:
            return None
        if len(kids) == 1:
            # Collapse singleton and/or to its child for cleaner eval.
            return kids[0]
        return And(children=kids) if t == "and" else Or(children=kids)
    if t == "not":
...
# ---------------------------------------------------------------------------
class TreePlanner:
    """LLM planner that emits a boolean tree of constraints."""
...
# ---------------------------------------------------------------------------
# Tree match evaluator
# ---------------------------------------------------------------------------
def evaluate_tree_match(
    plan: TreeQueryPlan,
    doc_ivs: list,
    leaf_anchor_resolver,  # callable: (leaf: Leaf) -> list[Interval]
    ...
    Returns a temporal-match score in [0, 1]. 1.0 if `plan.expr is None`.

    AND aggregators (strict → permissive):
      "min"        - Zadeh strict; any 0 leaf collapses the clause to 0.
      "product"    - probabilistic AND; multiplicative.
      "mean"       - arithmetic mean; linear partial credit (BM25-style).
      "noisy_or"   - 1 - ∏(1 - α·fᵢ); cumulative evidence per leaf
                     (Pearl's noisy-OR, α = per-leaf activation prob).
      "sum_capped" - min(1, Σfᵢ); additive with cap, very permissive.

    OR aggregators (selection → fusion):
      "max"     - Zadeh selection; any one clause suffices.
      "probsum" - probabilistic OR; 1 - ∏(1 - cⱼ); bonus for multi-match.
      "mean"    - arithmetic mean of clause scores.

    Both axes are orthogonal. Backward-compat: legacy `aggregator=` was
    a single switch — passed via and_aggregator only.
    """
    from .core import (
    ...
    if and_aggregator not in ("min", "product", "mean", "noisy_or", "sum_capped",
                              "zadeh", "flat_mean"):  # accept legacy "zadeh" alias
        raise ValueError(f"unknown and_aggregator: {and_aggregator!r}")
    if or_aggregator not in ("max", "probsum", "mean", "zadeh"):
        raise ValueError(f"unknown or_aggregator: {or_aggregator!r}")
    # legacy alias: "zadeh" → min for AND, max for OR
    if and_aggregator == "zadeh":
        and_aggregator = "min"
    ...
    def _and(vs: list) -> float:
        ...
        if and_aggregator == "product":
            p = 1.0
            for v in vs:
                p *= v
            return p
        if and_aggregator == "noisy_or":
            p = 1.0
            for v in vs:
                p *= (1.0 - noisy_alpha * v)
            return 1.0 - p
        if and_aggregator == "sum_capped":
    ...
    # Flat mode: collect all leaf factors, ignore tree structure.
    # When `and_aggregator == "flat_mean"`, _eval is bypassed and we
    # average every leaf factor uniformly.
    ...
    def _eval(node: Node) -> float:
        if isinstance(node, Leaf):
            ...
            if node.relation == "intersect" and intersect_leaf == "min_norm":
                return _intersect_min_norm(doc_ivs, anchor_ivs)
            return constraint_factor_for_doc(doc_ivs, anchor_ivs, node.relation)
        if isinstance(node, And):
            return _and([_eval(c) for c in node.children])
        if isinstance(node, Or):
            return _or([_eval(c) for c in node.children])
        if isinstance(node, Not):
    ...
    if and_aggregator == "flat_mean":
        # Ignore tree structure; uniform mean of every leaf factor.
        # Each leaf still uses the right per-relation formula (disjoint,
        # min-norm intersect, etc.); AND/OR distinction is collapsed.
        factors = []
        for leaf in _iter_leaves(plan.expr):
            anchor_ivs = leaf_anchor_resolver(leaf)
            if not anchor_ivs:
                continue
            if leaf.relation == "disjoint":
                ...
                factors.append(_intersect_min_norm(doc_ivs, anchor_ivs))
            else:
                factors.append(constraint_factor_for_doc(doc_ivs, anchor_ivs,
                                                        leaf.relation))
        if not factors:
            return 1.0
