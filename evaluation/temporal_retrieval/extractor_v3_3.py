"""V3.3 extractor: drop `surface` and `granularity` from LLM output.

Retrieval scoring only reads `earliest` and `latest`. The `surface`
field is debug breadcrumb; `granularity` is also dead-weight in the
retrieval algorithm (only the produced earliest/latest matter, not
their label). V3.3 ablates both from the LLM output schema and from
the prompt's output section.

Keeps the "fuzzy widening by one granularity level" rule in the
prompt body, because the WIDENING affects earliest/latest. Just
removes the requirement to emit the label.

Cache directory: cache/temporal_retrieval_v3_3/.
"""
...
SINGLE_PASS_SYSTEM_V3_3 = """You are a temporal-reference extractor.
...
class TemporalExtractorV3_3:
    def __init__(
        ...
        self.cache = _LLMCache(cd / "single_v3_3.json")
    ...
    async def _call(self, text: str, ref_time: datetime) -> list[dict]:
        ...
        if cached is None:
            resp = await self.client.responses.create(
                model=self.model,
                input=[
                    {"role": "system", "content": SINGLE_PASS_SYSTEM_V3_3},
                    {"role": "user", "content": user},
                ],
                text={"format": {"type": "json_schema", **V3_3_JSON_SCHEMA}},
            )
