"""Run a single hard scenario by name. Used for parallel exploration.

Usage:
  uv run python run_one.py <scenario_name>

Where scenario_name is the module name in scenarios/ (without .py).
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: run_one.py <scenario>")
        sys.exit(1)
    scenario_name = sys.argv[1]
    scenario = importlib.import_module(scenario_name)

    name = f"r24_one_{scenario_name}"
    ...
    # Dump obs/cog facts for diagnostic
    out["obs_texts"] = [
        {
            "uuid": f.fact_uuid,
            "ts": f.ts,
