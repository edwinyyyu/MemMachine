"""Round 19 — variable-binding architecture.

Tests whether emitting RESOLUTION items at name-turn (binding label to existing
anonymous cluster) outperforms framing B (refs from name-entry to descriptor-
entry).

Variants:
  1. binding_K3_w7_w14   K-block centered window, 2x fudge on future lookahead.

Metrics:
  - binding_correctness: did the writer create an anon cluster at descriptor
    AND emit a RESOLUTION binding it to the right canonical_label by name-turn?
  - chain_consistency: does the @User.<predicate> chain head's cluster_id match
    the resolved entity (via cluster_label)?
  - QA judge (compares against round 16a baselines on the same scenario).
"""
...
def binding_correctness(
    log: list[aen2_binding.LogEntry],
    resolutions: list[aen2_binding.Resolution],
    idx: aen2_binding.IndexedLog,
    gt: multi_batch_coref.GroundTruth,
) -> dict:
    """For each pair, check the variable-binding workflow:

    A. anon_cluster_at_descriptor: an entry exists with the right
       (subject=@User, predicate=@User.<pred>) AT or NEAR the descriptor turn,
       and its cluster_id has canonical_label=None at descriptor time.
    B. resolution_at_name: a Resolution exists at or near the name turn that
       binds SOME cluster to canonical_label="Marcus".
    C. binding_links_correctly: the resolution's cluster_id matches the chain
       head's cluster_id for (@User, @User.<pred>, "real") at the time of
       binding. (i.e., we didn't create a fresh cluster for Marcus and orphan
       the anonymous one.)
    D. chain_label_resolves: at the END of ingestion, the chain head for
       (@User, @User.<pred>, "real") has cluster_label == "Marcus".
    """
    by_ts: dict[int, list[aen2_binding.LogEntry]] = {}
    for e in log:
        by_ts.setdefault(e.ts, []).append(e)
    res_by_ts: dict[int, list[aen2_binding.Resolution]] = {}
    ...
    for p in gt.pairs:
        ...
        n_turn = p["name_turn"]
        target_pred_str = f"{pred[0]}.{pred[1]}".replace("@@", "@")  # "@User.boss"
        target_subj = pred[0] if pred[0].startswith("@") else f"@{pred[0]}"

        # A. Anon cluster at descriptor (entry near d_turn with the right chain key).
        ...
        # B. Resolution at name (any resolution near n_turn with label==name).
        ...
        # C. Binding links correctly: resolution's cluster_id is in anon_cluster_ids.
        binding_links_correctly = any(
            r.cluster_id in anon_cluster_ids for r in bind_resolutions
        )

        # D. Chain label resolves at end: chain head's cluster has label==name.
        chain_key = (target_subj, target_pred_str, "real")
...
def grade_qa(qs, idx, cache, budget) -> dict:
    answers = {}
    for q in qs:
        a = aen2_binding.answer_question(q.question, idx, cache, budget, top_k=14)
        answers[q.qid] = a
    cache.save()
...
def telemetry_summary(telemetry: list[dict]) -> dict:
    ...
    return {
        ...
        "n_resolutions_total": sum(
            t.get("n_resolutions_emitted", 0) for t in telemetry
        ),
    }
    ...
    *,
    ...
    try:
        ...
        print(
            ...
            f"links_correctly={bind['binding_links_pass']}/{bind['total']} "
...
def save(results: dict, budget: Budget):
    results["budget"] = {
        "cost": budget.cost(),
...
def main():
    ...
    plan = [
        # 2x fudge: w_future=14 covers gap=14; K=3 boost reaches gap=16 (Quentin).
        # Marcus (gap=17) is still marginal but might land via the asymmetric reach.
        ("binding_K3_w7_w14", 7, 14, 3, True),
        # Symmetric natural baseline.
        ("binding_K3_w7_w7", 7, 7, 3, True),
    ]
