"""Round 19 v3 — refined filler-skip prompt to recover hobbies/commute/car/gym.

V2 was too aggressive on filler skip — categorized possessions, hobbies,
and recurring routines as filler. V3 prompt explicitly lists durable LIFE
FACTS vs FILLER with a distinction test: "would User's friend still want to
know this in a month?".

Re-runs multi_batch_coref + dense_chains[:200] + dormant_chains[:200] with
fresh caches (prompt change invalidates cache).
"""
...
def main():
    ...
    plan = [
        ("v3_coref_K3_w7_w7", coref_turns, coref_gt, coref_qs, 7, 7, 3),
        ("v3_dense_K3_w7_w7", dc_turns, dc_gt, dc_qs, 7, 7, 3),
        ("v3_dorm_K3_w7_w7", dorm_turns, dorm_gt, dorm_qs, 7, 7, 3),
    ]
