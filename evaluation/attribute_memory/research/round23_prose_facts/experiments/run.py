"""Round 23 — prose-fact + symmetric mention/entity DSU architecture.

Tests:
  - multi_batch_coref: cross-batch coref via mention merge into shared entity
  - HBR: conditional + outcome via prose facts and entity resolution
  - dorm[:200]: long-gap chain transitions via entity_facts retrieval
  - dense[:200]: many-chain density without explicit chain_head pointer
"""
...
import aen6_prose as ap  # noqa: E402
...
def run_variant(name, *, turns, qs, budget):
    ...
    print(
        f"[ingest] facts={len(facts)} mentions={len(mentions)} "
        f"entities={n_entities} merges={n_merges}"
    )
    out["log_size"] = len(facts)
    out["n_mentions"] = len(mentions)
...
def main():
    # 4 scenarios. Estimate ~280 LLM calls for ingest, ~50 QA + judge = ~330 calls.
    ...
    plan = [
        ("r23_coref", coref_turns, coref_qs),
        ("r23_hbr", hbr_turns, hbr_qs),
        ("r23_dorm", dorm_turns, dorm_qs),
        ("r23_dense", dc_turns, dc_qs),
    ]
