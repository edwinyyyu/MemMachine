"""Round 23 v3 — prose-fact + DSU + trigger-gated cognition pass.

Adds the R22 cognition machinery on top of R23 v2's writer prompt. Cognition
facts go into a separate cognition collection. Retrieval gates by question
type (cognitive questions pull from cognition + observations).

Predicted: matches or exceeds R22 cog_on's 91% by adding +5-7 on dense (cog
confirmation entries anchor current-state) and +1 on HBR (already 5/5 in v2,
so no headroom there).
"""
...
import aen6_prose_v3 as ap  # noqa: E402
...
def run_variant(name, *, turns, qs, budget, enable_cognition=True):
    cache = Cache(CACHE_DIR / f"{name}.json")
    pairs = [(t.idx, t.text) for t in turns]
    print(f"\n=== {name} (n_turns={len(pairs)}, cog={enable_cognition}) ===")
    ...
    try:
        obs_f, obs_m, cog_f, cog_m, store, telemetry = ap.ingest_turns(
            pairs,
            cache,
            budget,
            w_past=7,
    ...
    print(
        f"[ingest] obs={len(obs_f)} cog={len(cog_f)} entities={n_entities} merges={n_merges}"
    )
    out["obs_facts"] = len(obs_f)
    out["cog_facts"] = len(cog_f)
...
def save(results, budget):
    ...
    p = RESULTS_DIR / "run_v3.json"
    p.write_text(json.dumps(results, indent=2, default=str))
    print(f"[checkpoint] {p} cost=${budget.cost():.3f}")


def main():
    # Cognition doubles ingest LLM calls. Bigger budget.
    budget = Budget(max_llm=1000, max_embed=500, stop_at_llm=950, stop_at_embed=470)
    ...
    plan = [
        ("r23v3_coref", coref_turns, coref_qs),
        ("r23v3_hbr", hbr_turns, hbr_qs),
        ("r23v3_dorm", dorm_turns, dorm_qs),
        ("r23v3_dense", dc_turns, dc_qs),
    ]

    for name, turns, qs in plan:
        try:
            res = run_variant(
                name, turns=turns, qs=qs, budget=budget, enable_cognition=True
            )
        except RuntimeError as e:
