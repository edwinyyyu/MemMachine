"""Round 21 — partitioned storage (observations + cognition collections).

Tests:
  1. Regression: cog_off should match v2's 8/8 on multi_batch_coref.
  2. Cognition with partition: does cog_on no longer corrupt observation chains?
  3. HBR: should still hit 5/5; cognition should add value on cognitive Qs.
"""
...
CACHE_DIR = ROUND21 / "cache"
RESULTS_DIR = ROUND21 / "results"
...
def run_variant(name, *, turns, gt, qs, w_past, w_future, k, enable_cognition, budget):
    ...
    try:
        obs_log, cog_log, store, telemetry = ap.ingest_turns(
            pairs,
            cache,
            budget,
            w_past=w_past,
...
def main():
    ...
    plan = [
        # 1. Cog OFF on coref — should match v2's 8/8 (regression check)
        ("coref_part_cog_off", coref_turns, coref_gt, coref_qs, 7, 7, 3, False),
        # 2. Cog ON on coref WITH partitioned storage — should also be 8/8 (no boss swap)
        ("coref_part_cog_on", coref_turns, coref_gt, coref_qs, 7, 7, 3, True),
        # 3. Cog OFF on HBR — baseline
        ("hbr_part_cog_off", hbr_turns, hbr_gt, hbr_qs, 7, 7, 3, False),
        # 4. Cog ON on HBR with partition — should be 5/5
        ("hbr_part_cog_on", hbr_turns, hbr_gt, hbr_qs, 7, 7, 3, True),
    ]
