"""TimeRange: list of disjoint half-open intervals on R̄ (with ±∞ sentinels).

The canonical temporal data type for V7. A TimeRange represents a set of
moments in time as a sorted list of non-overlapping, non-adjacent
half-open intervals `[earliest_us, latest_us)`. Endpoints may be the
sentinel values `NEG_INF`/`POS_INF` to express unbounded extent.
...
class TimeRange:
    ...
    @classmethod
    def from_intervals(cls, ivs: list[Interval] | tuple[Interval, ...]) -> "TimeRange":
        """Build a canonical TimeRange from arbitrary intervals."""
    ...
    @classmethod
    def universal(cls) -> "TimeRange":
        return cls(intervals=(Interval(NEG_INF, POS_INF),))

    @classmethod
    def closed(cls, lo: int, hi: int) -> "TimeRange":
    ...
    @classmethod
    def after(cls, t: int) -> "TimeRange":
        """[t, +∞)."""
        return cls.closed(t, POS_INF)

    @classmethod
    def before(cls, t: int) -> "TimeRange":
        """(-∞, t)."""
...
def union(A: TimeRange, B: TimeRange) -> TimeRange:
    """A ∪ B."""
    return TimeRange.from_intervals(list(A.intervals) + list(B.intervals))


def union_all(ranges: list[TimeRange]) -> TimeRange:
    if not ranges:
        return TimeRange.empty()
    all_ivs: list[Interval] = []
    for r in ranges:
        all_ivs.extend(r.intervals)
    return TimeRange.from_intervals(all_ivs)
...
def intersect(A: TimeRange, B: TimeRange) -> TimeRange:
    ...
    while i < len(a) and j < len(b):
        ...
        if ai.latest_us < bj.latest_us:
            ...
            j += 1
    return TimeRange(intervals=tuple(out))


def intersect_all(ranges: list[TimeRange]) -> TimeRange:
    """Intersect a list of TimeRanges. Empty list → universal."""
    if not ranges:
...
def is_infinite_measure(A: TimeRange) -> bool:
