"""T_full vs T_axis × blind_pair on ALL 4 benchmarks.

Checks whether T_axis's hard_bench advantage is offset by tempreason
regression once blind_pair tuning is added — and whether T_full's robustness
holds up under tuning across distributions.

Single seed for now (5 seeds × 4 benchmarks would be expensive); reports
fixed-best vs blind_pair for each combination.
"""
...
async def main():
    ...
    judge.save()
    out_path = ROOT / "results" / "t_full_vs_axis_all_bench.json"
    ...
    print("\n=== SUMMARY (R@1 for each T variant × {fixed, blind_pair}) ===")
    print(
        f"{'Benchmark':22} {'T_axis fixed':>13} {'T_axis+pair':>13} {'T_full fixed':>13} {'T_full+pair':>13}  ΔPair(axis-full)"
    )
    for bname, r in all_results.items():
        ax_f = r["T_axis"]["fixed"]["r@1"]
        ax_p = r["T_axis"]["blind_pair"]["r@1"]
        fu_f = r["T_full"]["fixed"]["r@1"]
        fu_p = r["T_full"]["blind_pair"]["r@1"]
        delta = ax_p - fu_p
        print(
            f"{bname:22} {ax_f:>13.3f} {ax_p:>13.3f} {fu_f:>13.3f} {fu_p:>13.3f}  {delta:>+15.3f}"
        )
