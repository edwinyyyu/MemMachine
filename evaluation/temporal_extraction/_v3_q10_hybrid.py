"""Q10 — hybrid retrieval: semantic top-K/2 from full corpus ∪ semantic top-K/2
from filter-survivors. Total pool capped at K=10 (same as R-S baseline).

Question: does giving the rerank a mix of "best semantic" and "best
semantic-among-filter-passers" let it discriminate better than either alone?
The fairness control is keeping the rerank pool size the same as R-S/R-SF.

Modes:
    R-S            : semantic top-10
    R-SF           : semantic top-10 from filter-survivors
    R-S5_SF5_K10   : (sem top-5 of all) ∪ (sem top-5 of filter-survivors), cap 10

Scoring: B-RMX (rerank × mask × (1+α·rec)), pool-norm, no tail-fill.
"""
...
HALF = max(1, POOL_CAP // 2)
MODES = ["R-S", "R-SF", "R-S_half_SF_half"]


def build_pool(mode, s_scores, all_dids, eligible_filt):
    if mode == "R-S":
        return semantic_topk_in_set(s_scores, all_dids, POOL_CAP)
    if mode == "R-SF":
        return semantic_topk_in_set(s_scores, eligible_filt, POOL_CAP)
    if mode == "R-S_half_SF_half":
        rs_top = semantic_topk_in_set(s_scores, all_dids, HALF)
        rsf_top = semantic_topk_in_set(s_scores, eligible_filt, HALF)
        pool = list(dict.fromkeys(rs_top + rsf_top))
        # Top up with more R-S if union has < POOL_CAP unique
        # (e.g., filter is no-op and the two halves coincide).
        if len(pool) < POOL_CAP:
            extra = semantic_topk_in_set(s_scores, all_dids, POOL_CAP * 2)
            for d in extra:
                if d not in pool:
                    pool.append(d)
                    if len(pool) >= POOL_CAP:
...
async def run_bench_q10(
    ...
    for q in queries:
        ...
        for mode in MODES:
            pool = build_pool(mode, per_q_s[qid], all_dids, eligible_filt)
    ...
    for q in queries:
        ...
        for mode in MODES:
            ...
            per_mode[mode] = (hit_rank(rank, gold_set), len(pool))

        results.append(
            {
                "qid": qid,
                "gold": list(gold_set),
                **{f"hit_{m}": per_mode[m][0] for m in MODES},
                **{f"poolsize_{m}": per_mode[m][1] for m in MODES},
            }
        )
...
async def main():
    ...
    for nm, dp, qp, gp, cl in benches_def:
        try:
            results = await run_bench_q10(nm, dp, qp, gp, cl, reranker, planner)
            ...
            for m in MODES:
                d = overall[m]
                print(
                    f"  {m:20s}  R@1={d['R@1']:.3f} ({d['r1_count']}/{overall['n']})",
                    flush=True,
                )
    ...
    macro = {
        m: sum(out["benches"][b]["overall"][m]["R@1"] for b in valid)
        / max(1, len(valid))
        for m in MODES
    }
    print(f"\nMacro R@1 across {len(valid)} benches:")
    for m in MODES:
        print(f"  {m:20s}: {macro[m]:.3f}")
    out["macro"] = macro

    out_dir = ROOT / "results"
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / f"T_q10_hybrid_K{POOL_CAP}_topup.json"
