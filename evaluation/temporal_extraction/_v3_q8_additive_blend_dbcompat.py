"""Q8 — recreate corpus-T-fusion's *additive blend* structure in a fully
DB-compatible form. Q7's multiplicative boost didn't recover the +0.018 gap;
the structural difference is that corpus-T-fusion does additive blend
between T and R with corpus-CV-gated weights and corpus-wide min-max
normalization.

DB-compatible recreation:
  - corpus-wide T_max: max raw lattice score across the matched-doc set
    returned by `lattice_retrieve_multi(...)`. (Corpus docs not in matched
    have T = 0; their max contribution is 0; T_max = max of matched values.)
  - corpus-wide CV: computed from matched-doc scores + n_total via
    sum_s/n_total, sum_s²/n_total - mean², no full corpus scan.
  - per-doc T_norm = raw_lat / T_max (zero for non-matched).
  - per-pool R_norm = rerank / max(rerank in pool).
  - fused = w_T · gate · T_norm + w_R · R_norm   (additive blend)
    Default w_T=0.4, w_R=0.6 (matches `score_blend`).
  - base = normalize_dict(fused) over pool only.
  - final = base × mask × (1 + α·rec).

Variants:
  B-RMX               : baseline (no lattice)
  B-add(0.2/0.8)      : low T weight
  B-add(0.4/0.6)      : matches corpus-T-fusion's defaults
  B-add(0.6/0.4)      : high T weight

Optionally with IDF tag-weighting per matched tag (Option 1 ⊕ Option 2).
"""
...
# Additive blend (w_T, w_R) pairs to test. (0, 1) means no lattice (baseline).
W_PAIRS = [
    (0.0, 1.0),
    (0.2, 0.8),
    (0.4, 0.6),
...
def fused_additive(
    ...
    """Additive blend `fused = w_T · gate · T_norm + w_R · R_norm` per pool doc.

    T_norm = raw / T_max (0 if no match).
    R_norm = rerank / R_max in pool.
    """
    r_max = max(rerank_pool.values()) if rerank_pool else 0.0
    fused = {}
    for did in pool:
        if t_max_corpus > 1e-12:
            t_norm = raw_lat_corpus.get(did, 0.0) / t_max_corpus
        else:
            t_norm = 0.0
        if idf_factor is not None:
            t_norm *= idf_factor.get(did, 1.0)
        r_norm = rerank_pool.get(did, 0.0) / r_max if r_max > 0 else 0.0
        fused[did] = w_T * gate_T * t_norm + w_R * r_norm
    return fused


def lattice_idf_factor_per_doc(
    pool_dids,
    lat: LatticeStore,
    expanded_query_tags,
    idf_map,
):
    """Per-doc IDF factor: max IDF among the doc's matched-query tags.

    Returns 1.0 (no boost) for docs with no matched tag — this means IDF
    only modulates docs that have a lattice match. Normalized by global
    max IDF so the multiplier is in [0, 1]."""
    matched = lat.query_by_tags(expanded_query_tags.keys())
    if not idf_map:
        return dict.fromkeys(pool_dids, 1.0)
    max_idf_global = max(idf_map.values())
    if max_idf_global <= 0:
        return dict.fromkeys(pool_dids, 1.0)
    out = {}
    for did in pool_dids:
        m = matched.get(did, set())
        if not m:
            out[did] = 1.0
            continue
        out[did] = max(idf_map.get(t, 0.0) for t in m) / max_idf_global
    return out


async def run_bench_q8(
    name,
    docs_path,
    queries_path,
    gold_path,
    ...
    lat_db = ROOT / "cache" / "composition_v3_q8" / f"lat_{name}_K{POOL_CAP}.sqlite"
    ...
    print(f"  pool (mode=R-S, K={POOL_CAP}) and reranking...", flush=True)
    per_q_pool = {}
    per_q_r_full = {}
    for q in queries:
        qid = q["query_id"]
    ...
    for q in queries:
        ...
        idf_factor = lattice_idf_factor_per_doc(
            pool,
            lat,
        ...
        for w_T, w_R in W_PAIRS:
            for use_idf in (False, True):
                fused = fused_additive(
                    ...
                    w_T,
                    w_R,
                    idf_factor=(idf_factor if use_idf else None),
                )
                ...
                key = f"wT{w_T:.1f}_wR{w_R:.1f}_idf{int(use_idf)}"
        ...
        results.append(
            {
                "qid": qid,
                "gold": list(gold_set),
                "cv_gate": per_q_cv[qid],
                "t_max": per_q_t_max[qid],
...
async def main():
    ...
    configs = [
        f"wT{w_T:.1f}_wR{w_R:.1f}_idf{int(use_idf)}"
        for (w_T, w_R) in W_PAIRS
        for use_idf in (False, True)
    ]

    out = {"benches": {}, "configs": configs, "POOL_CAP": POOL_CAP}
    for nm, dp, qp, gp, cl in benches_def:
        try:
            results = await run_bench_q8(nm, dp, qp, gp, cl, reranker, planner)
            ...
            print(f"  {'config':22s}  R@1", flush=True)
            for c in configs:
                d = overall[c]
                print(f"  {c:22s}  {d['R@1']:.3f}", flush=True)
    ...
    for c in configs:
        print(f"  {c:22s}: {macro[c]:.3f}")
    out["macro"] = macro

    out_dir = ROOT / "results"
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / f"T_q8_additive_blend_K{POOL_CAP}.json"
