"""Adversarial re-eval with v2' extractor (ship-best).

Based on adversarial_eval.py but:
- Uses ExtractorV2P (v2') instead of ExtractorV2.
- Separate cache dir ``cache/adversarial_v2p/``.
- Same scoring pipeline (multi-axis alpha/beta/gamma + anchor dual-score
  + Allen routing + semantic rerank), so results isolate the extractor change.
"""
...
CACHE_DIR = ROOT / "cache" / "adversarial_v2p"
...
async def run_v2p_extract(items, label):
    ex = ExtractorV2P(concurrency=8)
    # point pass1 cache at the adversarial v2p-specific subdir
    from extractor_common import LLMCache

    ex.cache = LLMCache(CACHE_DIR / "extractor_v2p_pass1" / "llm_cache.json")
    ...
    async def one(iid, text, ref):
        try:
            ...
            print(f"  [{label}] v2p TIMEOUT for {iid}")
            return iid, []
        except Exception as e:
            print(f"  [{label}] v2p failed for {iid}: {e}")
            return iid, []
    ...
    print(f"  [{label}] v2p usage: {ex.usage}")
...
async def main() -> None:
    ...
    v2p_docs, u1 = await run_v2p_extract(doc_items, "docs-v2p")
    v2p_qs, u2 = await run_v2p_extract(q_items, "queries-v2p")
    ...
    doc_ext = {
        d["doc_id"]: merge_tes(
            v2p_docs.get(d["doc_id"], []), era_docs.get(d["doc_id"], [])
        )
        for d in docs
    }
    q_ext = {
        q["query_id"]: merge_tes(
            v2p_qs.get(q["query_id"], []), era_qs.get(q["query_id"], [])
        )
        for q in queries
    }
    ...
    (RESULTS_DIR / "adversarial_v2p.json").write_text(
    ...
    baseline = {}
    try:
        b_data = json.loads((RESULTS_DIR / "adversarial.json").read_text())
        baseline = {
            "overall": b_data.get("overall", {}),
            "per_category": b_data.get("per_category", {}),
            "extraction_signals": b_data.get("extraction_signals", {}),
        }
    except Exception as e:
        print(f"  (could not load v2 baseline: {e})")

    # Markdown
    lines: list[str] = []
    lines.append("# Adversarial Re-Eval with v2' (v2-prime) Extractor\n\n")
    lines.append(
        f"Corpus: {len(docs)} docs, {len(queries)} queries. Wall: {wall_s:.1f}s. LLM cost: ${cost_usd:.4f}.\n\n"
    )

    lines.append("## Per-category — v2' vs v2 baseline\n\n")
    lines.append(
    ...
    for cat in sorted(per_cat):
        ...
        b_m = baseline.get("per_category", {}).get(cat, {})
        b_e = baseline.get("extraction_signals", {}).get(cat, {})
        ...
        dr5 = (m["recall@5"] - b_m.get("recall@5", 0.0)) if b_m else float("nan")
        lines.append(
            f"| {cat} | {m['n']} | {_fmt(b_m.get('recall@5'))} | **{_fmt(m['recall@5'])}** | "
            f"{_fmt(dr5)} | {_pct(b_e.get('emit_rate'))} | **{_pct(e.get('emit_rate'))}** | "
            f"{_pct(e.get('avg_tes_per_doc'))} | {cs_s} |\n"
        )

    lines.append(
        f"\n**Overall v2' R@5**: {overall['recall@5']:.3f} "
        f"(v2 baseline {baseline.get('overall', {}).get('recall@5', 0):.3f}, "
        f"Δ {overall['recall@5'] - baseline.get('overall', {}).get('recall@5', 0):+.3f})\n"
    )
    lines.append(
        f"**Overall v2' R@10**: {overall['recall@10']:.3f}, MRR: {overall['mrr']:.3f}, NDCG@10: {overall['ndcg@10']:.3f}\n\n"
    )

    lines.append("## Extraction-level diagnosis\n\n")
    lines.append(
        "Per-category emit rate and TEs/doc: how often does v2' produce SOMETHING vs v2?\n\n"
    )
    still_zero_emit = [
        c
        for c, e in extraction_signals.items()
        if e.get("emit_rate", 0.0) == 0.0 and c != "A7"
    ]
    if still_zero_emit:
        lines.append(
            f"**Categories with 0% emit rate even under v2'**: {still_zero_emit}\n\n"
        )
    else:
        lines.append("**All non-A7 categories now emit at least some TEs.**\n\n")

    lines.append("## Top failure examples (after v2')\n\n")
    sorted_failures = sorted(failure_examples, key=lambda f: (f.get("qid", ""),))[:15]
    ...
    lines.append("## Extraction sample (first 25 docs)\n\n")
    for e in doc_ext_summary[:25]:
        lines.append(
            f"- **{e['doc_id']}** ({e['category']}) `{e['text'][:80]}` -> "
            f"{e['n_tes']} TEs (surfaces={e['surfaces']}, kinds={e['kinds']})\n"
        )
    ...
    (RESULTS_DIR / "adversarial_v2p.md").write_text("".join(lines))
    print("\nWrote results/adversarial_v2p.{md,json}")
    print(
        f"v2' overall: R@5={overall['recall@5']:.3f} (v2 was {baseline.get('overall', {}).get('recall@5', 0):.3f})"
    )
