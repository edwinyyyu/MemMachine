...
Deterministic helpers that compose with the LLM extractor. The
extractor returns `Interval(earliest_us, latest_us)` per temporal
reference; all scoring (mask, filter, recency) operates on these
...
def excluded_containment(d_ivs: list[Interval], excl_ivs: list[Interval]) -> float:
    ...
    the doc has other intervals far from the exclusion. An aggregate
    alternative (sum over union, total-overlap fraction) was tested and
    lost on the discriminating bench — strict is the production
    semantics here.
    """
