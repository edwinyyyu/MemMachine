...
Identifies every temporal reference in a passage and resolves each
into an earliest/latest envelope on the calendar. Skip-don't-emit
semantics for non-anchorable phrases — empty output means "no
calendar reference here", not "low confidence". The retrieval
pipeline scores only on earliest/latest, so no other fields are
emitted.
