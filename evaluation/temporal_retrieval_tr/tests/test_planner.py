"""Unit tests for QueryPlanner's JSON → IntervalSet conversion.
...
from temporal_retrieval_tr import NEG_INF, POS_INF, intersect, is_empty
...
# ---------------------------------------------------------------------------
# JSON → list[IntervalSet] conversion
# ---------------------------------------------------------------------------


def test_json_to_targets_simple() -> None:
    """{lo: 2024-03-01, hi: 2024-04-01} → one bounded target."""
    json_targets = [{"intervals": [{"lo": "2024-03-01", "hi": "2024-04-01"}]}]
    targets = _json_to_targets(json_targets)
    assert len(targets) == 1
    iv = targets[0].intervals[0]
    assert iv.earliest_us == us(2024, 3, 1)
    assert iv.latest_us == us(2024, 4, 1)


def test_json_to_targets_unbounded_via_null() -> None:
    """{lo: null, hi: 2024-01-01} → (-∞, 2024)."""
    json_targets = [{"intervals": [{"lo": None, "hi": "2024-01-01"}]}]
    targets = _json_to_targets(json_targets)
    iv = targets[0].intervals[0]
...
def test_json_to_targets_complement_two_pieces() -> None:
    """Complement of [2023] → one target with two intervals."""
    json_targets = [{
        "intervals": [
...
def test_json_to_targets_multi_period_three_targets() -> None:
...
def test_json_to_targets_two_period_or() -> None:
    """Two disjoint periods (e.g., Q1 or Q4 2023) → two targets."""
    json_targets = [
...
def test_json_to_targets_drops_invalid_intervals() -> None:
    """lo >= hi intervals silently dropped."""
    json_targets = [{"intervals": [
        {"lo": "2024-03-01", "hi": "2024-04-01"},   # valid
        {"lo": "2024-05-01", "hi": "2024-04-01"},   # invalid (hi <= lo)
    ]}]
    targets = _json_to_targets(json_targets)
    assert len(targets[0].intervals) == 1


def test_json_to_targets_skips_empty() -> None:
    """If a target ends up with no intervals (all invalid), drop the target."""
    json_targets = [{"intervals": [
...
def test_json_to_targets_complement_membership() -> None:
    """'in 2024 not in summer' — one target, two intervals. Doc in March passes,
    doc in July excluded (intra-target OR via multi-interval)."""
    from temporal_retrieval_tr import IntervalSet
    json_targets = [{"intervals": [
    ...
    targets = _json_to_targets(json_targets)
    qtarget = targets[0]
    march = IntervalSet.closed(us(2024, 3, 1), us(2024, 4, 1))
    july = IntervalSet.closed(us(2024, 7, 1), us(2024, 8, 1))
    assert not is_empty(intersect(qtarget, march))
    assert is_empty(intersect(qtarget, july))
