"""A/B: cleaned ship (neutral examples) vs cleaned ship + v3.7 extractor.

Tests whether v3.7's three-mode handling (pure recurring / era-anchored
/ specific occurrence with recurring descriptor) fixes the v3.6
regressions on negation_temporal, era, disc while preserving the
adversarial/goldilocks gains.

Also tests with the NEUTRAL-EXAMPLES planner (PROMPT_VERSION = v5),
which removed bench-mirroring queries like "What do I do on Thursdays?"
and "When did I most recently cook tonkotsu ramen?".
"""
...
from temporal_retrieval_tr.research._extractor_v3_7_surgical import (
    TemporalExtractorV3_7,
)
...
async def run_bench(bench, embed_fn, rerank_fn):
    ...
    # A = cleaned ship: planner v5-neutral + v3.3 extractor
    ...
    # B = cleaned planner + v3.7 surgical extractor
    vd_b = TemporalRetriever(embed_fn=embed_fn, rerank_fn=rerank_fn,
                             extractor=TemporalExtractorV3_7())
...
async def main():
    ...
    print(f"\n=== Cleaned ship + v3.7 surgical extractor vs cleaned ship "
          f"over {len(BENCH_NAMES)} benches ===\n", flush=True)
    hdr = (f"{'bench':30s}  {'ship R@1':>9s} {'v3.7 R@1':>9s} {'ΔR@1':>7s}  "
           f"{'ship R@5':>9s} {'v3.7 R@5':>9s}  {'n':>4s}")
