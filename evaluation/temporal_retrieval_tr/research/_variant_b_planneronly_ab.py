"""A/B: Variant B planner ONLY (with current v3.3 extractor) vs current ship.

Tests whether the planner-side recurring rule is sufficient, given that
frac-min scoring gives full score for any overlap (60s doc anchor in a
full-day query interval = 1.0).

If this matches the combined-with-v3.5 results, we ship the planner
change alone and skip the extractor surgery.
"""
...
async def run_bench(bench, embed_fn, rerank_fn):
    ...
    # A = current ship: NoAnaphoraPlanner + v3.3 extractor
    ...
    # B = variant B planner ONLY + v3.3 extractor (default)
...
async def main():
    ...
    print(f"\n=== Variant B planner ONLY (with v3.3 extractor) vs current ship "
