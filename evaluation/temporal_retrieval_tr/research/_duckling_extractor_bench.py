"""A/B: Duckling-HTTP extractor vs LLM extractor.

Requires a running Duckling HTTP server:
    docker run -d -p 8000:8000 --name duckling-temp rasa/duckling
...
from temporal_retrieval_tr.research._duckling_extractor import (
...
async def main():
    ...
    print("\n=== Duckling-HTTP EXTRACTOR + LLM planner ===\n", flush=True)
    rows = []
    ...
    print(f"\nMACRO duckling-extr + LLM-plan:    R@1={r1:.4f}  R@5={r5:.4f}  n={n}",
          flush=True)
    print("MACRO dateparser-extr + LLM-plan:  R@1=0.7946  R@5=0.9492",
          flush=True)
