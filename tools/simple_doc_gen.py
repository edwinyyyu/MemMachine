        ...
        if current_section == "description":
            ...
            if match:
                ...
                sections["args"].append(
                    {"name": name, "type": type_ or "", "desc": desc}
                )
            elif sections["args"]:
...
def format_mdx_method(name, args, docstring_info, is_class=False):
    ...
    for arg in args:
        ...
        doc_arg = next(
            (a for a in docstring_info["args"] if a["name"] == arg_name), None
        )
...
def generate_markdown(file_path):
    ...
    for node in tree.body:
        if isinstance(node, ast.ClassDef):
            ...
            md_output.append(
                format_mdx_method(node.name, init_args, doc_info, is_class=True)
            )
            ...
            for item in node.body:
                if isinstance(item, ast.FunctionDef):
                    ...
                    md_output.append(
                        format_mdx_method(item.name, item.args.args, method_info)
                    )
