...
def _build_vector_store(config: MemoryConfig, vector_engine: AsyncEngine) -> VectorStore:
...
def _solve_demotion_alpha(
    base: np.ndarray, query: np.ndarray, target: float
) -> float:
...
def in_context_exclusion_filter(
    ...
    return (
        f"{not_this_session} OR timestamp < date({_filter_str(compaction_cutoff.isoformat())})"
    )
...
class MemoryCore:
    ...
    async def demote(self, memory_id: str, cue: str) -> DemoteResult:
        ...
        query = _l2norm(
            np.asarray(
                (await self.stores.embedder.search_embed([cue]))[0], dtype=float
            )
        )
        ...
        for derivative_uuid, d_orig in zip(derivative_uuids, d_origs, strict=False):
            ...
            new_delta = delta + _solve_demotion_alpha(d_orig - delta, query, target) * query
            if float(np.linalg.norm(new_delta)) > _DEMOTE_MAX_NORM:
                before = max(before_values, default=0.0)
                return DemoteResult(
                    False, "saturated", _SATURATED_MESSAGE, memory_id, cue, before, before
                )
...
# --- wire (de)serialization for the daemon protocol ---
