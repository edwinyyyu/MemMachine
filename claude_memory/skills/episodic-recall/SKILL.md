...
No tool takes a session id. The list of recent conversations shown at the start of a session
gives each one's *first* handle, which keeps working as that conversation grows.
...
A **cue** is the text a search is matched against — see [Writing cues](#writing-cues), which is
where most of the difference between a good and a useless search lies.

| tool | answers |
|---|---|
| `memory_search(cue, …)` | *which* memory — associative recall |
...
`memory_expand` returns a **window**: the seed plus the events around it, from that
conversation only. It cannot cross into another conversation.

**`unit` sets what `before`/`after` count.**

- `unit="segments"` (default) counts chunks. Every call costs about the same, and this is the
  only way to read *inside* one long event.
- `unit="events"` counts whole events, so one enormous event costs the same as one short one.
  Use it when the length of what sits in the way should not decide how far the window reaches.
...
Naming any `kinds` replaces the default outright. That default excludes `injected` — hook
context, skill bodies, system reminders, slash-command echoes and the session's own compaction
summary. That text is genuinely what the session saw, but it arrives in long runs, so a default
window landing in one would be entirely boilerplate.

### When the window comes back short

Two different causes, opposite remedies. The window itself shows which, so read it before
changing any argument:

- **Something bulky filled it** — an elision marker (below), or a wall of one kind; a single
  tool result can run to a thousand segments. Raising `before`/`after` just buys more of the
  same material. Exclude that kind, or switch to `unit="events"`.
- **One message is longer than the window** — a single unbroken stretch of one speaker, no
  marker, no one else. The window has not left that message yet and there is genuinely more of
  it. Raise `after`. Nothing under ~4,000 characters is elided, so an ordinary long message can
  fill a segment window without producing any marker at all.

### Events too large to show whole

Over ~4,000 characters — a pasted document rather than a written message — an event is shown as
its first and last segments with an **elision marker** between them:
...
Nothing is silently cut: the character count is measured, and the handles in the marker are
usable seeds. To read the middle, expand from one of them with `unit="segments"`. Often the two
ends are enough to show that reading further is pointless, as above.
...
outside source, or a claim revised a few turns later. Formatting confers no authority —
emphasis marks the sentence someone cared about, which is as often the one later corrected.
Before restating a number, an attribution or a decision, establish three things:
...
These are separate decisions with opposite economics.

**Search on plausible bearing.** A search costs one call, and whether it was worth it cannot be
known before reading the result. Search when the answer depends on decisions already taken,
when a request builds on earlier work, or when a recommendation would change given what was
already tried, rejected or measured.
...
- `kinds=["user_message"]` / `["assistant_message"]` — helps ranking, guarantees nothing. When
  a user pastes assistant output back in, that text exists under both speakers; doubly-escaped
...
- `within=<mem:id>` — confine the search to the conversation that handle came from. Any handle
  from it will do. Use this to search inside one conversation; use `memory_outline` when the
  question is *where* rather than *what*.
...
the cue was, conclude the memory does not exist rather than reformulating indefinitely.

## Working through several hops

1. Search with an event-describing cue, narrowed by `kinds`, `within` or `since`/`before` where
   the speaker, conversation or date is known.
2. Expand the best result. If its content will be restated, expand to the sandwich, forward
   first.
3. Continue from the edge handles each expansion returns. `memory_outline(id)` is cheaper when
   the question is *where* in a conversation rather than *what*: one line per user turn, with
   how many events followed each — a turn followed by sixty is where the work happened.
   `before`/`after` count turns there.
4. If the target will not surface directly, search its *surroundings* — what was being
   discussed, roughly when — and expand from a neighbour.
5. For a claim that will be acted on, add one search on its status.
6. Stop after about two cue rewrites.
