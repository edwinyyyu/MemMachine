"""KMS clients backed by HashiCorp Vault/OpenBao Transit."""
...
class VaultTransitKMSCryptoClientParams(BaseModel):
    ...
    @classmethod
    def _normalize_mount_point(cls, value: str) -> str:
...
class VaultTransitKMSCryptoClient(KMSCryptoClient):
    ...
    def __init__(self, params: VaultTransitKMSCryptoClientParams) -> None:
        """Initialize the crypto client with the provided parameters."""
...
class VaultTransitKMSKeyAdminClientParams(BaseModel):
    """
    Parameters for VaultTransitKMSKeyAdminClient.
    ...
    Attributes:
        ...
        client_kwargs (dict[str, Any]):
            Keyword arguments forwarded to `hvac.Client`.
        mount_point (str):
            Transit mount point to use for admin operations.
    """
    ...
    mount_point: str = Field(
        default=DEFAULT_MOUNT_POINT,
        description="Transit mount point to use for admin operations",
        min_length=1,
    )
...
class VaultTransitKMSKeyAdminClient(KMSKeyAdminClient):
    """KMS key admin client backed by HashiCorp Vault/OpenBao Transit."""

    def __init__(self, params: VaultTransitKMSKeyAdminClientParams) -> None:
        """Initialize the admin client with the provided parameters."""
        self._client = hvac.Client(*params.client_args, **params.client_kwargs)
        self._mount_point = params.mount_point
    ...
    async def create_key(self) -> str:
        key_ref = str(uuid4())
        await to_thread(self._create_key_sync, key_ref)
        return key_ref

    def _create_key_sync(self, key_ref: str) -> None:
        with self._client_lock:
    ...
    async def exists_key(self, key_ref: str) -> bool:
        return await to_thread(self._exists_key_sync, key_ref)

    def _exists_key_sync(self, key_ref: str) -> bool:
        with self._client_lock:
            try:
                self._transit.read_key(
                    name=key_ref,
                    mount_point=self._mount_point,
                )
    ...
    async def delete_key(self, key_ref: str) -> None:
        await to_thread(self._delete_key_sync, key_ref)

    def _delete_key_sync(self, key_ref: str) -> None:
        with self._client_lock:
            self._transit.update_key_configuration(
                name=key_ref,
                deletion_allowed=True,
                mount_point=self._mount_point,
            )
            self._transit.delete_key(
                name=key_ref,
                mount_point=self._mount_point,
            )
