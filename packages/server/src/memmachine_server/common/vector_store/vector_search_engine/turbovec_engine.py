...
turbovec stores TurboQuant-compressed vectors (2-, 3-, or 4-bit), not the
original floats, so search scores are approximate and the original vectors cannot be
recovered -- ``get_vectors`` therefore raises ``NotImplementedError``.

Notes:
    - The index is inner-product only; ``COSINE`` is served by L2-normalizing
      vectors on add and query, ``DOT`` by passing them through unchanged.
    - ``num_dimensions`` must be a positive multiple of 8 and keys must be
      non-negative (they are stored as ``uint64``); turbovec enforces both.
"""
...
class TurboVecVectorSearchEngine(VectorSearchEngine):
    """Vector search engine backed by turbovec's quantized inner-product index.

    Vectors are stored quantized, so scores are approximate and the original
    vectors cannot be recovered.
    """
    ...
    def _prepare_vectors(self, vectors: Sequence[Sequence[float]]) -> np.ndarray:
        """Cast to float32 and, for cosine, L2-normalize (the index uses inner product)."""
    ...
    def _sync_search(
        ...
        # Filtering happens after retrieval, so overfetch and grow the fetch
        # window until each query has ``limit`` allowed matches or the index is
        # exhausted (a row shorter than the requested fetch means no more exist).
        return [
