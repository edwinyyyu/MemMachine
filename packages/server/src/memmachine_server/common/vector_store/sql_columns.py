...
each indexed with the partition incarnation ahead of it, so a filter is an
indexed comparison within one partition rather than a scan over serialized
properties. Column names are prefixed so a declared key can never collide
with the table's own columns.
...
def property_indexes(
    ...
    """One index per declared key over (incarnation, column), named after the table and the column."""
    return [
        Index(
            f"{table.name}__in_{property_column_name(key)}",
            table.c.incarnation,
