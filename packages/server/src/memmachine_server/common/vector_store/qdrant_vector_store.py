...
class QdrantVectorStore(VectorStore):
    ...
    minted by the registry. The registry is `SqlPartitionRegistry` in the
    deployment's relational database, shared by every Qdrant store there:
    it arbitrates creation and deletion across processes, which Qdrant,
    with no transactions or unique constraints, cannot.
    """
    ...
    async def _ensure_native_collection(self) -> None:
        ...
        try:
            ...
            await self._client.create_payload_index(
                collection_name=self._collection,
                field_name=_PAYLOAD_INCARNATION,
