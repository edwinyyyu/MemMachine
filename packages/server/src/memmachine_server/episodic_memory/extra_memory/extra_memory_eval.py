"""Extra memory system for retrieval evaluation (no reranking or limiting)."""
...
class RankedSegmentContext(BaseModel):
    """A seed segment and its surrounding context, in vector-search order."""

    rank: int
    seed_segment: Segment
    segment_context: list[Segment]


class QueryResult(BaseModel):
    """Result of a retrieval-evaluation query."""

    ranked_segment_contexts: list[RankedSegmentContext]
    segments_per_derivative: dict[UUID, int]


class ExtraMemoryParams(BaseModel):
    """
    Parameters for ExtraMemory (evaluation variant).
...
class ExtraMemory:
    """Extra memory system (evaluation variant, no reranking)."""
    ...
    async def query(
        ...
        *,
        ...
        Returns all matched segment contexts in vector-search similarity order,
        without reranking or limiting.

        Args:
            query (str):
                The search query.
            max_num_segments (int):
                Controls the vector search limit
                (default: 20).
            expand_context (int):
        ...
        Returns:
            QueryResult:
                Query results containing segment contexts in vector-search order.
        ...
        # Build results, preserving similarity ordering from seed_segments.
        # Deduplicate by UUID (multiple derivatives can map to the same segment).
        seen: set[UUID] = set()
        ranked_segment_contexts: list[RankedSegmentContext] = []
        for seed_segment in seed_segments:
            if seed_segment.uuid in seen:
                continue
            if seed_segment.uuid not in segment_contexts_by_seed:
                continue
            seen.add(seed_segment.uuid)
            ranked_segment_contexts.append(
                RankedSegmentContext(
                    rank=len(ranked_segment_contexts),
                    seed_segment=seed_segment,
                    segment_context=segment_contexts_by_seed[seed_segment.uuid],
                )
            )

        segments_per_derivative = {
            derivative_uuid: len(segments)
            for derivative_uuid, segments in segments_by_derivatives.items()
        }

        return QueryResult(
            ranked_segment_contexts=ranked_segment_contexts,
            segments_per_derivative=segments_per_derivative,
        )
