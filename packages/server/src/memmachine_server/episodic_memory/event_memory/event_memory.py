...
class EventMemory:
    ...
    def _build_derivative_record(
        ...
        vector store declares. Every other property stays on the segment:
        the segment store holds them all and evaluates the whole filter
        (see `_vector_store_filter`); it also owns which segment a
        derivative belongs to, so the segment uuid is not copied here.
        """
    ...
    async def _query(
        ...
        *,
        ...
        # The vector store gets the conjuncts it can evaluate; the segment
        # store gets the whole filter below.
        collection_filter = (
