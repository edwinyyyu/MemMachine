...
async def test_open_after_create_and_after_close(
    ...
    await manager.create_session(session_key, mock_episodic_memory_conf, "", {})
...
async def test_open_deleted_session_raises_error(
    ...
    # Re-creating a session that is being deleted is refused too.
...
async def test_storage_is_created_with_the_session_row_and_only_then(
    ...
    with patch(
        ...
        # A re-create is refused before any storage is touched; opening never
        # creates storage.
        with pytest.raises(SessionAlreadyExistsError):
            await manager.create_session("s1", mock_episodic_memory_conf, "", {})
