...
@pytest.mark.asyncio
async def test_the_projects_properties_schema_is_declared_on_its_collection(
    stores, resource_manager
):
    """The collection is created with the system keys and the project's own."""
    _segment_store, vector_store = stores
    conf = _EVENT_CONF.model_copy(
        update={"properties_schema": {"customer_tier": "str", "score": "int"}}
    )
    key = partition_key_for_session("sess_1")
    ...
    schema = collection.config.indexed_properties_schema
    assert schema["customer_tier"] is str
    assert schema["score"] is int
    assert "_timestamp" in schema
    assert isinstance(params, EventBackendParams)
    assert params.user_property_keys == frozenset({"customer_tier", "score"})
