...
async def test_segment_store_purge_loop_ticks_and_survives_failures(
    ...
    task = asyncio.create_task(
        resource_manager_module._purge_forever(
            store.purge_deleted_partitions, "Segment store s"
        )
    )
...
async def test_get_vector_store_starts_one_sweeper_per_store(
    ...
    """The first hand-out of a vector store starts its purge loop; close() ends it."""
    monkeypatch.setattr(resource_manager_module, "_PURGE_INTERVAL_SECONDS", 3600)
    store = create_autospec(VectorStore, instance=True)
    store.purge_deleted_collections.return_value = False
    invalid_resource_manager._database_manager.get_vector_store = AsyncMock(
        return_value=store
    )
    ...
    assert set(invalid_resource_manager._vector_store_purge_tasks) == {"vs"}
    assert store.purge_deleted_collections.await_count == 1
    task = invalid_resource_manager._vector_store_purge_tasks["vs"]

    await invalid_resource_manager.close()

    assert task.cancelled()
    with pytest.raises(ResourceManagerClosedError):
        await invalid_resource_manager.get_vector_store("other")
