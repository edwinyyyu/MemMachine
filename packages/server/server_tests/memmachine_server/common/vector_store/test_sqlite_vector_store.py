...
class TestEngineAccess:
    ...
    async def test_a_reader_never_sees_a_rewrite_half_done(self, tmp_path):
        ...
        await store.startup()
        try:
            collection = await _open_or_create(
                store,
                namespace=NAMESPACE,
                name=NAME,
                config=VectorStoreCollectionConfig(vector_dimensions=VECTOR_DIM),
...
class TestOneLockPerCollection:
    ...
    async def test_two_handles_on_one_collection_are_serialized(self, tmp_path):
        ...
        try:
            ...
            deleter = await store.get_partition(namespace=NAMESPACE, name=NAME)
...
class TestConcurrentWrites:
    ...
    async def test_racing_creates_admit_exactly_one(self, store):
        ...
        attempts = [
            store.create_partition(namespace=NAMESPACE, name="raced", config=CONFIG)
            for _ in range(4)
        ]
