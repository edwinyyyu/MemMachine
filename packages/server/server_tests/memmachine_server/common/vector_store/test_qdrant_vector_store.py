...
class TestUpsertAndQuery:
    ...
    async def test_upsert_and_query_basic(self, collection):
        ...
        results = list(next(iter(await collection.query(query_vectors=[v1], limit=3))))
    ...
    async def test_query_with_similarity_threshold(self, collection):
        ...
        results = list(
            next(iter(await collection.query(query_vectors=[v1], score_threshold=0.9)))
        )
    ...
    async def test_query_with_limit(self, collection):
        ...
        results = list(
            next(iter(await collection.query(query_vectors=[vectors[0]], limit=2)))
        )
    ...
    async def test_query_with_limit_none(self, collection):
        ...
        results = list(
            next(iter(await collection.query(query_vectors=[vectors[0]], limit=None)))
        )
    ...
    async def test_query_return_vector_false(self, collection):
        ...
        results = list(
            next(iter(await collection.query(query_vectors=[v1], return_vector=False)))
        )
        assert len(results) == 1
    ...
    async def test_query_return_properties_false(self, collection):
        ...
        results = list(
            next(
                iter(
                    await collection.query(query_vectors=[v1], return_properties=False)
                )
            )
        )
        assert len(results) == 1
    ...
    async def test_query_batch_multiple_vectors(self, collection):
        ...
        results_for_v1 = list(all_results[0])
        results_for_v2 = list(all_results[1])
        assert results_for_v1[0].record.uuid == r1.uuid
        assert results_for_v2[0].record.uuid == r2.uuid
...
class TestFilters:
    ...
    async def _query(self, collection, query_vec, field, op, value):
        ...
        return {r.record.uuid for r in all_results[0]}
            ...
            next(
                iter(
                    await collection.query(
                        query_vectors=[v1],
                        property_filter=Not(
                            expr=Comparison(field="age", op=">", value=30)
                        ),
                    )
                )
            )
...
class TestGet:
    ...
    async def test_get_return_properties_false(self, collection):
        ...
        results = list(
            await collection.get(record_uuids=[r1.uuid], return_properties=False)
        )
        assert len(results) == 1
