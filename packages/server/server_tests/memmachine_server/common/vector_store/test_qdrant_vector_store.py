...
class TestStrictMode:
    """Both collections are created in strict mode: a filter on an unindexed key is refused."""

    @pytest.mark.asyncio
    async def test_both_collections_are_strict(self, qdrant_client):
        store = QdrantVectorStore(
            QdrantVectorStoreParams(
                collection=COLLECTION,
                vector_dimensions=VECTOR_DIM,
        ...
        await store.provision()
        await store.startup()
        for collection_name in (COLLECTION, store._registry_collection_name):
            info = await qdrant_client.get_collection(collection_name)
            assert info.config.strict_mode_config is not None, collection_name
            assert info.config.strict_mode_config.enabled is True, collection_name
            assert (
                info.config.strict_mode_config.unindexed_filtering_retrieve is False
            ), collection_name
    ...
    @pytest.mark.asyncio
    async def test_a_predicate_of_another_type_matches_nothing(self, qdrant_client):
...
class TestStrictModeIsRequested:
    ...
    @pytest.mark.asyncio
    async def test_provision_creates_both_collections_strict(self, monkeypatch):
        ...
        assert set(requested) == {COLLECTION, store._registry_collection_name}
