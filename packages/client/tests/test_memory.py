...
from memmachine_client.models import AddMemoryResult, EpisodeType, SearchResult
...
class TestMemory:
    ...
    def test_dict_to_filter_string_with_non_string_value(self, mock_client):
        ...
        with pytest.raises(TypeError, match="All filter_dict values must be strings"):
            memory._dict_to_filter_string({"rating": 5})  # type: ignore[arg-type]

        # Test with None value
        with pytest.raises(TypeError, match="All filter_dict values must be strings"):
            memory._dict_to_filter_string({"deleted_at": None})  # type: ignore[arg-type]

        # Test with list value
        with pytest.raises(TypeError, match="All filter_dict values must be strings"):
            memory._dict_to_filter_string({"tags": ["tag1", "tag2"]})  # type: ignore[arg-type]

        # Test with boolean value
        with pytest.raises(TypeError, match="All filter_dict values must be strings"):
            memory._dict_to_filter_string({"active": True})  # type: ignore[arg-type]
    ...
    def test_dict_to_filter_string_with_non_string_key(self, mock_client):
        ...
        with pytest.raises(TypeError, match="All filter_dict keys must be strings"):
            memory._dict_to_filter_string({123: "value"})  # type: ignore[arg-type]
    ...
    def test_get_default_filter_dict_with_partial_fields(self, mock_client):
        ...
        memory = Memory(
            ...
            org_id="test_org",
            project_id="test_project",
            metadata={"user_id": "user1", "agent_id": None, "session_id": "session1"},  # type: ignore[arg-type]
        )
