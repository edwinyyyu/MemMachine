...
describe("Handle API Error", () => {
  it("should throw MemMachineAPIError with message from AxiosError", () => {
    ...
    expect(() => handleAPIError(axiosError, "Context")).toThrow(
      MemMachineAPIError,
    );
    expect(() => handleAPIError(axiosError, "Context")).toThrow(
      "Context: Axios error - Extra detail",
    );
  });
  ...
  it("should throw MemMachineAPIError with message from Error", () => {
    ...
    expect(() => handleAPIError(error, "Context")).toThrow(
      "Context: Test error",
    );
  });
  ...
  it("should throw MemMachineAPIError with stringified error for other error", () => {
    ...
    expect(() => handleAPIError(errorObj, "Context")).toThrow(
      MemMachineAPIError,
    );
    expect(() => handleAPIError(errorObj, "Context")).toThrow(
      'Context: {"foo":"bar"}',
