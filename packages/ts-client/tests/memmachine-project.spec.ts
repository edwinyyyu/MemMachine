...
describe("MemMachine Project", () => {
  ...
  it("should handle error when creating project", async () => {
    ...
    const project = client.project(mockProjectContext);
    jest
      .spyOn(project.client, "post")
      .mockRejectedValue(new Error("Network Error"));
    await expect(
      project.create({ description: "Test Project" }),
    ).rejects.toThrow(
  ...
  it("should handle error when getting project", async () => {
    ...
    await expect(project.get()).rejects.toThrow(
      /Failed to get project with payload: .*: Network Error/,
    );
  });
  ...
  it("should handle error when deleting project", async () => {
    ...
    await expect(project.delete()).rejects.toThrow(
      /Failed to delete project with payload: .*: Network Error/,
    );
  });
});
