...
def mock_memory_managers(monkeypatch):
    ...
    mock_inst.get_memory_context.return_value = MagicMock(
        group_id="g", session_id="s"
    )
    mock_inst.query_memory = AsyncMock(
        return_value=([], [], ["EpisodicMemory"])
    )
    ...
    monkeypatch.setattr(
        app_module, "episodic_memory", DummyEpisodicMemoryManager()
    )
    ...
    # up.
    monkeypatch.setattr(
        app_module, "AsyncEpisodicMemory", MockAsyncEpisodicMemory
    )
...
def test_post_memories_missing_required_field(
    valid_post_payload, missing_field
):
...
def test_episodic_memory_search_valid(valid_query_payload):
    ...
    response = client.post(
        "/v1/memories/episodic/search", json=valid_query_payload
    )
    assert response.status_code in (200, 201, 204)
    rsp = response.json()["content"]
    assert len(rsp) == 1
...
def test_profile_memory_search_valid(valid_query_payload):
    ...
    """
    response = client.post(
        "/v1/memories/profile/search", json=valid_query_payload
    )
    assert response.status_code in (200, 201, 204)
    rsp = response.json()["content"]
    assert len(rsp) == 1
...
def test_delete_memories_valid(valid_delete_payload):
    response = client.request(
        "DELETE", "/v1/memories", json=valid_delete_payload
    )
    assert response.status_code in (200, 201, 204)
